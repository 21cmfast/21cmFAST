...
from py21cmfast import UserParams, FlagOptions, CosmoParams, AstroParams
...
flag_options = FlagOptions({"INHOMO_RECO":True, 'USE_MASS_DEPENDENT_ZETA':True, 'USE_TS_FLUCT':True, 'USE_MINI_HALOS':True}, USE_VELS_AUX=True)
...
for h2  in range(200, 1500, 100):
     ...
     mem = estimate_memory_lightcone(max_redshift=1420/50-1, redshift=1420/200-1, user_params=user_params, flag_options=flag_options, cosmo_params=CosmoParams(), astro_params=AstroParams())
     mems4.append(mem)
...
plt.scatter(1300, mem['peak_memory']/1024**3, marker='*', color='k', label='Proposed Sims', s=100, zorder=3)

plt.scatter(1300, 2924.847, marker='+', color='r', label='Measured Peak RAM', s=75, zorder=3)

theory = h2dim**3
plt.plot(h2dim, theory * (peaks3.max() / theory.max()) * 1.2, label='HII_DIM^3 scaling', color='grey', ls='--')
