...
the code in v3, you can either set ``SOURCE_MODEL`` to ``"CONST-ION-EFF"`` or ``"E-INTEGRAL"``,
the former (latter) corresponds to setting ``USE_MASS_DEPENDENT_ZETA`` to ``False`` (``True``) in v3.
