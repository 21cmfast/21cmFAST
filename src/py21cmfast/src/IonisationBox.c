...
#pragma omp for reduction(+ : global_xH)
        ...
        {
            const char *label = USE_CUDA ? "IB_GPU" : "IB_CPU";
            double sum, sum2, val, mean, std, vmin, vmax;
            unsigned long long ii;

            // neutral_fraction
            sum = 0.0; sum2 = 0.0; vmin = box->neutral_fraction[0]; vmax = vmin;
            ...
            // ionisation_rate_G12
            if (box->ionisation_rate_G12 != NULL) {
                sum = 0.0; sum2 = 0.0; vmin = box->ionisation_rate_G12[0]; vmax = vmin;
                for (ii = 0; ii < HII_TOT_NUM_PIXELS; ii++) {
                    val = box->ionisation_rate_G12[ii];
                    sum += val; sum2 += val * val;
                    if (val < vmin) vmin = val; if (val > vmax) vmax = val;
                }
                mean = sum / HII_TOT_NUM_PIXELS; std = sqrt(sum2 / HII_TOT_NUM_PIXELS - mean * mean);
                fprintf(stderr, "[DIAG] %s ionisation_rate_G12 mean=%.6e std=%.6e min=%.6e max=%.6e\n", label, mean, std, vmin, vmax);
            }

            // mean_free_path
            if (box->mean_free_path != NULL) {
                sum = 0.0; sum2 = 0.0; vmin = box->mean_free_path[0]; vmax = vmin;
                for (ii = 0; ii < HII_TOT_NUM_PIXELS; ii++) {
                    val = box->mean_free_path[ii];
                    sum += val; sum2 += val * val;
                    if (val < vmin) vmin = val; if (val > vmax) vmax = val;
                }
                mean = sum / HII_TOT_NUM_PIXELS; std = sqrt(sum2 / HII_TOT_NUM_PIXELS - mean * mean);
                fprintf(stderr, "[DIAG] %s mean_free_path mean=%.6e std=%.6e min=%.6e max=%.6e\n", label, mean, std, vmin, vmax);
            }

            // z_reion
            if (box->z_reion != NULL) {
                sum = 0.0; sum2 = 0.0; vmin = box->z_reion[0]; vmax = vmin;
                for (ii = 0; ii < HII_TOT_NUM_PIXELS; ii++) {
                    val = box->z_reion[ii];
                    sum += val; sum2 += val * val;
                    if (val < vmin) vmin = val; if (val > vmax) vmax = val;
                ...
                mean = sum / HII_TOT_NUM_PIXELS; std = sqrt(sum2 / HII_TOT_NUM_PIXELS - mean * mean);
                fprintf(stderr, "[DIAG] %s z_reion mean=%.6e std=%.6e min=%.6e max=%.6e\n", label, mean, std, vmin, vmax);
            }

            // cumulative_recombinations
            if (box->cumulative_recombinations != NULL) {
                sum = 0.0; sum2 = 0.0; vmin = box->cumulative_recombinations[0]; vmax = vmin;
                for (ii = 0; ii < HII_TOT_NUM_PIXELS; ii++) {
                    val = box->cumulative_recombinations[ii];
                    sum += val; sum2 += val * val;
                    if (val < vmin) vmin = val; if (val > vmax) vmax = val;
                }
                mean = sum / HII_TOT_NUM_PIXELS; std = sqrt(sum2 / HII_TOT_NUM_PIXELS - mean * mean);
                fprintf(stderr, "[DIAG] %s cumulative_recombinations mean=%.6e std=%.6e min=%.6e max=%.6e\n", label, mean, std, vmin, vmax);
            }

            // kinetic_temperature
            if (box->kinetic_temperature != NULL) {
                sum = 0.0; sum2 = 0.0; vmin = box->kinetic_temperature[0]; vmax = vmin;
                for (ii = 0; ii < HII_TOT_NUM_PIXELS; ii++) {
                    val = box->kinetic_temperature[ii];
                    sum += val; sum2 += val * val;
                    if (val < vmin) vmin = val; if (val > vmax) vmax = val;
                }
                mean = sum / HII_TOT_NUM_PIXELS; std = sqrt(sum2 / HII_TOT_NUM_PIXELS - mean * mean);
                fprintf(stderr, "[DIAG] %s kinetic_temperature mean=%.6e std=%.6e min=%.6e max=%.6e\n", label, mean, std, vmin, vmax);
            }
