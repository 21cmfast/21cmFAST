...
#define HII_ROUND_ERR \
  (1e-5)  // do not run excursion-set below this expected HII fraction
...
// TODO: Something like these structs should also replace the globals in
// SpinTemperature.c

// TODO: consider the case of having Radiusspec as array of structs (current)
//   vs struct of arrays. The former allows you to pass a single struct into
//   each function without an index so they don't need to know about other
//   radii. The second is simpler to understand/work with in terms of
//   allocation/scoping
struct RadiusSpec {
...
struct FilteredGrids {
  ...
  fftwf_complex *log10_Mturnover_MINI_unfiltered,
...
void set_ionbox_constants(double redshift, double prev_redshift,
                          CosmoParams *cosmo_params, UserParams *user_params,
  ...
  // The halo fields already have Fstar,Fesc,nion taken into account, so their
  // global factor differs from the local one
  ...
  //      else
  //          consts->mfp_meandens = 112 / cosmo_params->hlittle * pow(
  //          (1.+redshift) / 5. , -4.4);
  ...
  consts->M_min =
      minimum_source_mass(redshift, false, astro_params, flag_options);
  ...
  if (!flag_options->USE_TS_FLUCT) {
    ...
    // finding the adiabatic index at the initial redshift from 2302.08506 to
    // fix adiabatic fluctuations.
    consts->adia_TK_term = cT_approx(redshift);
  }
  consts->pixel_length =
      user_params_global->BOX_LEN / (double)user_params_global->HII_DIM;
  consts->pixel_mass =
      cosmo_params_global->OMm * RHOcrit * pow(consts->pixel_length, 3);

  consts->gamma_prefactor = pow(1 + redshift, 2) * CMperMPC * SIGMA_HI *
  ...
  if (flag_options->USE_HALO_FIELD)
    consts->gamma_prefactor /=
        RHOcrit *
        cosmo_params->OMb;  // TODO: double-check these unit differences,
                            // HaloBox.halo_wsfr vs Nion_General units
  else
    consts->gamma_prefactor /= sc.t_h / sc.t_star;
  consts->gamma_prefactor_mini = consts->gamma_prefactor *
                                 consts->ion_eff_factor_mini /
                                 consts->ion_eff_factor;
  LOG_SUPER_DEBUG("Gamma Prefactor %.3e ion eff factor %.3e",
                  consts->gamma_prefactor, consts->ion_eff_factor);
...
void allocate_fftw_grids(struct FilteredGrids **fg_struct) {
  *fg_struct = malloc(sizeof(**fg_struct));
  ...
  (*fg_struct)->deltax_unfiltered = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  (*fg_struct)->deltax_filtered = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

  if (flag_options_global->USE_MINI_HALOS &&
      !flag_options_global->USE_HALO_FIELD) {
    (*fg_struct)->prev_deltax_unfiltered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->prev_deltax_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

    (*fg_struct)->log10_Mturnover_unfiltered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->log10_Mturnover_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->log10_Mturnover_MINI_unfiltered =
        (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) *
                                      HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->log10_Mturnover_MINI_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  }

  if (flag_options_global->USE_TS_FLUCT) {
    (*fg_struct)->xe_unfiltered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->xe_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  }
  ...
  if (flag_options_global->INHOMO_RECO && !flag_options_global->CELL_RECOMB) {
    (*fg_struct)->N_rec_unfiltered = (fftwf_complex *)fftwf_malloc(
        ...
        HII_KSPACE_NUM_PIXELS);  // cumulative number of recombinations
    (*fg_struct)->N_rec_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  }

  if (flag_options_global->USE_HALO_FIELD) {
    (*fg_struct)->stars_unfiltered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->stars_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

    (*fg_struct)->sfr_unfiltered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    (*fg_struct)->sfr_filtered = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  }
...
void prepare_box_for_filtering(float *input_box, fftwf_complex *output_c_box,
                               double const_factor, double limit_min,
                               double limit_max) {
  int i, j, k;
...
#pragma omp parallel for private(i, j, k, curr_cell) \
  ...
  dft_r2c_cube(user_params_global->USE_FFTW_WISDOM, user_params_global->HII_DIM,
               HII_D_PARA, user_params_global->N_THREADS, output_c_box);
...
// Populating a previous box which has the required fields for the first
// snapshot:
//  Since the values of all arrays should be passed in as zero, we only assign
//  special first snapshot values here
void setup_first_z_prevbox(IonizedBox *previous_ionize_box,
                           PerturbedField *previous_perturb, int n_radii) {
...
#pragma omp for
  ...
  // previous delta and Fcoll are used for the trapezoidal integral when
  // USE_MINI_HALOS
  if (flag_options_global->USE_MINI_HALOS) {
...
void calculate_mcrit_boxes(IonizedBox *prev_ionbox, TsBox *spin_temp,
                           InitialConditions *ini_boxes,
                           struct IonBoxConstants *consts,
                           fftwf_complex *log10_mcrit_acg,
                           fftwf_complex *log10_mcrit_mcg,
                           double *avg_mturn_acg, double *avg_mturn_mcg) {
      ...
      for (y = 0; y < user_params_global->HII_DIM; y++) {
        for (z = 0; z < HII_D_PARA; z++) {
          Mcrit_RE = reionization_feedback(
              consts->redshift, prev_ionbox->Gamma12_box[HII_R_INDEX(x, y, z)],
              prev_ionbox->z_re_box[HII_R_INDEX(x, y, z)]);

          if (user_params_global->USE_RELATIVE_VELOCITIES &&
              !flag_options_global->FIX_VCB_AVG)
            curr_vcb = ini_boxes->lowres_vcb[HII_R_INDEX(x, y, z)];

          Mcrit_LW = lyman_werner_threshold(
              consts->redshift, spin_temp->J_21_LW_box[HII_R_INDEX(x, y, z)],
              curr_vcb, astro_params_global);
          if (Mcrit_LW != Mcrit_LW || Mcrit_LW == 0) {
            LOG_ERROR("Mcrit error %d %d %d: M %.2e z %.2f J %.2e v %.2e", x, y,
                      z, Mcrit_LW, consts->redshift,
                      spin_temp->J_21_LW_box[HII_R_INDEX(x, y, z)], curr_vcb);
            Throw(ValueError);
          }

          // JBM: this only accounts for effect 3 (largest on minihaloes).
          // Effects 1 and 2 affect both minihaloes (MCGs) and regular ACGs, but
          // they're smaller ~10%. See Sec 2 of Muñoz+21 (2110.13919)
          curr_Mt = log10(fmax(Mcrit_RE, consts->scale_consts.mturn_a_nofb));
          curr_Mt_MINI = log10(fmax(
              Mcrit_RE, fmax(Mcrit_LW, consts->scale_consts.mturn_m_nofb)));

          // To avoid allocating another box we directly assign turnover masses
          // to the fftw grid
...
// F_coll = f(z_current,Mturn_current) - f(z_previous,Mturn_current) +
// f(z_previous,Mturn_previous) all mturns are average log10 over the the
// `limit` outputs are set to the total value at the maximum redshift and
// current turnover, these form a lower limit on any grid cell
void set_mean_fcoll(struct IonBoxConstants *c, IonizedBox *prev_box,
                    IonizedBox *curr_box, double mturn_acg, double mturn_mcg,
  ...
  if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
    f_coll_curr =
        Nion_General(c->redshift, c->lnMmin, c->lnMmax_gl, mturn_acg, sc_ptr);
    *f_limit_acg = Nion_General(user_params_global->Z_HEAT_MAX, c->lnMmin,
                                c->lnMmax_gl, mturn_acg, sc_ptr);

    if (flag_options_global->USE_MINI_HALOS) {
      if (prev_box->mean_f_coll * c->ion_eff_factor_gl < 1e-4) {
        // we don't have enough ionising radiation in the previous snapshot,
        // just take the current value
        curr_box->mean_f_coll = f_coll_curr;
      } else {
        f_coll_prev = Nion_General(c->prev_redshift, c->lnMmin, c->lnMmax_gl,
                                   mturn_acg, sc_ptr);
        curr_box->mean_f_coll =
            prev_box->mean_f_coll + f_coll_curr - f_coll_prev;
      }
      f_coll_curr_mini = Nion_General_MINI(c->redshift, c->lnMmin, c->lnMmax_gl,
                                           mturn_mcg, sc_ptr);
      if (prev_box->mean_f_coll_MINI * c->ion_eff_factor_gl < 1e-4) {
        curr_box->mean_f_coll_MINI = f_coll_curr_mini;
      } else {
        f_coll_prev_mini = Nion_General_MINI(c->prev_redshift, c->lnMmin,
                                             c->lnMmax_gl, mturn_mcg, sc_ptr);
        curr_box->mean_f_coll_MINI =
            prev_box->mean_f_coll_MINI + f_coll_curr_mini - f_coll_prev_mini;
      }
      *f_limit_mcg =
          Nion_General_MINI(user_params_global->Z_HEAT_MAX, c->lnMmin,
                            c->lnMmax_gl, mturn_mcg, sc_ptr);
    } else {
      curr_box->mean_f_coll = f_coll_curr;
    ...
    *f_limit_acg =
        Fcoll_General(user_params_global->Z_HEAT_MAX, c->lnMmin,
                      c->lnMmax_gl);  // JD: the old parametrisation didn't have
                                      // this limit before
  }
  ...
  if (isfinite(curr_box->mean_f_coll) == 0 || curr_box->mean_f_coll < 0) {
    ...
    LOG_ERROR("prev box %g prev intgrl %g curr intrgl %g --> %g",
              prev_box->mean_f_coll, f_coll_prev, f_coll_curr,
              curr_box->mean_f_coll);
    Throw(InfinityorNaNError);
  }
  LOG_SUPER_DEBUG("excursion set normalisation, mean_f_coll: %e",
                  curr_box->mean_f_coll);
...
double set_fully_neutral_box(IonizedBox *box, TsBox *spin_temp,
...
#pragma omp for reduction(+ : global_xH)
      for (ct = 0; ct < HII_TOT_NUM_PIXELS; ct++) {
        box->xH_box[ct] =
            1. - spin_temp->x_e_box[ct];  // convert from x_e to xH
...
#pragma omp for
      for (ct = 0; ct < HII_TOT_NUM_PIXELS; ct++) {
        ...
        box->temp_kinetic_all_gas[ct] =
            consts->TK_nofluct *
            (1.0 + consts->adia_TK_term *
                       perturbed_field
                           ->density[ct]);  // Is perturbed_field defined
                                            // already here? we need it for cT.
                                            // I'm also assuming we don't need
                                            // to multiply by other z here.
      }
    }
  }
  return global_xH;
...
//   if the first is faster, consider reordering prepare_filter_grids() in the
//   same way if the second is faster, make a function to do this for one grid
void copy_filter_transform(struct FilteredGrids *fg_struct,
                           struct IonBoxConstants *consts,
  ...
  if (flag_options_global->USE_HALO_FIELD) {
    ...
    if (flag_options_global->USE_MINI_HALOS) {
      ...
      memcpy(fg_struct->log10_Mturnover_MINI_filtered,
             fg_struct->log10_Mturnover_MINI_unfiltered,
             sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
      memcpy(fg_struct->log10_Mturnover_filtered,
  ...
  if (rspec.R_index > 0) {
    ...
    if (flag_options_global->USE_HALO_FIELD) {
      int filter_hf =
          flag_options_global->USE_EXP_FILTER ? 3 : consts->hii_filter;
      filter_box(fg_struct->stars_filtered, 1, filter_hf, R,
                 consts->mfp_meandens);
      filter_box(fg_struct->sfr_filtered, 1, filter_hf, R,
                 consts->mfp_meandens);
    } else {
      if (flag_options_global->USE_MINI_HALOS) {
        filter_box(fg_struct->prev_deltax_filtered, 1, consts->hii_filter, R,
                   0.);
        filter_box(fg_struct->log10_Mturnover_MINI_filtered, 1,
                   consts->hii_filter, R, 0.);
        filter_box(fg_struct->log10_Mturnover_filtered, 1, consts->hii_filter,
                   R, 0.);
      }
    }
  }
  ...
  dft_c2r_cube(user_params_global->USE_FFTW_WISDOM, user_params_global->HII_DIM,
               HII_D_PARA, user_params_global->N_THREADS,
               fg_struct->deltax_filtered);
  if (flag_options_global->USE_HALO_FIELD) {
    dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
                 user_params_global->N_THREADS, fg_struct->stars_filtered);
    dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
                 user_params_global->N_THREADS, fg_struct->sfr_filtered);
  } else {
    if (flag_options_global->USE_MINI_HALOS) {
      dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                   user_params_global->HII_DIM, HII_D_PARA,
                   user_params_global->N_THREADS,
  ...
  if (flag_options_global->USE_TS_FLUCT) {
    dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
                 user_params_global->N_THREADS, fg_struct->xe_filtered);
  }
  if (consts->filter_recombinations) {
    dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
                 user_params_global->N_THREADS, fg_struct->N_rec_filtered);
  }
}

// After filtering the grids, we need to clip them to physical values and take
// the extrema for some interpolation tables
void clip_and_get_extrema(fftwf_complex *grid, double lower_limit,
                          double upper_limit, double *grid_min,
                          double *grid_max) {
...
void setup_integration_tables(struct FilteredGrids *fg_struct,
                              struct IonBoxConstants *consts,
                              struct RadiusSpec rspec, bool need_prev) {
  double min_density, max_density, prev_min_density, prev_max_density;
  double log10Mturn_min, log10Mturn_max, log10Mturn_min_MINI,
      log10Mturn_max_MINI;
  struct ScalingConstants *sc_ptr = &(consts->scale_consts);

  // TODO: instead of putting a random upper limit, put a proper flag for
  // switching of one/both sides of the clipping
  clip_and_get_extrema(fg_struct->deltax_filtered, -1, 1e6, &min_density,
                       &max_density);
  min_density -= 0.001;
  max_density += 0.001;
  ...
  if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
    if (flag_options_global->USE_MINI_HALOS) {
      ...
      clip_and_get_extrema(fg_struct->prev_deltax_filtered, -1, 1e6,
                           &prev_min_density, &prev_max_density);
      clip_and_get_extrema(fg_struct->log10_Mturnover_filtered, 0.,
                           LOG10_MTURN_MAX, &log10Mturn_min, &log10Mturn_max);
      clip_and_get_extrema(fg_struct->log10_Mturnover_MINI_filtered, 0.,
                           LOG10_MTURN_MAX, &log10Mturn_min_MINI,
                           &log10Mturn_max_MINI);
    }

    LOG_SUPER_DEBUG(
        "Tb limits d (%.2e,%.2e), m (%.2e,%.2e) t (%.2e,%.2e) tm (%.2e,%.2e)",
    ...
    if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
      // Buffers to avoid both zero bin widths and max cell segfault in 2D
      // interptables
      ...
      initialise_Nion_Conditional_spline(
          consts->redshift, min_density, max_density, consts->M_min,
          rspec.M_max_R, rspec.M_max_R, log10Mturn_min, log10Mturn_max,
          log10Mturn_min_MINI, log10Mturn_max_MINI, sc_ptr, false);

      // previous redshift tables if needed
      if (need_prev && flag_options_global->USE_MINI_HALOS) {
        // NOTE: we intentionally use the lower turnovers at this redshift, but
        // should we be doing the same for the upper turnover?
        initialise_Nion_Conditional_spline(
            consts->prev_redshift, prev_min_density, prev_max_density,
            consts->M_min, rspec.M_max_R, rspec.M_max_R, log10Mturn_min,
            log10Mturn_max, log10Mturn_min_MINI, log10Mturn_max_MINI, sc_ptr,
            true);
      }
    }
  } else {
    // This was previously one table for all R, which can be done with the EPS
    // mass function (and some others)
    // TODO: I don't expect this to be a bottleneck, but we can look into
    // re-making the 2/3D ERFC tables if needed
    if (user_params_global->USE_INTERPOLATION_TABLES > 1)
...
// TODO: We should speed test different configurations, separating grids,
...
void calculate_fcoll_grid(IonizedBox *box, IonizedBox *previous_ionize_box,
                          struct FilteredGrids *fg_struct,
                          struct IonBoxConstants *consts,
                          struct RadiusSpec *rspec) {
...
#pragma omp parallel num_threads(user_params_global->N_THREADS)
  {
    ...
    log10_Mturnover =
        log10(consts->scale_consts
                  .mturn_a_nofb);  // is only overwritten with minihalos
      ...
      for (y = 0; y < user_params_global->HII_DIM; y++) {
        for (z = 0; z < HII_D_PARA; z++) {
          ...
          *((float *)fg_struct->deltax_filtered + HII_R_FFT_INDEX(x, y, z)) =
              fmaxf(*((float *)fg_struct->deltax_filtered +
          ...
          if (consts->filter_recombinations) {
            *((float *)fg_struct->N_rec_filtered + HII_R_FFT_INDEX(x, y, z)) =
                fmaxf(*((float *)fg_struct->N_rec_filtered +
                        HII_R_FFT_INDEX(x, y, z)),
                      0.0);
          }
          ...
          if (flag_options_global->USE_TS_FLUCT) {
            *((float *)fg_struct->xe_filtered + HII_R_FFT_INDEX(x, y, z)) =
                fmaxf(*((float *)fg_struct->xe_filtered +
                        HII_R_FFT_INDEX(x, y, z)),
                      0.);
            *((float *)fg_struct->xe_filtered + HII_R_FFT_INDEX(x, y, z)) =
                fminf(*((float *)fg_struct->xe_filtered +
                        HII_R_FFT_INDEX(x, y, z)),
                      0.999);
          }
          ...
          if (flag_options_global->USE_HALO_FIELD) {
            *((float *)fg_struct->stars_filtered + HII_R_FFT_INDEX(x, y, z)) =
                fmaxf(*((float *)fg_struct->stars_filtered +
                        HII_R_FFT_INDEX(x, y, z)),
                      0.0);
            *((float *)fg_struct->sfr_filtered + HII_R_FFT_INDEX(x, y, z)) =
                fmaxf(*((float *)fg_struct->sfr_filtered +
                        HII_R_FFT_INDEX(x, y, z)),
                      0.0);

            // Ionising photon output
            Splined_Fcoll = *((float *)fg_struct->stars_filtered +
                              HII_R_FFT_INDEX(x, y, z));
            // Minihalos are taken care of already
            Splined_Fcoll_MINI = 0.;
            // The smoothing done with minihalos corrects for sudden changes in
            // M_crit Nion_smoothed(z,Mcrit) = Nion(z,Mcrit) +
            // (Nion(z_prev,Mcrit_prev) - Nion(z_prev,Mcrit))
            prev_Splined_Fcoll = 0.;
            prev_Splined_Fcoll_MINI = 0.;
          } else {
            curr_dens = *((float *)fg_struct->deltax_filtered +
                          HII_R_FFT_INDEX(x, y, z));
            if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
              if (flag_options_global->USE_MINI_HALOS) {
                log10_Mturnover =
                ...
                if (previous_ionize_box->mean_f_coll_MINI *
                  ...
                  prev_Splined_Fcoll_MINI = EvaluateNion_Conditional_MINI(
                      prev_dens, log10_Mturnover_MINI,
              ...
              Splined_Fcoll = EvaluateNion_Conditional(
                  curr_dens, log10_Mturnover, consts->growth_factor,
                  consts->M_min, rspec->M_max_R, rspec->M_max_R,
                  rspec->sigma_maxmass, sc_ptr, false);
            } else {
              Splined_Fcoll = EvaluateFcoll_delta(
                  curr_dens, consts->growth_factor, consts->sigma_minmass,
                  rspec->sigma_maxmass);
            }
          }
          // save the value of the collasped fraction into the Fcoll array
          // TODO: each of these grids are clipped before filtering, after
          // filtering,
          //  after the Fcoll integration, and after trapezoidal integration
          //  here we should figure which of these clips are necessary/useful
          if (flag_options_global->USE_MINI_HALOS &&
              !flag_options_global->USE_HALO_FIELD) {
            ...
            box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] =
                previous_ionize_box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS +
                                           HII_R_INDEX(x, y, z)] +
                Splined_Fcoll - prev_Splined_Fcoll;

            if (box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS +
                           HII_R_INDEX(x, y, z)] > 1.)
              box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] =
                  1.;
            // if (box->Fcoll[counter * HII_TOT_NUM_PIXELS + HII_R_INDEX(x,y,z)]
            // <0.) box->Fcoll[counter * HII_TOT_NUM_PIXELS +
            // HII_R_INDEX(x,y,z)] = 1e-40; if (box->Fcoll[counter *
            ...
            //     = previous_ionize_box->Fcoll[counter * HII_TOT_NUM_PIXELS +
            //     HII_R_INDEX(x,y,z)];
            f_coll_total += box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS +
                                       HII_R_INDEX(x, y, z)];
            if (isfinite(f_coll_total) == 0) {
              LOG_ERROR(
                  "f_coll is either infinite or NaN! %d %g (%d,%d,%d)?: dens "
                  "%g, prev %g",
              ...
              LOG_ERROR(
                  ...
                  box->Fcoll[fc_r_idx * HII_TOT_NUM_PIXELS +
                             HII_R_INDEX(x, y, z)],
                  log10_Mturnover,
                  *((float *)fg_struct->log10_Mturnover_filtered +
                    HII_R_FFT_INDEX(x, y, z)));
            ...
            if (box->Fcoll_MINI[fc_r_idx * HII_TOT_NUM_PIXELS +
                                HII_R_INDEX(x, y, z)] > 1.)
              box->Fcoll_MINI[fc_r_idx * HII_TOT_NUM_PIXELS +
                              HII_R_INDEX(x, y, z)] = 1.;
            // if (box->Fcoll_MINI[counter * HII_TOT_NUM_PIXELS +
            // HII_R_INDEX(x,y,z)] <0.) box->Fcoll_MINI[counter *
            // HII_TOT_NUM_PIXELS + HII_R_INDEX(x,y,z)] = 1e-40; if
            // (box->Fcoll_MINI[counter * HII_TOT_NUM_PIXELS +
            // HII_R_INDEX(x,y,z)] < previous_ionize_box->Fcoll_MINI[counter *
            // HII_TOT_NUM_PIXELS + HII_R_INDEX(x,y,z)])
            //     box->Fcoll_MINI[counter * HII_TOT_NUM_PIXELS +
            //     HII_R_INDEX(x,y,z)] = previous_ionize_box->Fcoll_MINI[counter
            //     * HII_TOT_NUM_PIXELS + HII_R_INDEX(x,y,z)];
            f_coll_MINI_total += box->Fcoll_MINI[fc_r_idx * HII_TOT_NUM_PIXELS +
                                                 HII_R_INDEX(x, y, z)];
            if (isfinite(f_coll_MINI_total) == 0) {
              LOG_ERROR(
                  "f_coll_MINI is either infinite or NaN %d R=%g (%d,%d,%d)?: "
                  "dens %g, prev %g",
                  rspec->R_index, rspec->R, x, y, z, curr_dens, prev_dens);
              LOG_ERROR(
                  "prev box %g intgrl %g curr int %g --> %g  l10mturn %g (%g)",
                  previous_ionize_box
                      ->Fcoll_MINI[fc_r_idx * HII_TOT_NUM_PIXELS +
                                   HII_R_INDEX(x, y, z)],
...
int setup_radii(struct RadiusSpec **rspec_array,
  ...
  if (flag_options_global->USE_HALO_FIELD &&
      !flag_options_global->IONISE_ENTIRE_SPHERE && (consts->pixel_length < 1))
    cell_length_factor = 1.;

  double minimum_radius = fmax(astro_params_global->R_BUBBLE_MIN,
                               cell_length_factor * consts->pixel_length);

  // minimum number such that min_R*delta^N > max_R
  int n_radii = (int)(log(maximum_radius / minimum_radius) /
                          log(astro_params_global->DELTA_R_HII_FACTOR) +
                      1);
  *rspec_array = malloc(sizeof(**rspec_array) * n_radii);
  ...
  for (i = 0; i < n_radii; i++) {
    ...
    (*rspec_array)[i].R =
        minimum_radius * pow(astro_params_global->DELTA_R_HII_FACTOR, i);
    // TODO: is this necessary? prevents the last step being small, but could
    // hide some
...
void find_ionised_regions(IonizedBox *box, IonizedBox *previous_ionize_box,
                          ...
                          struct RadiusSpec rspec,
                          struct IonBoxConstants *consts,
                          struct FilteredGrids *fg_struct, double f_limit_acg,
                          double f_limit_mcg) {
  ...
  LOG_SUPER_DEBUG(
      "global mean fcoll (mini) %.3e (%.3e) box mean fcoll %.3e (%.3e) ratio "
      "%.3e (%.3e)",
...
#pragma omp for
    for (x = 0; x < user_params_global->HII_DIM; x++) {
      for (y = 0; y < user_params_global->HII_DIM; y++) {
        for (z = 0; z < HII_D_PARA; z++) {
          // Use unfiltered density for CELL_RECOMB case, since the "Fcoll"
          // represents photons
          //   reaching the central cell rather than photons in the entire
          //   sphere
          // If we don't filter on the last step, might as well use the original
          // field to prevent aliasing?
          if (rspec.R_index == 0)
          ...
          // Since the halo boxes give ionising photon output, this term
          // accounts for the local density of absorbers
          //   We have separated the source/absorber filtering in the halo model
          //   so this is necessary
          if (flag_options_global->USE_HALO_FIELD)
            curr_fcoll *=
                1 / (RHOcrit * cosmo_params_global->OMb * (1 + curr_dens));

          // MINIHALOS are already included in the halo model
          curr_fcoll_mini = 0.;
          if (flag_options_global->USE_MINI_HALOS &&
              !flag_options_global->USE_HALO_FIELD) {
            curr_fcoll_mini = box->Fcoll_MINI[fc_r_idx * HII_TOT_NUM_PIXELS +
                                              HII_R_INDEX(x, y, z)];
          ...
          if (flag_options_global->INHOMO_RECO) {
            if (flag_options_global->CELL_RECOMB)
              ...
              rec = (*(
                  (float *)fg_struct->N_rec_filtered +
                  HII_R_FFT_INDEX(
                      x, y, z)));  // number of recombinations per mean baryon

            rec /= (1. + curr_dens);  // number of recombinations per baryon
                                      // inside cell/filter
          } else {
            rec = 0.;
          }

          // adjust the denominator of the collapse fraction for the residual
          // electron fraction in the neutral medium
          if (flag_options_global->USE_TS_FLUCT) {
            xHII_from_xrays =
                *((float *)fg_struct->xe_filtered + HII_R_FFT_INDEX(x, y, z));
          } else {
          ...
          if (x + y + z == 0) {
            LOG_SUPER_DEBUG(
                "Cell 0: R=%.1f | d %.4e | fcoll %.4e (%.4e) | rec %.4e | X "
                "%.4e",
                rspec.R, curr_dens, curr_fcoll, curr_fcoll_mini, rec,
                xHII_from_xrays);
          }
#endif
          ...
          if ((curr_fcoll * consts->ion_eff_factor +
            ...
            // if this is the first crossing of the ionization barrier for this
            // cell (largest R), record the gamma this assumes photon-starved
            // growth of HII regions...  breaks down post EoR
            if (flag_options_global->INHOMO_RECO &&
                (box->xH_box[HII_R_INDEX(x, y, z)] > FRACT_FLOAT_ERR)) {
              if (flag_options_global->USE_HALO_FIELD) {
                // since ION_EFF_FACTOR==1 here, gamma_prefactor is the same for
                // ACG and MCG
            ...
            if (!flag_options_global->IONISE_ENTIRE_SPHERE)  // center method
              ...
              update_in_sphere(box->xH_box, user_params_global->HII_DIM,
                               HII_D_PARA,
                               rspec.R / (user_params_global->BOX_LEN),
                               x / (user_params_global->HII_DIM + 0.0),
                               y / (user_params_global->HII_DIM + 0.0),
                               z / (HII_D_PARA + 0.0));
          }  // end ionized
             // If not fully ionized, then assign partial ionizations
          else if (rspec.R_index == 0 &&
                   (box->xH_box[HII_R_INDEX(x, y, z)] > TINY)) {
            // NOTE: Previously there was an RNG model here which multiplied
            // Fcoll by a sampled
            //   Poisson/<Poisson> term if 1/5 < M_coll / M_min < 5. This only
            //   ever affected the old parametrisation due to the M_min term.
            ...
            if (flag_options_global->USE_TS_FLUCT) {
              box->temp_kinetic_all_gas[HII_R_INDEX(x, y, z)] =
                  ComputePartiallyIoinizedTemperature(
                      spin_temp->Tk_box[HII_R_INDEX(x, y, z)], res_xH,
                      consts->T_re);
            } else {
              box->temp_kinetic_all_gas[HII_R_INDEX(x, y, z)] =
                  ComputePartiallyIoinizedTemperature(
...
void set_ionized_temperatures(IonizedBox *box, PerturbedField *perturbed_field,
                              TsBox *spin_temp,
                              struct IonBoxConstants *consts) {
  int x, y, z;
          ...
          if ((box->z_re_box[HII_R_INDEX(x, y, z)] > 0) &&
            ...
            box->temp_kinetic_all_gas[HII_R_INDEX(x, y, z)] =
                ComputeFullyIoinizedTemperature(
                    box->z_re_box[HII_R_INDEX(x, y, z)],
                    consts->stored_redshift,
                    perturbed_field->density[HII_R_INDEX(x, y, z)],
                    consts->T_re);
            // Below sometimes (very rare though) can happen when the density
            // drops too fast and to below T_HI
            if (flag_options_global->USE_TS_FLUCT) {
    ...
    for (y = 0; y < user_params_global->HII_DIM; y++) {
      for (z = 0; z < HII_D_PARA; z++) {
        if (isfinite(box->temp_kinetic_all_gas[HII_R_INDEX(x, y, z)]) == 0) {
          LOG_ERROR(
              "Tk after fully ioinzation is either infinite or a Nan. "
              "Something has gone wrong "
              "in the temperature calculation: z_re=%.4f, redshift=%.4f, "
              "curr_dens=%.4e",
...
#pragma omp for
    for (x = 0; x < user_params_global->HII_DIM; x++) {
      for (y = 0; y < user_params_global->HII_DIM; y++) {
        for (z = 0; z < HII_D_PARA; z++) {
          // use the original density and redshift for the snapshot (not the
          // adjusted redshift) Only want to use the adjusted redshift for the
          // ionisation field
          ...
          dNrec = splined_recombination_rate(
                      z_eff - 1., box->Gamma12_box[HII_R_INDEX(x, y, z)]) *
          ...
          if (isfinite(dNrec) == 0) {
            finite_error = true;
            LOG_ERROR(
                "RECOMB: non-finite cell (%d,%d,%d): d %.4e | G12 %.4e | xH "
                "%.4e ==> dNrec %.4e Nrec_last (%.4e --> %.4e)",
...
int ComputeIonizedBox(float redshift, float prev_redshift,
  ...
  Try {  // This Try brackets the whole function, so we don't indent.
    ...
    set_ionbox_constants(redshift, prev_redshift, cosmo_params, user_params,
                         astro_params, flag_options, &ionbox_constants);

    // boxes which aren't guaranteed to have every element assigned to need to
    // be initialised
...
#pragma omp parallel shared(box) private(ct) \
...
#pragma omp for
    ...
    debugSummarizeBox(box->z_re_box, user_params->HII_DIM, user_params->HII_DIM,
                      HII_D_PARA, "  ");

    // Modify the current sampled redshift to a redshift which matches the
    // expected filling factor given our astrophysical parameterisation. This is
    // the photon non-conservation correction
    float absolute_delta_z = 0.;
    float redshift_pc, stored_redshift_pc;
    if (flag_options->PHOTON_CONS_TYPE == 1) {
      redshift_pc = redshift;
      adjust_redshifts_for_photoncons(user_params, astro_params, flag_options,
      ...
      // TODO: if we want this to work with minihalos we need to change
      // prev_redshift too
      LOG_DEBUG("PhotonCons data:");
      LOG_DEBUG("original redshift=%f, updated redshift=%f delta-z = %f",
                stored_redshift_pc, redshift_pc, absolute_delta_z);
      if (isfinite(redshift_pc) == 0 || isfinite(absolute_delta_z) == 0) {
        LOG_ERROR(
            "Updated photon non-conservation redshift is either infinite or "
            "NaN!");
        LOG_ERROR(
            "This can sometimes occur when reionisation stalls (i.e. extremely "
            "low"
    ...
    if (prev_redshift < 1)
      setup_first_z_prevbox(previous_ionize_box, previous_perturbed_field,
                            n_radii);
    ...
    if (flag_options->USE_MASS_DEPENDENT_ZETA) {
      if (flag_options->USE_HALO_FIELD) {
        ...
        LOG_SUPER_DEBUG(
            "Calculating and outputting Mcrit boxes for atomic and molecular "
            "halos...");
        calculate_mcrit_boxes(
            previous_ionize_box, spin_temp, ini_boxes, &ionbox_constants,
        ...
        LOG_DEBUG(
            "average log10 turnover masses are %.2f and %.2f for ACGs and MCGs",
            box->log10_Mturnover_ave, box->log10_Mturnover_MINI_ave);
      } else {
    ...
    // lets check if we are going to bother with computing the inhmogeneous
    // field at all...
    global_xH = 0.0;
    ...
    if (user_params->USE_INTERPOLATION_TABLES > 0) {
      if (user_params->INTEGRATION_METHOD_ATOMIC == 2 ||
        ...
        initialiseSigmaMInterpTable(fmin(MMIN_FAST, ionbox_constants.M_min),
                                    1e20);
      else
    ...
    if (user_params->INTEGRATION_METHOD_ATOMIC == 1 ||
        (flag_options->USE_MINI_HALOS &&
         user_params->INTEGRATION_METHOD_MINI == 1))
      initialise_GL(ionbox_constants.lnMmin, ionbox_constants.lnMmax_gl);

    double f_limit_acg;
    double f_limit_mcg;
    set_mean_fcoll(&ionbox_constants, previous_ionize_box, box,
                   Mturnover_global_avg, Mturnover_global_avg_MINI,
                   &f_limit_acg, &f_limit_mcg);
    double exp_global_hii =
        box->mean_f_coll * ionbox_constants.ion_eff_factor_gl +
    ...
    if (exp_global_hii <
        HII_ROUND_ERR) {  // way too small to ionize anything...
      LOG_DEBUG(
          "Mean collapsed fraciton %.3e too small to ionize, stopping early",
          exp_global_hii);
      global_xH = set_fully_neutral_box(box, spin_temp, perturbed_field,
                                        &ionbox_constants);
    } else {
      // DO THE R2C TRANSFORMS
      // TODO: add debug average printing to these boxes
      // TODO: put a flag for to turn off clipping instead of putting the wide
      // limits
      prepare_box_for_filtering(
          perturbed_field->density, grid_struct->deltax_unfiltered,
          ionbox_constants.photoncons_adjustment_factor, -1., 1e6);
      if (flag_options->USE_HALO_FIELD) {
        prepare_box_for_filtering(halos->n_ion, grid_struct->stars_unfiltered,
                                  1., 0., 1e20);
        prepare_box_for_filtering(halos->whalo_sfr, grid_struct->sfr_unfiltered,
                                  1., 0., 1e20);
      } else {
        if (flag_options->USE_MINI_HALOS) {
          prepare_box_for_filtering(previous_perturbed_field->density,
                                    grid_struct->prev_deltax_unfiltered, 1., -1,
                                    1e6);
          // since the turnover mass boxes were assigned separately (they needed
          // more complex functions)...
          dft_r2c_cube(user_params->USE_FFTW_WISDOM, user_params->HII_DIM,
                       HII_D_PARA, user_params->N_THREADS,
                       grid_struct->log10_Mturnover_MINI_unfiltered);
          dft_r2c_cube(user_params->USE_FFTW_WISDOM, user_params->HII_DIM,
                       HII_D_PARA, user_params->N_THREADS,
                       grid_struct->log10_Mturnover_unfiltered);
          for (ct = 0; ct < HII_KSPACE_NUM_PIXELS; ct++) {
            grid_struct->log10_Mturnover_unfiltered[ct] /=
      ...
      if (flag_options->USE_TS_FLUCT) {
        prepare_box_for_filtering(spin_temp->x_e_box,
      ...
      // *************************** //
      // *************************************************************************************
      // // set the max radius we will use, making sure we are always sampling
      // the same values of radius (this avoids aliasing differences w redshift)
      ...
      for (R_ct = n_radii; R_ct--;) {
        ...
        // TODO: As far as I can tell, This was the previous behaviour with the
        // while loop
        //   So if the cell size is smaller than the minimum mass (rare) we
        //   still filter the last step and don't assign any partial ionisaitons
        if (ionbox_constants.M_min > RtoM(curr_radius.R)) {
          LOG_DEBUG(
              "Radius %.2e Mass %.2e smaller than minimum %.2e, stopping...",
              radii_spec[R_ct].R, radii_spec[R_ct].M_max_R,
              ionbox_constants.M_min);
          break;
        }
        ...
        bool need_prev_ion = flag_options->USE_MINI_HALOS &&
                             (previous_ionize_box->mean_f_coll_MINI *
                                  ...
                                  previous_ionize_box->mean_f_coll *
                                      ionbox_constants.ion_eff_factor_gl >
                              1e-4);
        ...
        calculate_fcoll_grid(box, previous_ionize_box, grid_struct,
                             &ionbox_constants, &curr_radius);
        // To avoid ST_over_PS becoming nan when f_coll = 0, I set f_coll =
        // FRACT_FLOAT_ERR.
        ...
        find_ionised_regions(box, previous_ionize_box, perturbed_field,
                             spin_temp, curr_radius, &ionbox_constants,
        ...
        debugSummarizeBox(box->z_re_box, user_params->HII_DIM,
                          user_params->HII_DIM, HII_D_PARA, "  ");
#endif
      }
      set_ionized_temperatures(box, perturbed_field, spin_temp,
                               &ionbox_constants);
...
#pragma omp for reduction(+ : global_xH)
      ...
      if (isfinite(global_xH) == 0) {
        LOG_ERROR(
            "Neutral fraction is either infinite or a Nan. Something has gone "
            "wrong in the ionisation calculation!");
        Throw(InfinityorNaNError);
      }
      ...
      if (!ionbox_constants.fix_mean) {
        // if we don't fix the mean, make the mean_f_coll in the output reflect
        // the box since currently it is the global expected mean from the
        // Unconditional MF
    ...
    if (!flag_options->USE_TS_FLUCT &&
        user_params->USE_INTERPOLATION_TABLES > 0) {
      freeSigmaMInterpTable();
    }

    // This function checks for allocation so don't worry about double-freeing
    // tables
