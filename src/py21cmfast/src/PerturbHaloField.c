...
int ComputePerturbHaloField(float redshift, UserParams *user_params,
  ...
  Try {  // This Try brackets the whole function, so we don't indent.
    ...
    float growth_factor, displacement_factor_2LPT, xf, yf, zf,
        growth_factor_over_BOX_LEN, displacement_factor_2LPT_over_BOX_LEN;
    ...
    displacement_factor_2LPT_over_BOX_LEN =
        displacement_factor_2LPT / user_params->BOX_LEN;

    // now add the missing factor of Ddot to velocity field
#pragma omp parallel shared(boxes, dimension,                                \
                                growth_factor_over_BOX_LEN) private(i, j, k) \
          ...
          for (k = 0; k < (unsigned long long)(user_params->NON_CUBIC_FACTOR *
            ...
            if (user_params->PERTURB_ON_HIGH_RES) {
              ...
              boxes->hires_vz[R_INDEX(i, j, k)] *=
                  (growth_factor_over_BOX_LEN / user_params->NON_CUBIC_FACTOR);
            } else {
...
#pragma omp parallel shared(boxes, displacement_factor_2LPT_over_BOX_LEN, \
...
#pragma omp parallel shared(boxes, halos, halos_perturbed) private( \
        i_halo, i, j, k, xf, yf, zf) num_threads(user_params -> N_THREADS)
    {
#pragma omp for
...
#pragma omp parallel shared(                                        \
        boxes, growth_factor_over_BOX_LEN, dimension,               \
