...
__constant__ double d_delta_c_sph;
__constant__ double d_delta_c_delos;
...
void updateGlobalParams(SimulationOptions *h_simulation_options, MatterOptions * h_matter_options, CosmoParams *h_cosmo_params, AstroParams *h_astro_params){
    ...
    cudaMemcpyToSymbol(d_delta_c_sph, &physconst.delta_c_sph, sizeof(double), 0, cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(d_delta_c_delos, &physconst.delta_c_delos, sizeof(double), 0, cudaMemcpyHostToDevice);
}
