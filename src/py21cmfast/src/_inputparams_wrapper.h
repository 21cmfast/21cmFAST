/*We need to explicitly define the types used by the warpper using ffi.cdef()
    However, that function does not take directives, so we separate the types
   here
*/
// WARNING: DO NOT #include THIS FILE IN THE C CODE EXCEPT FOR IN
// InputParameters.h
...
/* Previously, we had a few structures spread throughout the code e.g
   user_params_ufunc which were all globally defined and separately broadcast at
   different times. Several of these were used across different files and some
   inside #defines (e.g indexing.h), so for now I've combined the parameter
   structures to avoid confusion (we shouldn't have the possibility of two files
   using different parameters).

   In future we should have a parameter structure in each .c file containing
   ONLY parameters relevant to it (look at HaloBox.c), and force the broadcast
   at each _compute() step (or even decorate any library call) However this
   would require us to be very careful about initialising the globals when ANY
   function from that file is called */
