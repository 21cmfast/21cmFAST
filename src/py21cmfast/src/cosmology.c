/* This file contatins functions regarding the matter power-sepctrum and
 * cosmology */
#include "cosmology.h"
...
// These globals hold values initialised in init_ps() and used throughout the
// rest of the file
static double sigma_norm, theta_cmb, omhh, z_equality, y_d, sound_horizon,
    alpha_nu, f_nu, f_baryon, beta_c, d2fact, R_CUTOFF;

/*
  this function reads the z=0 matter (CDM+baryons)  and relative velocity
  transfer functions from CLASS (from a file) flag_int = 0 to initialize
  interpolator, flag_int = -1 to free memory, flag_int = else to interpolate.
  flag_dv = 0 to output density, flag_dv = 1 to output velocity.
  similar to built-in function "double T_RECFAST(float z, int flag)"
*/

// FUNCTION TFmdm is the power spectrum transfer function from Eisenstein & Hu
// ApJ, 1999, 511, 5
double TFmdm(double k) {
  double q, gamma_eff, q_eff, TF_m, q_nu;

  q = k * pow(theta_cmb, 2) / omhh;
  gamma_eff = sqrt(alpha_nu) +
              (1.0 - sqrt(alpha_nu)) / (1.0 + pow(0.43 * k * sound_horizon, 4));
  q_eff = q / gamma_eff;
  TF_m = log(E + 1.84 * beta_c * sqrt(alpha_nu) * q_eff);
  TF_m /=
      TF_m + pow(q_eff, 2) * (14.4 + 325.0 / (1.0 + 60.5 * pow(q_eff, 1.11)));
...
double TF_CLASS(double k, int flag_int, int flag_dv) {
  ...
  if (k > kclass[CLASS_LENGTH - 1]) {  // k>kmax
    LOG_WARNING(
        "Called TF_CLASS with k=%f, larger than kmax! Returning value at kmax.",
        k);
    if (flag_dv == 0) {  // output is density
      return (Tmclass[CLASS_LENGTH - 1] / kclass[CLASS_LENGTH - 1] /
              kclass[CLASS_LENGTH - 1]);
    } else if (flag_dv == 1) {  // output is rel velocity
      return (Tvclass_vcb[CLASS_LENGTH - 1] / kclass[CLASS_LENGTH - 1] /
              kclass[CLASS_LENGTH - 1]);
    }  // we just set it to the last value, since sometimes it wants large k for
       // R<<cell_size, which does not matter much.
    else {
...
// FUNCTION sigma_z0(M)
// Returns the standard deviation of the normalized, density excess (delta(x))
// field, smoothed on the comoving scale of M (see filter definitions for M<->R
// conversion). The sigma is evaluated at z=0, with the time evolution contained
// in the dicke(z) factor, i.e. sigma(M,z) = sigma_z0(m) * dicke(z)

// normalized so that sigma_z0(M->8/h Mpc) = SIGMA8 in
// ../Parameter_files/COSMOLOGY.H

// NOTE: volume is normalized to = 1, so this is equvalent to the mass standard
// deviation
...
double dsigma_dk(double k, void *params) {
  ...
  if (user_params_global->POWER_SPECTRUM == 0) {  // Eisenstein & Hu
    T = TFmdm(k);
    // check if we should cuttoff power spectrum according to Bode et al. 2000
    // transfer function
    // TODO: revisit the WDM model perhaps as another power spectrum option
    // (which may also alter Mturn)
    // if (P_CUTOFF) T *= pow(1 + pow(BODE_e*k*R_CUTOFF, 2*BODE_v),
    // -BODE_n/BODE_v);
    ...
    gamma = cosmo_params_global->OMm * cosmo_params_global->hlittle *
            pow(E, -(cosmo_params_global->OMb) -
                       (cosmo_params_global->OMb / cosmo_params_global->OMm));
    ...
    T = (log(1.0 + 2.34 * q) / (2.34 * q)) *
        pow(1.0 + 3.89 * q + pow(16.1 * q, 2) + pow(5.46 * q, 3) +
            ...
            -0.25);
    p = pow(k, cosmo_params_global->POWER_INDEX) * T * T;
  } else if (user_params_global->POWER_SPECTRUM ==
             2) {  // Efstathiou,G., Bond,J.R., and White,S.D.M., MNRAS,258,1P
                   // (1992)
    gamma = 0.25;
    aa = 6.4 / (cosmo_params_global->hlittle * gamma);
    bb = 3.0 / (cosmo_params_global->hlittle * gamma);
    cc = 1.7 / (cosmo_params_global->hlittle * gamma);
    p = pow(k, cosmo_params_global->POWER_INDEX) /
        pow(1 + pow(aa * k + pow(bb * k, 1.5) + pow(cc * k, 2), 1.13),
            2.0 / 1.13);
    ...
    p = pow(k, cosmo_params_global->POWER_INDEX) /
        pow(1 + aa * k + bb * k * k, 2);
             ...
             4) {  // White, SDM and Frenk, CS, 1991, 379, 52
    gamma = cosmo_params_global->OMm * cosmo_params_global->hlittle *
            pow(E, -(cosmo_params_global->OMb) -
                       (cosmo_params_global->OMb / cosmo_params_global->OMm));
    ...
    p = pow(k, cosmo_params_global->POWER_INDEX) * 19400.0 /
        pow(1 + aa * k + bb * pow(k, 1.5) + cc * k * k, 2);
  } else if (user_params_global->POWER_SPECTRUM == 5) {  // output of CLASS
    T = TF_CLASS(
        k, 1,
        0);  // read from z=0 output of CLASS. Note, flag_int = 1 here always,
             // since now we have to have initialized the interpolator for CLASS
    p = pow(k, cosmo_params_global->POWER_INDEX) * T * T;
    if (user_params_global
            ->USE_RELATIVE_VELOCITIES) {  // jbm:Add average relvel suppression
      p *= 1.0 -
           A_VCB_PM * exp(-pow(log(k / KP_VCB_PM), 2.0) /
                          (2.0 * SIGMAK_VCB_PM * SIGMAK_VCB_PM));  // for v=vrms
    }
  } else {
    LOG_ERROR("No such power spectrum defined: %i. Output is bogus.",
  ...
  if ((user_params_global->FILTER == 0) || (sigma_norm < 0)) {  // top hat
    ...
    LOG_ERROR("No such filter: %i. Output is bogus.",
              user_params_global->FILTER);
    Throw(ValueError);
  }
  return k * k * p * w * w;
}
double sigma_z0(double M) {
  ...
  }  // we establish a maximum k of KTOP_CLASS~1e3 Mpc-1 and a minimum at
     // KBOT_CLASS,~1e-5 Mpc-1 since the CLASS transfer function has a max!
  else {
...
void TFset_parameters() {
  ...
  z_drag =
      1 + z_drag * pow(cosmo_params_global->OMb * cosmo_params_global->hlittle *
                           cosmo_params_global->hlittle,
                       0.238 * pow(omhh, 0.223));
  z_drag *= 1291 * pow(omhh, 0.251) / (1 + 0.659 * pow(omhh, 0.828));

  y_d = (1 + z_equality) / (1.0 + z_drag);

  R_drag = 31.5 * cosmo_params_global->OMb * cosmo_params_global->hlittle *
           cosmo_params_global->hlittle * pow(theta_cmb, -4) * 1000 /
           (1.0 + z_drag);
  R_equality = 31.5 * cosmo_params_global->OMb * cosmo_params_global->hlittle *
               cosmo_params_global->hlittle * pow(theta_cmb, -4) * 1000 /
               (1.0 + z_equality);
...
double power_in_k(double k) {
  ...
  if (user_params_global->POWER_SPECTRUM == 0) {  // Eisenstein & Hu
    T = TFmdm(k);
    // check if we should cuttoff power spectrum according to Bode et al. 2000
    // transfer function if (P_CUTOFF) T *= pow(1 + pow(BODE_e*k*R_CUTOFF,
    // 2*BODE_v), -BODE_n/BODE_v);
    p = pow(k, cosmo_params_global->POWER_INDEX) * T * T;
...
double power_in_vcb(double k) {
  ...
  if (user_params_global->POWER_SPECTRUM == 5) {  // CLASS
    T = TF_CLASS(k, 1, 1);  // read from CLASS file. flag_int=1 since we have
                            // initialized before, flag_vcb=1 for velocity
    p = pow(k, cosmo_params_global->POWER_INDEX) * T * T;
  } else {
    LOG_ERROR(
        "Cannot get P_cb unless using CLASS: %i\n Set USE_RELATIVE_VELOCITIES "
        "0 or use CLASS.\n",
        user_params_global->POWER_SPECTRUM);
    Throw(ValueError);
  }
...
void init_ps() {
  ...
  omhh = cosmo_params_global->OMm * cosmo_params_global->hlittle *
         cosmo_params_global->hlittle;
  theta_cmb = T_cmb / 2.7;
  ...
  LOG_DEBUG(
      "Initializing Power Spectrum with lower_limit=%e, upper_limit=%e, "
      "rel_tol=%e, radius_8=%g",
  ...
  sigma_norm = cosmo_params_global->SIGMA_8 /
               sqrt(result);  // takes care of volume factor
  return;
}
...
double dsigmasq_dm(double k, void *params) {
  ...
  // get the power spectrum.. choice of 5:
  if (user_params_global->POWER_SPECTRUM ==
      0) {  // Eisenstein & Hu ApJ, 1999, 511, 5
    T = TFmdm(k);
    // check if we should cuttoff power spectrum according to Bode et al. 2000
    // transfer function if (.P_CUTOFF) T *= pow(1 + pow(BODE_e*k*R_CUTOFF,
    // 2*BODE_v), -BODE_n/BODE_v);
    ...
    gamma = cosmo_params_global->OMm * cosmo_params_global->hlittle *
            pow(E, -(cosmo_params_global->OMb) -
                       (cosmo_params_global->OMb) / (cosmo_params_global->OMm));
    aa = 8.0 / (cosmo_params_global->hlittle * gamma);
    bb = 4.7 / (cosmo_params_global->hlittle *
                gamma);  // NOTE: Different here than the other two functions???
    p = pow(k, cosmo_params_global->POWER_INDEX) /
        pow(1 + aa * k + bb * k * k, 2);
  } else if (user_params_global->POWER_SPECTRUM ==
             4) {  // White, SDM and Frenk, CS, 1991, 379, 52
    ...
    cc = 1.0 / pow(cosmo_params_global->hlittle * gamma, 2);
    p = pow(k, cosmo_params_global->POWER_INDEX) * 19400.0 /
        pow(1 + aa * k + pow(bb * k, 1.5) + cc * k * k,
            2);  // NOTE: Different here than the other two functions???
  ...
  if (user_params_global->FILTER == 0) {  // top hat
    ...
    if ((kR) < 1.0e-10) {
      ...
      dwdr = 9 * cos(kR) * k / pow(kR, 3) +
             3 * sin(kR) * (1 - 3 / (kR * kR)) / (kR * Radius);
    }
    // 3*k*( 3*cos(kR)/pow(kR,3) + sin(kR)*(-3*pow(kR, -4) + 1/(kR*kR)) );}
    //      dwdr = -1e8 * k / (R*1e3);
    drdm =
        1.0 / (4.0 * PI * cosmo_params_global->OMm * RHOcrit * Radius * Radius);
  } else if (user_params_global->FILTER == 2) {  // gaussian of width 1/R
    w = pow(E, -kR * kR / 2.0);
    dwdr = -k * kR * w;
    drdm = 1.0 / (pow(2 * PI, 1.5) * cosmo_params_global->OMm * RHOcrit * 3 *
                  Radius * Radius);
  } else {
    LOG_ERROR("No such filter: %i. Output is bogus.",
              user_params_global->FILTER);
...
double dsigmasqdm_z0(double M) {
  ...
  if (user_params_global->POWER_SPECTRUM ==
      5) {  // for CLASS we do not need to renormalize the sigma integral.
    d2fact = 1.0;
  } else {
...
double M_J_WDM() {
  ...
  return fudge * 3.06e8 * (1.5 / g_x) *
         sqrt((cosmo_params_global->OMm - cosmo_params_global->OMb) *
              cosmo_params_global->hlittle * cosmo_params_global->hlittle /
              0.15) *
...
double ddicke_dz(double z) {
  float dz = 1e-10;
  // NOTE: this isnt called a lot, I'm not sure why we don't use a similar
  // method to ddicke_dt
...
double MtoR(double M) {
  ...
  if (user_params_global->FILTER == 0)  // top hat M = (4/3) PI <rho> R^3
    return pow(3 * M / (4 * PI * cosmo_params_global->OMm * RHOcrit),
               1.0 / 3.0);
  else if (user_params_global->FILTER == 2)  // gaussian: M = (2PI)^1.5 <rho>
                                             // R^3
    return pow(M / (pow(2 * PI, 1.5) * cosmo_params_global->OMm * RHOcrit),
               1.0 / 3.0);
  else  // filter not defined
    LOG_ERROR("No such filter = %i. Results are bogus.",
              user_params_global->FILTER);
  Throw(ValueError);
}
...
double omega_mz(float z) {
  return cosmo_params_global->OMm * pow(1 + z, 3) /
         (cosmo_params_global->OMm * pow(1 + z, 3) + cosmo_params_global->OMl +
          ...
          cosmo_params_global->OMk * pow(1 + z, 2));
}

/* Physical (non-linear) overdensity at virialization (relative to critical
 density) i.e. answer is rho / rho_crit In Einstein de sitter model = 178
...
double TtoM(double z, double T, double mu) {
  return 7030.97 / (cosmo_params_global->hlittle) *
         ...
         pow(T / (mu * (1 + z)), 1.5);
  /*  if (!SUPRESS || (z >= z_re) ) // pre-reionization or don't worry about
   supression return 7030.97 / hlittle * sqrt( omega_mz(z) /
   (OMm*Deltac_nonlinear(z)) ) * pow( T/(mu * (1+z)), 1.5 );
...
/*
 FUNCTION dicke(z)
 Computes the dicke growth function at redshift z, i.e. the z dependance part of
 sigma
...
double dicke(double z) {
  ...
  if (fabs(cosmo_params_global->OMm - 1.0) <
      tiny) {  // OMm = 1 (Einstein de-Sitter)
    return 1.0 / (1.0 + z);
    ...
    // this is a flat, cosmological CONSTANT universe, with only lambda, matter
    // and radiation it is taken from liddle et al.
    omegaM_z =
        cosmo_params_global->OMm * pow(1 + z, 3) /
        (cosmo_params_global->OMl + cosmo_params_global->OMm * pow(1 + z, 3) +
         cosmo_params_global->OMr * pow(1 + z, 4));
    dick_z = 2.5 * omegaM_z /
             (1.0 / 70.0 + omegaM_z * (209 - omegaM_z) / 140.0 +
              pow(omegaM_z, 4.0 / 7.0));
    dick_0 =
        2.5 * cosmo_params_global->OMm /
        (1.0 / 70.0 +
         cosmo_params_global->OMm * (209 - cosmo_params_global->OMm) / 140.0 +
    ...
    dick_0 = 1 + 3.0 / x_0 +
             3 * log(sqrt(1 + x_0) - sqrt(x_0)) * sqrt(1 + x_0) / pow(x_0, 1.5);
    x = fabs(1.0 / (cosmo_params_global->OMm + 0.0) - 1.0) / (1 + z);
    dick_z = 1 + 3.0 / x +
             3 * log(sqrt(1 + x) - sqrt(x)) * sqrt(1 + x) / pow(x, 1.5);
...
double dtdz(float z) {
  ...
  x = sqrt(cosmo_params_global->OMl / cosmo_params_global->OMm) *
      pow(1 + z, -3.0 / 2.0);
  dxdz = sqrt(cosmo_params_global->OMl / cosmo_params_global->OMm) *
         pow(1 + z, -5.0 / 2.0) * (-3.0 / 2.0);
  const1 = 2 * sqrt(1 + cosmo_params_global->OMm / cosmo_params_global->OMl) /
           (3.0 * Ho);
...
double ddickedt(double z) {
  ...
  return (dicke(z + dz) - dicke(z)) / dz /
         dtdz(z);  // lazy non-analytic form getting
  ...
  if (fabs(cosmo_params_global->OMm - 1.0) <
    ...
    ddickdz = (domegaMdz / (1 + z)) *
              (1.0 / 70.0 * pow(omegaM_z, -2) + 1.0 / 140.0 +
               3.0 / 7.0 * pow(omegaM_z, -10.0 / 3.0)) *
              pow(1.0 / 70.0 / omegaM_z + (209.0 - omegaM_z) / 140.0 +
                      pow(omegaM_z, -3.0 / 7.0),
                  -2);
    ddickdz -=
        pow(1 + z, -2) / (1.0 / 70.0 / omegaM_z + (209.0 - omegaM_z) / 140.0 +
                          pow(omegaM_z, -3.0 / 7.0));
