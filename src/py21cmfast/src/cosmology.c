...
double transfer_function(double k) {
    switch (matter_flags_global->POWER_SPECTRUM) {
        case 0:
        ...
        default:
            LOG_ERROR("No such power spectrum defined: %i", matter_flags_global->POWER_SPECTRUM);
            Throw(ValueError);
    }
}
...
double power_in_k_integrand(double k) {
    ...
    // if(matter_flags_global->POWER_SPECTRUM == 0)
    //   p = pow(k, POWER_INDEX - 0.05*log(k/0.05)) * T * T; //running, alpha=0.05

    // NOTE: USE_RELATIVE_VELOCITIES is only allowed if using CLASS
    if (matter_flags_global->POWER_SPECTRUM == 5 && matter_flags_global->USE_RELATIVE_VELOCITIES) {
...
double power_in_vcb(double k) {
    ...
    if (matter_flags_global->POWER_SPECTRUM == 5) {  // CLASS
        ...
        LOG_ERROR(
            ...
            "CLASS.\n",
            matter_flags_global->POWER_SPECTRUM);
        Throw(ValueError);
    }
...
// normalized so that sigma_z0(M->8/h Mpc) = SIGMA_8 in the MatterParams
...
double sigma_z0(double M) {
    ...
    struct SigmaIntegralParams sigma_params = {.radius = Radius,
                                               .filter_type = matter_flags_global->FILTER};
...
void init_ps() {
    ...
    if (matter_flags_global->POWER_SPECTRUM == 5) {
...
double MtoR(double M) {
    ...
    if (matter_flags_global->FILTER == 0)  // top hat M = (4/3) PI <rho> R^3
        return pow(3 * M / (4 * PI * cosmo_params_global->OMm * RHOcrit), 1.0 / 3.0);
    else if (matter_flags_global->FILTER == 2)  // gaussian: M = (2PI)^1.5 <rho> R^3
        return pow(M / (pow(2 * PI, 1.5) * cosmo_params_global->OMm * RHOcrit), 1.0 / 3.0);
    else  // filter not defined
        LOG_ERROR("No such filter = %i. Results are bogus.", matter_flags_global->FILTER);
    Throw(ValueError);
}
