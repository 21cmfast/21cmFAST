...
int dft_c2r_cube(bool use_wisdom, int dim, int dim_los, int n_threads, fftwf_complex *box){
    ...
    Try{
        if(use_wisdom) {
            ...
            sprintf(wisdom_filename,"%s/c2r_DIM%d_DIM%d_NTHREADS%d",config_settings.wisdoms_path, dim, dim_los, n_threads);
...
int dft_r2c_cube(bool use_wisdom, int dim, int dim_los, int n_threads, fftwf_complex *box){
    ...
    Try{
        if(use_wisdom) {
            // Check to see if the wisdom exists
            sprintf(wisdom_filename,"%s/r2c_DIM%d_DIM%d_NTHREADS%d", config_settings.wisdoms_path, dim, dim_los, n_threads);
...
int CreateFFTWWisdoms(UserParams *user_params, CosmoParams *cosmo_params) {
    ...
    Try{ // This Try wraps the entire function so we don't indent.
        ...
        sprintf(wisdom_filename,"%s/r2c_DIM%d_DIM%d_NTHREADS%d",config_settings.wisdoms_path, user_params->DIM,(int)D_PARA,user_params->N_THREADS);
        if(fftwf_import_wisdom_from_filename(wisdom_filename)==0) {
        ...
        sprintf(wisdom_filename,"%s/c2r_DIM%d_DIM%d_NTHREADS%d",config_settings.wisdoms_path, user_params->DIM,(int)D_PARA,user_params->N_THREADS);
        if(fftwf_import_wisdom_from_filename(wisdom_filename)==0) {
        ...
        sprintf(wisdom_filename,"%s/r2c_DIM%d_DIM%d_NTHREADS%d",config_settings.wisdoms_path, user_params->HII_DIM,(int)HII_D_PARA,user_params->N_THREADS);
        if(fftwf_import_wisdom_from_filename(wisdom_filename)==0) {
        ...
        sprintf(wisdom_filename,"%s/c2r_DIM%d_DIM%d_NTHREADS%d",config_settings.wisdoms_path, user_params->HII_DIM,(int)HII_D_PARA,user_params->N_THREADS);
        if(fftwf_import_wisdom_from_filename(wisdom_filename)==0) {
