...
__global__ void ws_compute_reduce_Fcoll(
    ...
    unsigned long long num_pixels, // HII_TOT_NUM_PIXELS
    ...
    __shared__ double sdata[32]; // shared mem for warp's partial sums

    int tid = threadIdx.x; // thread within block
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x; // grid index
    ...
    double Splined_Fcoll; // store computed collapse fraction
    ...
    while (idx < num_pixels) {
        ...
        *((float *) deltax_filtered + fft_idx) = fmaxf(*((float *) deltax_filtered + fft_idx), -1. + fract_float_err);
        ...
        Splined_Fcoll = exp(EvaluateRGTable1D_f_gpu(*((float *) deltax_filtered + fft_idx), x_min, x_width, y_arr));
        Fcoll[idx] = Splined_Fcoll; // add to grid

        val += Splined_Fcoll; // add to thread's partial sum
        idx += gridDim.x * blockDim.x; // increment index for thread
    }
    ...
    if (warpID == 0) {
        ...
        if (tid == 0) {
            atomicAdd(Fcoll_sum, val);
        }
    }
}

void init_ionbox_gpu_data_ws(
...
void calculate_fcoll_grid_gpu_ws(
    ...
    double *h_Fcoll_sum, // temp for f_coll_grid_mean
    ...
    *f_coll_grid_mean = *h_Fcoll_sum / (double) hii_tot_num_pixels;
    LOG_INFO("Fcoll grid mean computed.");
