...
void writeUserParams(UserParams *p) {
    LOG_INFO(
        ...
        "MINIMIZE_MEMORY=%1d,\n"
        "       KEEP_3D_VELOCITIES=%1d, SAMPLER_MIN_MASS=%10.3e, SAMPLER_BUFFER_FACTOR=%8.3f, \n"
        "       N_COND_INTERP=%4d, N_PROB_INTERP=%4d, MIN_LOGPROB=%8.3f, SAMPLE_METHOD=%2d, "
        "AVG_BELOW_SAMPLER=%1d,\n"
        "       HALOMASS_CORRECTION=%8.3f",
        p->HII_DIM, p->DIM, p->BOX_LEN, p->NON_CUBIC_FACTOR, p->HMF, p->POWER_SPECTRUM,
        p->USE_RELATIVE_VELOCITIES, p->N_THREADS, p->PERTURB_ON_HIGH_RES, p->NO_RNG,
        ...
        p->SAMPLER_MIN_MASS, p->SAMPLER_BUFFER_FACTOR, p->N_COND_INTERP, p->N_PROB_INTERP,
        p->MIN_LOGPROB, p->SAMPLE_METHOD, p->AVG_BELOW_SAMPLER, p->HALOMASS_CORRECTION);
}
    ...
    if (fo->USE_MASS_DEPENDENT_ZETA) {
        ...
        if (fo->USE_MINI_HALOS) {
            LOG_INFO(
                ...
                p->ALPHA_STAR_MINI, p->F_ESC7_MINI, p->L_X_MINI, p->F_STAR7_MINI, p->F_H2_SHIELD,
...
void writeFlagOptions(FlagOptions *p) {
    LOG_INFO(
        "\n        FlagOptions:\n"
        "       USE_HALO_FIELD=%1d, USE_MINI_HALOS=%1d, USE_MASS_DEPENDENT_ZETA=%1d, "
        "SUBCELL_RSD=%1d,\n"
        ...
        p->HALO_STOCHASTICITY, p->FIXED_HALO_GRIDS, p->USE_EXP_FILTER, p->USE_CMB_HEATING,
