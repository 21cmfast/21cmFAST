...
void seed_rng_threads(gsl_rng* rng_arr[], unsigned long long int seed) {
    ...
    unsigned int seeds[simulation_options_global->N_THREADS];
    ...
    gsl_ran_choose(rseed, seeds, simulation_options_global->N_THREADS, many_ints, num_int,
                   sizeof(unsigned int));
    // Shuffle the randomly selected integers
    gsl_ran_shuffle(rseed, seeds, simulation_options_global->N_THREADS, sizeof(unsigned int));
...
void seed_rng_threads_fast(gsl_rng* rng_arr[], unsigned long long int seed) {
    ...
    checker = 0;
    // seed the random number generators
    sample_n_unique_integers(simulation_options_global->N_THREADS, seed, st_arr);
