...
double apply_subcell_rsds(UserParams *user_params, CosmoParams *cosmo_params,
                          ...
                          IonizedBox *ionized_box, BrightnessTemp *box,
                          float redshift, TsBox *spin_temp, float T_rad,
                          float *v, float H) {
  int i, ii, j, k;
  double ave;
  ...
  float **delta_T_RSD_LOS =
      (float **)calloc(user_params->N_THREADS, sizeof(float *));
  ...
  // normalised units of cell length. 0 equals beginning of cell, 1 equals end
  // of cell These are the sub-cell central positions (x_pos_offset), and the
  // corresponding normalised value (x_pos) between 0 and 1
...
from s -> r + (1+z)*v/H(z). To convert the velocity within the array v to km/s,
it is a*dD/dt*delta. Where the scale factor a comes from the continuity equation
The array v as defined in 21cmFAST is (ik/k^2)*dD/dt*delta, as it is defined as
a comoving quantity (scale factor is implicit). However, the conversion between
real and redshift space also picks up a scale factor, therefore the scale
factors drop out and therefore the displacement of the sub-cells is purely
determined from the array, v and the Hubble factor: v/H.
*/
#pragma omp parallel shared(                                                  \
        delta_T_RSD_LOS, box, ionized_box, v, x_val1, x_val2, x_pos,          \
            x_pos_offset,                                                     \
            subcell_width) private(i, j, k, ii, d1_low, d2_low, d1_high,      \
                                       d2_high, subcell_displacement,         \
                                       RSD_pos_new, RSD_pos_new_boundary_low, \
                                       RSD_pos_new_boundary_high,             \
                                       cell_distance, fraction_outside,       \
                                       fraction_within)                       \
...
#pragma omp for reduction(+ : ave)
    for (i = 0; i < user_params->HII_DIM; i++) {
      for (j = 0; j < user_params->HII_DIM; j++) {
        ...
        for (k = 0; k < HII_D_PARA; k++) {
          if ((fabs(box->brightness_temp[HII_R_INDEX(i, j, k)]) >=
               FRACT_FLOAT_ERR) &&
            ...
            // Displacements (converted from velocity) for the original cell
            // centres straddling half of the sub-cells (cell after)
            if (k == (HII_D_PARA - 1)) {
            ...
            for (ii = 0; ii < astro_params->N_RSD_STEPS; ii++) {
              // linearly interpolate the displacements to determine the
              // corresponding displacements of the sub-cells Checking of 0.5 is
              // for determining if we are left or right of the mid-point of the
              // original cell (for the linear interpolation of the
              // displacement) to use the appropriate cell

              if (x_pos[ii] <= 0.5) {
                subcell_displacement = d1_low + ((x_pos[ii] + 0.5) - x_val1) *
                                                    (d2_low - d1_low) /
                                                    (x_val2 - x_val1);
              } else {
                subcell_displacement = d1_high + ((x_pos[ii] - 0.5) - x_val1) *
                                                     (d2_high - d1_high) /
                                                     (x_val2 - x_val1);
              }

              // The new centre of the sub-cell post R.S.D displacement.
              // Normalised to units of cell width for determining it's
              // displacement
              RSD_pos_new =
              ...
              // fractional contribution of the sub-cell to neighbouring cells
              // when the sub-cell straddles two cell positions
              ...
              if (RSD_pos_new_boundary_low >= 0.0 &&
                ...
                // sub-cell has remained in the original cell (just add it back
                // to the original cell)

                delta_T_RSD_LOS[omp_get_thread_num()][k] +=
                    box->brightness_temp[HII_R_INDEX(i, j, k)] /
                    ((float)astro_params->N_RSD_STEPS);
                ...
                // Determine the location of the sub-cell relative to the
                // original cell binning
                if (fabs(RSD_pos_new_boundary_high) > cell_distance) {
                  // sub-cell is entirely contained within the new cell (just
                  // add it to the new cell)

                  // check if the new cell position is at the edge of the box.
                  // If so, periodic boundary conditions
                  ...
                  // Determine the fraction of the sub-cell which is in either
                ...
                // Check the periodic boundaries conditions and move the
                // fraction of each sub-cell to the appropriate new cell
                if (k == 0) {
