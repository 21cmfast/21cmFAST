...
double expected_nhalo(double redshift, UserParams *user_params, CosmoParams *cosmo_params) {
    ...
    init_ps();
    if (user_params->USE_INTERPOLATION_TABLES > 0) initialiseSigmaMInterpTable(M_min, M_max);
...
void stoc_set_consts_z(struct HaloSamplingConstants *const_struct, double redshift,
    ...
    const_struct->M_min =
        user_params_global->SAMPLER_MIN_MASS / user_params_global->SAMPLER_BUFFER_FACTOR;
    ...
    if (redshift_desc > 0) {
        ...
        initialise_dNdM_tables(log(user_params_global->SAMPLER_MIN_MASS), const_struct->lnM_max_tb,
        ...
        if (user_params_global->SAMPLE_METHOD == 0 || user_params_global->SAMPLE_METHOD == 1) {
            initialise_dNdM_inverse_table(log(user_params_global->SAMPLER_MIN_MASS),
        ...
        double delta_crit = get_delta_crit(user_params_global->HMF, const_struct->sigma_cond,
...
void stoc_set_consts_cond(struct HaloSamplingConstants *const_struct, double cond_val) {
    ...
    if (const_struct->from_catalog) {
        ...
        const_struct->delta = get_delta_crit(user_params_global->HMF, const_struct->sigma_cond,
    ...
    const_struct->delta_crit =
        get_delta_crit(user_params_global->HMF, const_struct->sigma_cond, const_struct->growth_out);
...
int stoc_partition_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng, int *n_halo_out,
    ...
    while (M_remaining > user_params_global->SAMPLER_MIN_MASS) {
        ...
        delta_current = (get_delta_crit(user_params_global->HMF, sigma_r, growthf) - d_cond) /
                        (M_remaining / M_cond);
...
int stoc_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng, int *n_halo_out,
    ...
    if (hs_constants->delta >= MAX_DELTAC_FRAC * get_delta_crit(user_params_global->HMF,
...
int single_test_sample(UserParams *user_params, CosmoParams *cosmo_params,
                       AstroParams *astro_params, FlagOptions *flag_options,
                       unsigned long long int seed, int n_condition, float *conditions,
...
#pragma omp critical
        ...
        if (hs_constants->from_catalog) {
            initialise_dNdM_tables(log(user_params->SAMPLER_MIN_MASS), hs_constants->lnM_max_tb,
                                   log(user_params->SAMPLER_MIN_MASS), hs_constants->lnM_max_tb,
                                   hs_constants->growth_out, hs_constants->growth_in, true);
        } else {
            double delta_crit = get_delta_crit(user_params_global->HMF, hs_constants->sigma_cond,
                                               hs_constants->growth_out);
