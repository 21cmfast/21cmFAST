...
void print_hs_consts(struct HaloSamplingConstants *c) {
  ...
  LOG_DEBUG("from_catalog %d z_in %.2f z_out %.2f d_in %.2f d_out %.2f",
            c->from_catalog, c->z_in, c->z_out, c->growth_in, c->growth_out);
  LOG_DEBUG("M_min %.2e (%.2e) (%.2f) M_max %.2e (%.2e)", c->M_min, c->lnM_min,
            c->sigma_min, c->M_max_tables, c->lnM_max_tb);
  LOG_DEBUG("Corr Star %.2e SFR %.2e XRAY %.2e", c->corr_star, c->corr_sfr,
            c->corr_xray);
  LOG_DEBUG("CONDITION DEPENDENT STUFF (may not be set)");
  LOG_DEBUG("delta %.2e M_c %.2e (%.2e) (%.2e) cond %.2e", c->delta, c->M_cond,
            c->lnM_cond, c->sigma_cond, c->cond_val);
  LOG_DEBUG("exp N %.2f exp M %.2e", c->expected_N, c->expected_M);
  return;
}

// This function, designed to be used in the wrapper to estimate Halo catalogue
// size, takes the parameters and returns average number of halos within the
// entire box
double expected_nhalo(double redshift, UserParams *user_params,
  ...
  result = Nhalo_General(redshift, log(M_min), log(M_max)) * VOLUME *
           cosmo_params->OMm * RHOcrit;
  LOG_DEBUG("Expected %.2e Halos in the box from masses %.2e to %.2e at z=%.2f",
            result, M_min, M_max, redshift);

  if (user_params->USE_INTERPOLATION_TABLES > 0) freeSigmaMInterpTable();

  return result;
}

double sample_dndM_inverse(double condition,
...
void stoc_set_consts_z(struct HaloSamplingConstants *const_struct,
                       double redshift, double redshift_desc) {
  if (redshift_desc > 0 && redshift < redshift_desc) {
    LOG_ERROR(
        "you have passed a descendant redshift above the progenitor redshift");
    Throw(ValueError);
  }
  ...
  const_struct->M_min = user_params_global->SAMPLER_MIN_MASS /
                        user_params_global->SAMPLER_BUFFER_FACTOR;
  ...
  if (user_params_global->USE_INTERPOLATION_TABLES > 0) {
    if (user_params_global->SAMPLE_METHOD == 3)
      initialiseSigmaMInterpTable(
          ...
          const_struct->M_max_tables);  // the binary split needs to go below
                                        // the resolution
    else
      initialiseSigmaMInterpTable(const_struct->M_min,
                                  const_struct->M_max_tables);

    if (user_params_global->SAMPLE_METHOD == 2) InitialiseSigmaInverseTable();
  }
  ...
  if (redshift_desc > 0) {
    ...
    if (astro_params_global->CORR_SFR > 0)
      const_struct->corr_sfr =
          exp(-(redshift - redshift_desc) / astro_params_global->CORR_SFR);
    else
      const_struct->corr_sfr = 0;
    if (astro_params_global->CORR_STAR > 0)
      const_struct->corr_star =
          exp(-(redshift - redshift_desc) / astro_params_global->CORR_STAR);
    else
      const_struct->corr_star = 0;
    if (astro_params_global->CORR_LX > 0)
      const_struct->corr_xray =
          exp(-(redshift - redshift_desc) / astro_params_global->CORR_LX);
    else
      const_struct->corr_xray = 0;

    const_struct->from_catalog = 1;
    initialise_dNdM_tables(log(user_params_global->SAMPLER_MIN_MASS),
                           const_struct->lnM_max_tb, const_struct->lnM_min,
                           const_struct->lnM_max_tb, const_struct->growth_out,
                           const_struct->growth_in, true);
    if (user_params_global->SAMPLE_METHOD == 0 ||
        user_params_global->SAMPLE_METHOD == 1) {
      initialise_dNdM_inverse_table(
          log(user_params_global->SAMPLER_MIN_MASS), const_struct->lnM_max_tb,
    ...
    double M_cond =
        RHOcrit * cosmo_params_global->OMm * VOLUME / HII_TOT_NUM_PIXELS;
    ...
    double delta_crit =
        get_delta_crit(user_params_global->HMF, const_struct->sigma_cond,
                       const_struct->growth_out);
    const_struct->from_catalog = 0;
    // TODO: determine the minimum density in the field and pass it in (<-1 is
    // fine for Lagrangian)
    initialise_dNdM_tables(DELTA_MIN, MAX_DELTAC_FRAC * delta_crit,
                           const_struct->lnM_min, const_struct->lnM_max_tb,
                           const_struct->growth_out, const_struct->lnM_cond,
                           false);
    initialise_dNdM_inverse_table(
        DELTA_MIN, MAX_DELTAC_FRAC * delta_crit, const_struct->lnM_min,
...
void stoc_set_consts_cond(struct HaloSamplingConstants *const_struct,
                          double cond_val) {
  double m_exp, n_exp;

  // Here the condition is a mass, volume is the Lagrangian volume and delta_l
  // is set by the redshift difference which represents the difference in
  // delta_crit across redshifts
  ...
  // Here the condition is a cell of a given density, the volume/mass is given
  // by the grid parameters
  else {
  ...
  // the splines don't work well for cells above Deltac, but there CAN be cells
  // above deltac, since this calculation happens before the overlap, and since
  // the smallest dexm mass is M_cell*(1.01^3) there *could* be a cell above
  // Deltac not in a halo NOTE: all this does is prevent integration errors
  // below since these cases are also dealt with in stoc_sample
  if (const_struct->delta >
      MAX_DELTAC_FRAC * get_delta_crit(user_params_global->HMF,
                                       const_struct->sigma_cond,
                                       const_struct->growth_out)) {
    const_struct->expected_M = const_struct->M_cond;
    const_struct->expected_N = 1;
  } else if (const_struct->delta <= DELTA_MIN ||
             const_struct->M_cond < const_struct->M_min) {
...
void place_on_hires_grid(int x, int y, int z, int *crd_hi, gsl_rng *rng) {
  // we want to randomly place each halo within each lores cell,then map onto
  // hires this is so halos are on DIM grids to match HaloField and Perturb
  // options
  int x_hi, y_hi, z_hi;
  double randbuf;
...
void set_prop_rng(gsl_rng *rng, bool from_catalog, double *interp,
                  double *input, double *output) {
  double rng_star, rng_sfr, rng_xray;

  // Correlate properties by interpolating between the sampled and descendant
  // gaussians
  rng_star = astro_params_global->SIGMA_STAR > 0. ? gsl_ran_ugaussian(rng) : 0.;
  rng_sfr =
      astro_params_global->SIGMA_SFR_LIM > 0. ? gsl_ran_ugaussian(rng) : 0.;
  ...
  if (from_catalog) {
    ...
    rng_star =
        sqrt(1 - interp[0] * interp[0]) * rng_star + interp[0] * input[0];
    rng_sfr = sqrt(1 - interp[1] * interp[1]) * rng_sfr + interp[1] * input[1];
    rng_xray =
        sqrt(1 - interp[2] * interp[2]) * rng_xray + interp[2] * input[2];
  }
...
// This is the function called to assign halo properties to an entire catalogue,
// used for DexM halos
int add_properties_cat(unsigned long long int seed, float redshift,
                       HaloField *halos) {
  // set up the rng
...
/* Creates a realisation of halo properties by sampling the halo mass function
 * and conditional property PDFs, the number of halos is poisson sampled from
 * the integrated CMF*/
int stoc_halo_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng,
                     int *n_halo_out, float *M_out) {
...
// This performs the number-limited sampling as above, modified to include a
// mass tolerance
//  halo samples are entirely thrown out and re-drawn if they do not meet the
//  tolerance This version is only currently used for internal testing against
//  Nikolic et al. 2024,
int stoc_halo_sample_tol(struct HaloSamplingConstants *hs_constants,
                         gsl_rng *rng, int *n_halo_out, float *M_out) {
  double exp_N = hs_constants->expected_N;
...
double remove_random_halo(gsl_rng *rng, int n_halo, int *idx, double *M_prog,
                          float *M_out) {
  double last_M_del;
  int random_idx;
  do {
...
// CURRENT IMPLEMENTATION: half the time I keep/throw away the last halo based
// on which sample is closer to the expected mass.
//  However this introduces a bias since the last halo is likely larger than
//  average So the other half the time, I throw away random halos until we are
//  again below exp_M, effectively the same process in reverse. which has the
//  opposite bias
void fix_mass_sample(gsl_rng *rng, double exp_M, int *n_halo_pt,
                     double *M_tot_pt, float *M_out) {
  ...
  // int sel = 1;
  if (sel) {
    if (fabs(*M_tot_pt - M_out[*n_halo_pt - 1] - exp_M) <
        fabs(*M_tot_pt - exp_M)) {
    ...
    do {
      // here we remove by setting halo mass to zero, skipping it during the
      // consolidation
      last_M_del =
          remove_random_halo(rng, *n_halo_pt, &random_idx, M_tot_pt, M_out);
    } while (*M_tot_pt > exp_M);

    // if the sample with the last subtracted halo is closer to the expected
    // mass, keep it LOG_ULTRA_DEBUG("Deciding to keep last halo M %.3e tot %.3e
    // exp %.3e",last_M_del,*M_tot_pt,exp_M);
...
/* Creates a realisation of halo properties by sampling the halo mass function
 * and conditional property PDFs, Sampling is done until there is no more mass
 * in the condition Stochasticity is ignored below a certain mass threshold */
int stoc_mass_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng,
                     int *n_halo_out, float *M_out) {
  double exp_M = hs_constants->expected_M;

  // The mass-limited sampling as-is has a slight bias to producing too many
  // halos,
  //   which is independent of density or halo mass,
  //   this factor reduces the total expected mass to bring it into line with
  //   the CMF
  ...
  // The above sample is above the expected mass, by up to 100%. I wish to make
  // the average mass equal to exp_M
...
bool partition_rejection(double sigma_m, double sigma_min, double sigma_cond,
                         double del_c, double growthf, gsl_rng *rng) {
  // no rejection in EPS
...
//   Do we not sample the same `particle` multiple times? (i.e 10x more samples
//   for a single 10x mass halo) How does the reduction of mass after each
//   sample *exactly* cancel this 1/M effect
// If you want a non-barrier-based CMF, I don't know how to implement it here
int stoc_partition_sample(struct HaloSamplingConstants *hs_constants,
                          gsl_rng *rng, int *n_halo_out, float *M_out) {
  ...
  while (M_remaining > user_params_global->SAMPLER_MIN_MASS) {
    ...
    delta_current =
        (get_delta_crit(user_params_global->HMF, sigma_r, growthf) - d_cond) /
        (M_remaining / M_cond);
    del_term = delta_current * delta_current / growthf / growthf;

    nu_min = sqrt(del_term / (sigma_min * sigma_min -
                              sigma_r * sigma_r));  // nu at minimum progenitor

    // we use the gaussian tail distribution to enforce our Mmin limit from the
    // sigma tables
    do {
      nu_sample = gsl_ran_ugaussian_tail(rng, nu_min) * nu_fudge_factor;
      sigma_sample =
          sqrt(del_term / (nu_sample * nu_sample) + sigma_r * sigma_r);
    } while (partition_rejection(sigma_sample, sigma_min, sigma_r,
                                 delta_current / growthf, growthf, rng));
...
double ComputeFraction_split(double sigma_start, double sigmasq_start,
                             double sigmasq_res, double G1, double dd,
                             double gamma1) {
  double u_res = sigma_start * pow(sigmasq_res - sigmasq_start, -.5);
  return sqrt(2. / PI) * EvaluateJ(u_res, gamma1) * G1 / sigma_start * dd;
}

// binary splitting with small internal steps based on Parkinson+08, Bensen+16,
// Qiu+20 (Darkforest) This code was modified from the tree generation function
// in Darkforest (Qiu et al 2020. ArXiv: 2007.14624)
int stoc_split_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng,
                      int *n_halo_out, float *M_out) {
  // define constants
  ...
  // counters for total mass, number at target z, active index, and total number
  // in sub-tree
  double M_total = 0;
  int n_out = 0;
  int idx = 0;
  ...
  while (idx < n_points) {
    ...
    if (q_res >= 0.5) {
      ...
      //   so we only consider the timestep restriction to make the EPS limit
      //   valid and take away the average mass below the resolution
      ...
      alpha_half = -m_half / (2 * sigma_half * sigma_half) *
                   alpha_half;  //-d(lnsigma)/d(lnm)
      // Compute B and beta
      ...
      // Compute ddelta1, the timestep limit ensuring the exponent in the EPS MF
      // is small (limit as time -> 0 is valid)
      dd1 = eps1 * sqrt(2) * sqrt(sigmasq_half - sigmasq_start);

      // Compute ddelta2, the timestep limit ensuring the assumption of maximum
      // 1 split is valid
      mu = gamma1 < 0. ? -log(sigma_res / sigma_half) / log(2. * q_res)
                       : alpha_half;
      eta = beta - 1 - gamma1 * mu;
      pow_diff = pow(.5, eta) - pow(q_res, eta);
      G2 = G1 * pow(sigma_half / sigma_start, gamma1) * pow(0.5, mu * gamma1);
      dN_dd = sqrt(2. / PI) * B * pow_diff / eta * alpha_half *
              G2;  // this is the number of progenitors expected per unit
                   // increase in the barrier
      dd2 = eps2 / dN_dd;  // barrier change which results in average of at most
                           // eps2 progenitors
      ...
      if (gsl_rng_uniform(rng) < N_upper) {
        ...
        alpha_q =
            -m_q / (2 * sigma_q * sigma_q) * alpha_q;  //-d(lnsigma)/d(lnm)
        sigmasq_q = sigma_q * sigma_q;

        factor1 = alpha_q / alpha_half;
        factor2 = sigmasq_q * pow(sigmasq_q - sigmasq_start, -1.5) /
                  (B * pow(q, beta));
        R_q = factor1 * factor2;
        if (gsl_rng_uniform(rng) > R_q) q = 0.;  // No split
      }
    }
    // LOG_ULTRA_DEBUG("split i %d n %d m %.2e d
    // %.2e",idx,n_points,m_start,d_start); LOG_ULTRA_DEBUG("q %.2e F %.2e dd
    // %.2e (%.2e %.2e) of %.2e",q,F,dd,dd1,dd2,dd_target);
    // LOG_ULTRA_DEBUG("dNdd %.2e B %.2e pow %.2e eta %.2e ah %.2e G2 %.2e b
    // %.2e",dN_dd,B,pow_diff,eta,alpha_half,G2,beta);

    // Compute progenitor mass from fraction q, always subtract the
    // below-resolution mass from the largest
    ...
    // We don't need the point at idx anymore, so we can put the first
    // progenitor at the start point, and the second at the end since the first
    // is always more massive, this saves memory NOTE: this still drifts by the
    // number of saved halos, figure out how to
    //    keep the active halo at zero until the end, but that's minor as it
    //    should only drift a few dozen
    else {
      if (m_prog1 > m_res) {
  ...
  *n_halo_out = n_out;
  // LOG_ULTRA_DEBUG("Total M = %.4e (%.4e) N =
  // %d",M_total,M_total/hs_constants->M_cond,n_out);
  return 0;
}

int stoc_sample(struct HaloSamplingConstants *hs_constants, gsl_rng *rng,
                int *n_halo_out, float *M_out) {
  // TODO: really examine the case for number/mass sampling
  // The poisson sample fails spectacularly for high delta (from_catalogs or
  // dense cells)
  //   and excludes the correlation between number and mass (e.g many small
  //   halos or few large ones)
  // The mass sample underperforms at low exp_M/M_max by excluding stochasticity
  // in the total collapsed fraction
  //   and excluding larger halos (e.g if exp_M is 0.1*M_max we can effectively
  //   never sample the large halos)
  // i.e there is some case for a delta cut between these two methods however I
  // have no intuition for the exact levels

  int err;
  // If the expected mass is below our minimum saved mass, don't bother
  // calculating NOTE: some of these conditions are redundant with
  // set_consts_cond()
...
void condense_sparse_halolist(HaloField *halofield,
                              unsigned long long int *istart_threads,
                              unsigned long long int *nhalo_threads) {
  int i = 0;
  unsigned long long int count_total = 0;
  for (i = 0; i < user_params_global->N_THREADS; i++) {
    memmove(&halofield->halo_masses[count_total],
            &halofield->halo_masses[istart_threads[i]],
            sizeof(float) * nhalo_threads[i]);
    memmove(&halofield->star_rng[count_total],
            &halofield->star_rng[istart_threads[i]],
            sizeof(float) * nhalo_threads[i]);
    memmove(&halofield->sfr_rng[count_total],
            &halofield->sfr_rng[istart_threads[i]],
            sizeof(float) * nhalo_threads[i]);
    memmove(&halofield->xray_rng[count_total],
            &halofield->xray_rng[istart_threads[i]],
            sizeof(float) * nhalo_threads[i]);
    memmove(&halofield->halo_coords[3 * count_total],
            &halofield->halo_coords[3 * istart_threads[i]],
            sizeof(int) * 3 * nhalo_threads[i]);
  ...
  memset(&halofield->xray_rng[count_total], 0,
         (halofield->buffer_size - count_total) * sizeof(float));
  LOG_SUPER_DEBUG("Set %llu elements beyond %llu to zero",
                  halofield->buffer_size - count_total, count_total);
}
...
int sample_halo_grids(gsl_rng **rng_arr, double redshift, float *dens_field,
                      float *halo_overlap_box, HaloField *halofield_large,
                      HaloField *halofield_out,
                      struct HaloSamplingConstants *hs_constants) {
  int lo_dim = user_params_global->HII_DIM;
  ...
  unsigned long long int arraysize_local =
      arraysize_total / user_params_global->N_THREADS;

  LOG_DEBUG("Beginning stochastic halo sampling on %d ^3 grid", lo_dim);
  LOG_DEBUG("z = %f, Mmin = %e, Mmax = %e,volume = %.3e, D = %.3e", redshift,
            Mmin, Mcell, Mcell / RHOcrit / cosmo_params_global->OMm, growthf);
  LOG_DEBUG(
      "Total Array Size %llu, array size per thread %llu (~%.3e GB total)",
      arraysize_total, arraysize_local,
      6. * arraysize_total * sizeof(int) / 1e9);
...
#pragma omp for reduction(+ : total_volume_dexm)
    for (halo_idx = 0; halo_idx < nhalo_in; halo_idx++) {
      halofield_out->halo_masses[istart + count] =
          halofield_large->halo_masses[halo_idx];
      halofield_out->star_rng[istart + count] =
          halofield_large->star_rng[halo_idx];
      halofield_out->sfr_rng[istart + count] =
          halofield_large->sfr_rng[halo_idx];
      halofield_out->xray_rng[istart + count] =
          halofield_large->xray_rng[halo_idx];
...
#pragma omp for reduction(+ : total_volume_excluded)
        ...
        for (z = 0; z < HII_D_PARA; z++) {
          ...
          for (i = 0; i < nh_buf; i++) {
            ...
            if (count >= arraysize_local) {
              LOG_ERROR(
                  "More than %llu halos (expected %.1e) with buffer size "
                  "factor %.1f",
                  arraysize_local,
                  arraysize_local / config_settings.HALO_CATALOG_MEM_FACTOR,
                  config_settings.HALO_CATALOG_MEM_FACTOR);
              LOG_ERROR(
                  "If you expected to have an above average halo number try "
                  "raising config_settings.HALO_CATALOG_MEM_FACTOR");
              Throw(ValueError);
            }
            ...
            if ((x + y + z) == 0) {
              LOG_ULTRA_DEBUG(
                  "Halo %d Mass %.2e Stellar %.2e SFR %.2e XRAY %.2e", i,
                  hm_buf[i], prop_buf[0], prop_buf[1], prop_buf[2]);
            }
          }
          if ((x + y + z) == 0) {
            LOG_SUPER_DEBUG(
                "Cell (%d %d %d): delta %.2f | N %d (exp. %.2e) | Total M %.2e "
                "(exp. %.2e) overlap defc %.2f",
                x, y, z, delta, nh_buf, hs_constants_priv.expected_N,
                M_tot_cell, hs_constants_priv.expected_M, mass_defc);
          }
        }
      }
    }
  ...
  LOG_SUPER_DEBUG(
      "Total dexm volume %.6e Total volume excluded %.6e (In units of HII_DIM "
      "cells)",
...
// NOTE: there's a lot of repeated code here and in build_halo_cats, find a way
// to merge
int sample_halo_progenitors(gsl_rng **rng_arr, double z_in, double z_out,
                            HaloField *halofield_in, HaloField *halofield_out,
                            struct HaloSamplingConstants *hs_constants) {
  if (z_in >= z_out) {
    LOG_ERROR(
        "halo progenitors must go backwards in time!!! z_in = %.1f, z_out = "
        "%.1f",
        z_in, z_out);
    Throw(ValueError);
  }
  ...
  LOG_DEBUG("Beginning stochastic halo sampling of progenitors on %llu halos",
            nhalo_in);
  ...
  double corr_arr[3] = {hs_constants->corr_star, hs_constants->corr_sfr,
                        hs_constants->corr_xray};
...
#pragma omp parallel num_threads(user_params_global->N_THREADS)
  {
    ...
    // we need a private version
    // also the naming convention should be better between structs/struct
    // pointers
    struct HaloSamplingConstants hs_constants_priv;
    hs_constants_priv = *hs_constants;
...
#pragma omp for
    for (ii = 0; ii < nhalo_in; ii++) {
      ...
      if (M2 < Mmin || M2 > Mmax_tb) {
        LOG_ERROR(
            "Input Mass = %.2e at %llu of %llu, something went wrong in the "
            "input catalogue",
            M2, ii, nhalo_in);
        Throw(ValueError);
      }
      ...
      for (jj = 0; jj < n_prog; jj++) {
        ...
        if (count >= arraysize_local) {
          LOG_ERROR(
              "More than %llu halos (expected %.1e) with buffer size factor "
              "%.1f",
              arraysize_local,
              arraysize_local / config_settings.HALO_CATALOG_MEM_FACTOR,
              config_settings.HALO_CATALOG_MEM_FACTOR);
          LOG_ERROR(
              "If you expected to have an above average halo number try "
              "raising config['HALO_CATALOG_MEM_FACTOR']");
          Throw(ValueError);
        }

        set_prop_rng(rng_arr[threadnum], true, corr_arr, propbuf_in,
                     propbuf_out);
        ...
        if (ii == 0) {
          ...
          LOG_ULTRA_DEBUG(
              "First Halo Prog %d: Mass %.2e Stellar %.2e SFR %.2e XRAY %.2e "
              "e_d %.3f",
      ...
      if (ii == 0) {
        LOG_ULTRA_DEBUG(
            ...
            get_delta_crit(user_params_global->HMF,
                           hs_constants_priv.sigma_cond,
                           hs_constants->growth_out),
            get_delta_crit(user_params_global->HMF,
                           hs_constants_priv.sigma_cond,
        ...
        print_hs_consts(&hs_constants_priv);
        LOG_SUPER_DEBUG(
            "First Halo: Mass %.2f | N %d (exp. %.2e) | Total M %.2e (exp. "
            "%.2e)",
            M2, n_prog, hs_constants_priv.expected_N, M_prog,
            hs_constants_priv.expected_M);
      }
    }
    istart_threads[threadnum] = istart;
    nhalo_threads[threadnum] = count;
...
// function that talks between the structures (Python objects) and the sampling
// functions
int stochastic_halofield(UserParams *user_params, CosmoParams *cosmo_params,
                         ...
                         unsigned long long int seed, float redshift_desc,
                         float redshift, float *dens_field,
                         float *halo_overlap_box, HaloField *halos_desc,
                         HaloField *halos) {
  Broadcast_struct_global_all(user_params, cosmo_params, astro_params,
                              flag_options);
  if (redshift_desc > 0 && halos_desc->n_halos == 0) {
    LOG_DEBUG("No halos to sample from redshifts %.2f to %.2f, continuing...",
              redshift_desc, redshift);
    return 0;
  }
  ...
  if (redshift_desc <= 0.) {
    ...
    sample_halo_grids(rng_stoc, redshift, dens_field, halo_overlap_box,
                      halos_desc, halos, &hs_constants);
  } else {
    LOG_DEBUG("Calculating halo progenitors from z=%.1f to z=%.1f | %llu",
              redshift_desc, redshift, halos_desc->n_halos);
    sample_halo_progenitors(rng_stoc, redshift_desc, redshift, halos_desc,
                            halos, &hs_constants);
  }
  ...
  if (halos->n_halos >= 3) {
    ...
    LOG_DEBUG("First few XRAY RNG:     %11.3e %11.3e %11.3e",
              halos->xray_rng[0], halos->xray_rng[1], halos->xray_rng[2]);
  }
...
// This is a test function which takes a list of conditions (cells or halos) and
// samples them to produce a descendant list
//       as well as per-condition number and mass counts
int single_test_sample(UserParams *user_params, CosmoParams *cosmo_params,
                       AstroParams *astro_params, FlagOptions *flag_options,
                       unsigned long long int seed, int n_condition,
                       float *conditions, int *cond_crd, double z_out,
                       double z_in, int *out_n_tot, int *out_n_cell,
  ...
  Try {
    ...
    if (z_in > 0 && z_out <= z_in) {
      LOG_DEBUG("progenitor sampling must go back in time z_out=%.2f z_in=%.2f",
                z_out, z_in);
      Throw(ValueError);
    }
    int i, j;
    ...
    LOG_DEBUG(
        "SINGLE SAMPLE: z = (%.2f,%.2f), Mmin = %.3e, "
        "cond(%d)=[%.2e,%.2e,%.2e...]",
        z_out, z_in, hs_constants->M_min, n_condition, conditions[0],
        conditions[1], conditions[2]);

    // halo catalogues + cell sums from multiple conditions, given M as cell
    // descendant halos/cells the result mapping is n_halo_total (1)
    // (exp_n,exp_m,n_prog,m_prog) (n_desc) M_cat (n_prog_total)
    int n_halo_tot = 0;
...
#pragma omp for
      for (j = 0; j < n_condition; j++) {
        ...
        stoc_sample(&hs_constants_priv, rng_stoc[omp_get_thread_num()], &n_halo,
                    out_hm);
...
#pragma omp critical
    ...
    if (hs_constants->from_catalog) {
      initialise_dNdM_tables(
          log(user_params->SAMPLER_MIN_MASS), hs_constants->lnM_max_tb,
          log(user_params->SAMPLER_MIN_MASS), hs_constants->lnM_max_tb,
          hs_constants->growth_out, hs_constants->growth_in, true);
    } else {
      double delta_crit =
          get_delta_crit(user_params_global->HMF, hs_constants->sigma_cond,
                         hs_constants->growth_out);
      initialise_dNdM_tables(DELTA_MIN, MAX_DELTAC_FRAC * delta_crit,
                             log(user_params->SAMPLER_MIN_MASS),
