        ...
        while (fail_condition != 0) {
            ...
            while (a < a_end) {
                ...
                if (!RecombPhotonCons) {
                    ...
                    dadt = Ho * sqrt(cosmo_params->OMm / a + cosmo_params_global->OMr / a / a +
                                     cosmo_params->OMl * a *
...
void adjust_redshifts_for_photoncons(UserParams *user_params, AstroParams *astro_params,
    ...
    if (*redshift < astro_params_global->PHOTONCONS_CALIBRATION_END) {
        LOG_ERROR(
            ...
            "(astro_params->PHOTONCONS_CALIBRATION_END = %f). If this behaviour is desired then "
...
void z_at_Q(double Q, double *splined_value) {
    ...
    if (Q < Qmin) {
        ...
        LOG_ERROR(
            "This error can occur if astro_params->PHOTONCONS_CALIBRATION_END is close to "
