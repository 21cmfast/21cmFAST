// New file to store functions that deal with the interpolation tables, since
// they are used very often.
//   We use regular grid tables since they are faster to evaluate (we always
//   know which bin we are in) So I'm making a general function for the 1D and
//   2D cases
...
double EvaluateRGTable1D(double x, RGTable1D *table) {
  ...
  double result = table->y_arr[idx] * (1 - interp_point) +
...
double EvaluateRGTable1D_f(double x, RGTable1D_f *table) {
  ...
  return table->y_arr[idx] * (1 - interp_point) +
         table->y_arr[idx + 1] * (interp_point);
}
