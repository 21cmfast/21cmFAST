...
bool cond_table_out_of_bounds(UserParams *user_params, struct HaloSamplingConstants *consts) {
    return consts->M_cond < user_params->SAMPLER_MIN_MASS ||
           ...
           consts->delta > MAX_DELTAC_FRAC * get_delta_crit(user_params->HMF, consts->sigma_cond,
                                                            consts->growth_out);
}
...
void get_global_SFRD_z(UserParams *user_params, CosmoParams *cosmo_params,
    ...
    if (user_params->USE_INTERPOLATION_TABLES > 0) initialiseSigmaMInterpTable(M_min, 1e20);
