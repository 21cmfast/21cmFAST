/* This file will contain functions which contain methods to directly calculate
 * integrals from the frontend */
#include <math.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
bool cond_table_out_of_bounds(UserParams *user_params,
                              struct HaloSamplingConstants *consts) {
  return consts->M_cond < user_params->SAMPLER_MIN_MASS ||
         consts->M_cond > consts->M_max_tables || consts->delta < -1 ||
         consts->delta > MAX_DELTAC_FRAC * get_delta_crit(user_params->HMF,
                                                          consts->sigma_cond,
                                                          consts->growth_out);
}
...
void get_condition_integrals(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  // unneccessarily creates the inverse table (a few seconds) but much cleaner
  // this way
...
// A more flexible form of the function above, but with many mass ranges for
// outputting tables of CHMF integrals
  ...
  for (i = 0; i < n_conditions; i++) {
    ...
    for (j = 0; j < n_masslim; j++) {
      buf = Nhalo_Conditional(hs_const_struct.growth_out, lnM_lo[j], lnM_hi[j],
                              ...
                              hs_const_struct.sigma_cond, hs_const_struct.delta,
                              0  // QAG
                              ) *
...
void get_halomass_at_probability(UserParams *user_params,
  ...
  for (i = 0; i < n_conditions; i++) {
    ...
    out_of_bounds =
        out_of_bounds || probabilities[i] < 0 || probabilities[i] > 1;
    if (out_of_bounds)
      out_mass[i] = -1;  // mark invalid
    else
      out_mass[i] =
          EvaluateNhaloInv(hs_const_struct.cond_val, probabilities[i]) *
          hs_const_struct.M_cond;
  }
}
...
void get_global_SFRD_z(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  double M_min =
      minimum_source_mass(redshifts[0], true, astro_params, flag_options);
  if (user_params->USE_INTERPOLATION_TABLES > 0)
  ...
  for (i = 0; i < n_redshift; i++) {
    ...
    if (flag_options->USE_MINI_HALOS)
      out_sfrd_mini[i] =
          EvaluateSFRD_MINI(redshifts[i], log10_turnovers_mcg[i], &sc);
  }
}
...
void get_conditional_FgtrM(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  double M_min =
      minimum_source_mass(redshift, true, astro_params, flag_options);
  if (user_params->USE_INTERPOLATION_TABLES > 0)
    initialiseSigmaMInterpTable(M_min, 1e20);
  ...
  if (user_params->USE_INTERPOLATION_TABLES > 1) {
    initialise_FgtrM_delta_table(min_dens, max_dens + 0.01, redshift, growthf,
  ...
  for (i = 0; i < n_densities; i++) {
    out_fcoll[i] =
        EvaluateFcoll_delta(densities[i], growthf, sigma_min, sigma_cond);
    out_dfcoll[i] =
        EvaluatedFcolldz(densities[i], redshift, sigma_min, sigma_cond);
  }
}
...
void get_conditional_SFRD(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  if (user_params->USE_INTERPOLATION_TABLES > 1) {
    initialise_SFRD_Conditional_table(redshift, min_dens, max_dens, M_min,
                                      M_cond, M_cond, &sc);
  }
  for (i = 0; i < n_densities; i++)
    out_sfrd[i] = EvaluateSFRD_Conditional(densities[i], growthf, M_min, M_cond,
                                           M_cond, sigma_cond, &sc);
  if (flag_options->USE_MINI_HALOS) {
    for (i = 0; i < n_densities; i++)
      out_sfrd_mini[i] =
          EvaluateSFRD_Conditional_MINI(densities[i], log10_mturns[i], growthf,
...
void get_conditional_Nion(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  if (user_params->USE_INTERPOLATION_TABLES > 1) {
    initialise_Nion_Conditional_spline(
        redshift, min_dens, max_dens, M_min, M_cond, M_cond, min_l10mturn_acg,
        max_l10mturn_acg, min_l10mturn_mcg, max_l10mturn_mcg, &sc, false);
  }
  for (i = 0; i < n_densities; i++)
    out_nion[i] =
        EvaluateNion_Conditional(densities[i], log10_mturns_acg[i], growthf,
                                 M_min, M_cond, M_cond, sigma_cond, &sc, false);
  if (flag_options->USE_MINI_HALOS) {
    for (i = 0; i < n_densities; i++)
      out_nion_mini[i] = EvaluateNion_Conditional_MINI(
          densities[i], log10_mturns_mcg[i], growthf, M_min, M_cond, M_cond,
          sigma_cond, &sc, false);
  }
}
...
void get_conditional_Xray(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  if (user_params->USE_INTERPOLATION_TABLES > 1) {
    initialise_Xray_Conditional_table(redshift, min_dens, max_dens, M_min,
                                      M_cond, M_cond, &sc);
  }
  for (i = 0; i < n_densities; i++)
    out_xray[i] = EvaluateXray_Conditional(densities[i], log10_mturns[i],
