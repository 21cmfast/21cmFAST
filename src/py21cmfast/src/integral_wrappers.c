...
void get_sigma(MatterParams *matter_params, MatterFlags *matter_flags, CosmoParams *cosmo_params,
               int n_masses, double *mass_values, double *sigma_out, double *dsigmasqdm_out) {
...
bool cond_table_out_of_bounds(MatterParams *matter_params, MatterFlags *matter_flags,
                              struct HaloSamplingConstants *consts) {
    return consts->M_cond < matter_params->SAMPLER_MIN_MASS ||
           consts->M_cond > consts->M_max_tables || consts->delta < -1 ||
           consts->delta > MAX_DELTAC_FRAC * get_delta_crit(matter_flags->HMF, consts->sigma_cond,
                                                            consts->growth_out);
}

// integrates at fixed (set by parameters) mass range for many conditions
void get_condition_integrals(MatterParams *matter_params, MatterFlags *matter_flags,
                             CosmoParams *cosmo_params, AstroParams *astro_params,
                             AstroFlags *astro_flags, double redshift, double z_prev,
                             int n_conditions, double *cond_values, double *out_n_exp,
                             double *out_m_exp) {
    ...
    for (i = 0; i < n_conditions; i++) {
        ...
        if (cond_table_out_of_bounds(matter_params, matter_flags, &hs_const_struct)) {
            out_n_exp[i] = -1.;
            continue;
        }
...
void get_halo_chmf_interval(MatterParams *matter_params, MatterFlags *matter_flags,
                            CosmoParams *cosmo_params, AstroParams *astro_params,
                            AstroFlags *astro_flags, double redshift, double z_prev,
                            int n_conditions, double *cond_values, int n_masslim, double *lnM_lo,
...
void get_halomass_at_probability(MatterParams *matter_params, MatterFlags *matter_flags,
                                 CosmoParams *cosmo_params, AstroParams *astro_params,
                                 AstroFlags *astro_flags, double redshift, double z_prev,
                                 int n_conditions, double *cond_values, double *probabilities,
                                 double *out_mass) {
    Broadcast_struct_global_all(matter_params, matter_flags, cosmo_params, astro_params,
                                astro_flags);
    ...
    for (i = 0; i < n_conditions; i++) {
        stoc_set_consts_cond(&hs_const_struct, cond_values[i]);
        out_of_bounds = cond_table_out_of_bounds(matter_params, matter_flags, &hs_const_struct);
...
void get_global_SFRD_z(MatterParams *matter_params, MatterFlags *matter_flags,
                       CosmoParams *cosmo_params, AstroParams *astro_params,
                       AstroFlags *astro_flags, int n_redshift, double *redshifts,
    ...
    double M_min = minimum_source_mass(redshifts[0], true, astro_params, astro_flags);
    if (matter_flags->USE_INTERPOLATION_TABLES > 0) initialiseSigmaMInterpTable(M_min, 1e20);

    struct ScalingConstants sc;
    set_scaling_constants(redshifts[0], matter_flags, astro_params, astro_flags, &sc, false);

    int i;
    double z_min = matter_params->Z_HEAT_MAX;
    double z_max = 0.;
    ...
    if (matter_flags->USE_INTERPOLATION_TABLES > 1) {
...
void get_global_Nion_z(MatterParams *matter_params, MatterFlags *matter_flags,
                       CosmoParams *cosmo_params, AstroParams *astro_params,
                       AstroFlags *astro_flags, int n_redshift, double *redshifts,
...
void get_conditional_FgtrM(MatterParams *matter_params, MatterFlags *matter_flags,
                           CosmoParams *cosmo_params, AstroParams *astro_params,
                           AstroFlags *astro_flags, double redshift, double R, int n_densities,
                           double *densities, double *out_fcoll, double *out_dfcoll) {
    Broadcast_struct_global_all(matter_params, matter_flags, cosmo_params, astro_params,
                                astro_flags);
    init_ps();

    double M_min = minimum_source_mass(redshift, true, astro_params, astro_flags);
    if (matter_flags->USE_INTERPOLATION_TABLES > 0) initialiseSigmaMInterpTable(M_min, 1e20);
...
void get_conditional_SFRD(MatterParams *matter_params, MatterFlags *matter_flags,
                          CosmoParams *cosmo_params, AstroParams *astro_params,
                          AstroFlags *astro_flags, double redshift, double R, int n_densities,
                          double *densities, double *log10_mturns, double *out_sfrd,
    ...
    struct ScalingConstants sc;
    set_scaling_constants(redshift, matter_flags, astro_params, astro_flags, &sc, false);
...
void get_conditional_Nion(MatterParams *matter_params, MatterFlags *matter_flags,
                          CosmoParams *cosmo_params, AstroParams *astro_params,
                          AstroFlags *astro_flags, double redshift, double R, int n_densities,
                          double *densities, double *log10_mturns_acg, double *log10_mturns_mcg,
...
void get_conditional_Xray(MatterParams *matter_params, MatterFlags *matter_flags,
                          CosmoParams *cosmo_params, AstroParams *astro_params,
                          AstroFlags *astro_flags, double redshift, double R, int n_densities,
                          double *densities, double *log10_mturns, double *out_xray) {
    Broadcast_struct_global_all(matter_params, matter_flags, cosmo_params, astro_params,
                                astro_flags);
    init_ps();
