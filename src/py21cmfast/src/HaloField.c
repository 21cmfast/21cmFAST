...
int check_halo(char *in_halo, UserParams *user_params, float R, int x, int y,
               int z, int check_type);
void init_halo_coords(HaloField *halos, long long unsigned int n_halos);
int pixel_in_halo(int grid_dim, int z_dim, int x, int x_index, int y,
                  int y_index, int z, int z_index, float Rsq_curr_index);
void free_halo_field(HaloField *halos);

int ComputeHaloField(float redshift_desc, float redshift,
                     UserParams *user_params, CosmoParams *cosmo_params,
                     AstroParams *astro_params, FlagOptions *flag_options,
                     InitialConditions *boxes,
                     unsigned long long int random_seed, HaloField *halos_desc,
                     HaloField *halos) {
  int status;
  ...
  Try {  // This Try brackets the whole function, so we don't indent.
    ...
    if (flag_options->HALO_STOCHASTICITY && redshift_desc > 0) {
      LOG_DEBUG(
          "Halo sampling switched on, bypassing halo finder to update %llu "
          "halos...",
          halos_desc->n_halos);
      // this would hold the two boxes used in the halo sampler, but here we are
      // taking the sample from a catalogue so we define a dummy here
      float *dummy_box = NULL;
      stochastic_halofield(user_params, cosmo_params, astro_params,
                           flag_options, random_seed, redshift_desc, redshift,
                           dummy_box, dummy_box, halos_desc, halos);
      return 0;
    }
    ...
    delta_crit =
        Deltac;  // for now set to spherical; check if we want elipsoidal later
    ...
    density_field = (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) *
                                                  KSPACE_NUM_PIXELS);
    density_field_saved = (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) *
                                                        KSPACE_NUM_PIXELS);
    ...
    double nhalo_debug =
        Nhalo_General(redshift, log(M_MIN), log(M_MAX_INTEGRAL)) * VOLUME *
        cosmo_params->OMm * RHOcrit;
    // expected halos above minimum filter mass
    LOG_DEBUG("DexM: We expect %.2f Halos between Masses [%.2e,%.2e] D %.3e",
              nhalo_debug, M_MIN, M_MAX_INTEGRAL, growth_factor);
#endif
...
#pragma omp for
    ...
    dft_r2c_cube(user_params->USE_FFTW_WISDOM, grid_dim, z_dim,
                 user_params->N_THREADS, density_field);

    // save a copy of the k-space density field
    memcpy(density_field_saved, density_field,
    ...
    if (flag_options->HALO_STOCHASTICITY) {
      // To save memory, we allocate the smaller (large mass) halofield here
      // instead of using halos_desc
    ...
    while ((R > 0.5 * Delta_R) &&
           (RtoM(R) >=
            M_MIN)) {  // filter until we get to half the pixel size or M_MIN
      M = RtoM(R);
      LOG_SUPER_DEBUG(
          "while loop for finding halos: R = %f 0.5*Delta_R = %f RtoM(R)=%e "
          "M_MIN=%e",
          R, 0.5 * Delta_R, M, M_MIN);

      // Pending a serious deep-dive into this algorithm, I will force DexM to
      // use the fitted parameters to the
      //       Sheth-Tormen mass function (as of right now, We do not even
      //       reproduce EPS results)
      delta_crit =
          growth_factor * sheth_delc_dexm(Deltac / growth_factor, sigma_z0(M));

      // first let's check if virialized halos of this size are rare enough
      // that we don't have to worry about them (let's define 7 sigma away, as
      // in Mesinger et al 05)
      ...
      memcpy(density_field, density_field_saved,
             sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
      ...
      dft_c2r_cube(user_params->USE_FFTW_WISDOM, grid_dim, z_dim,
                   user_params->N_THREADS, density_field);

      // *****************  BEGIN OPTIMIZATION ***************** //
      // to optimize speed, if the filter size is large (switch to collapse
      // fraction criteria later)
      if (user_params->DEXM_OPTIMIZE) {
        if (M > user_params->DEXM_OPTIMIZE_MINMASS) {
          memset(forbidden, 0, sizeof(char) * TOT_NUM_PIXELS);
          // now go through the list of existing halos and paint on the no-go
          // region onto <forbidden>
...
#pragma omp for
              ...
              for (y = 0; y < grid_dim; y++) {
                for (z = 0; z < z_dim; z++) {
                  ...
                  if (halo_buf > 0.) {
                    ...
                    check_halo(forbidden, user_params,
                               R_temp + user_params->DEXM_R_OVERLAP * R, x, y,
                               z, 2);
                  }
                }
              }
      ...
      // *****************  END OPTIMIZATION ***************** //
      // now lets scroll through the box, flagging all pixels with delta_m >
      // delta_crit
      r_halo_num = 0;

      // THREADING: Fix the race condition propertly to thread: it doesn't
      // matter which thread finds the halo first
      //   but if two threads find a halo in the same region simultaneously
      //   (before the first one updates in_halo) some halos could double-up
      // checking for overlaps in new halos after this loop could work, but I
      // would have to calculate distances between all new halos which sounds
      // slow
      for (x = 0; x < grid_dim; x++) {
        for (y = 0; y < grid_dim; y++) {
          for (z = 0; z < z_dim; z++) {
            delta_m = *((float *)density_field + R_FFT_INDEX(x, y, z)) *
                      growth_factor / TOT_NUM_PIXELS;
            // if not within a larger halo, and radii don't overlap, update
            // in_halo box
            // *****************  BEGIN OPTIMIZATION ***************** //
            if (user_params->DEXM_OPTIMIZE &&
                (M > user_params->DEXM_OPTIMIZE_MINMASS)) {
              if ((delta_m > delta_crit) && !forbidden[R_INDEX(x, y, z)]) {
                check_halo(in_halo, user_params, R, x, y, z,
                           2);  // flag the pixels contained within this halo
                check_halo(forbidden, user_params,
                           (1. + user_params->DEXM_R_OVERLAP) * R, x, y, z,
                           2);  // flag the pixels contained within this halo
            ...
            // *****************  END OPTIMIZATION ***************** //
              ...
              if ((delta_m > delta_crit)) {
                if (!in_halo[R_INDEX(x, y, z)]) {
                  if (!check_halo(in_halo, user_params, R, x, y, z,
                                  1)) {  // we found us a "new" halo!
                    check_halo(
                        in_halo, user_params, R, x, y, z,
                        2);  // flag the pixels contained within this halo
      ...
      LOG_SUPER_DEBUG("n_halo = %llu, total = %llu , D = %.3f, delcrit = %.3f",
                      r_halo_num, total_halo_num, growth_factor, delta_crit);

      R /= user_params->DELTA_R_FACTOR;
    }

    LOG_DEBUG(
        "Obtained %llu halo masses and positions, now saving to HaloField "
        "struct.",
        total_halo_num);
    ...
    // NOTE: To thread this part, we would need to keep track of how many halos
    // are in each thread before
    //       OR assign a buffer of size n_halo * n_thread (in case the last
    //       thread has all the halos), copy the structure from stochasticity.c
    //       with the assignment and condensing
    ...
    if (flag_options->HALO_STOCHASTICITY) {
      ...
      // First we construct a grid which corresponds to how much of a HII_DIM
      // cell is covered by halos
...
#pragma omp for
      ...
      stochastic_halofield(user_params, cosmo_params, astro_params,
                           flag_options, random_seed, redshift_desc, redshift,
                           boxes->lowres_density, halo_overlap_box, halos_dexm,
                           halos);
    ...
    if (halos->n_halos > 3)
      LOG_DEBUG("Halo Masses: %e %e %e %e", halos->halo_masses[0],
...
// Function check_halo combines the original two functions overlap_halo and
// update_in_halo from the original 21cmFAST. Lots of redundant code, hence
// reduced into a single function
int check_halo(char *in_halo, UserParams *user_params, float R, int x, int y,
               int z, int check_type) {
  ...
  //          Funtion UPDATE_IN_HALO takes in a box <in_halo> and flags all
  //          points which fall within radius R of (x,y,z).
  ...
  Rsq_curr_index =
      pow(R / user_params->BOX_LEN * grid_dim, 2);  // convert to index
      ...
      for (z_curr = z_min; z_curr <= z_max; z_curr++) {
        ...
        if (check_type == 1) {
          if (in_halo[curr_index] &&
              pixel_in_halo(grid_dim, z_dim, x, x_index, y, y_index, z, z_index,
                            Rsq_curr_index)) {
            // this pixel already belongs to a halo, and would want to become
            // part of this halo as well
            return 1;
          }
        } else if (check_type == 2) {
          // now check
          if (!in_halo[curr_index]) {
            if (pixel_in_halo(grid_dim, z_dim, x, x_index, y, y_index, z,
                              z_index, Rsq_curr_index)) {
              // we are within the sphere defined by R, so change flag in
              // in_halo array
...
int pixel_in_halo(int grid_dim, int z_dim, int x, int x_index, int y,
  ...
  // This checks the center, 6 faces, 12 edges and 8 corners of the cell == 27
  // points NOTE:The center check is not really necessary
