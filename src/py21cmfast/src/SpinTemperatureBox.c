...
void ts_main(float redshift, float prev_redshift, UserParams *user_params,
             CosmoParams *cosmo_params, AstroParams *astro_params,
             FlagOptions *flag_options, float perturbed_field_redshift,
             short cleanup, PerturbedField *perturbed_field,
             XraySourceBox *source_box, TsBox *previous_spin_temp,
...
double *dxheat_dt_box_MINI, *dxion_source_dt_box_MINI, *dxlya_dt_box_MINI,
    *dstarlya_dt_box_MINI;
...
double **freq_int_heat_tbl, **freq_int_ion_tbl, **freq_int_lya_tbl,
    **freq_int_heat_tbl_diff;
double **freq_int_ion_tbl_diff, **freq_int_lya_tbl_diff;

// R-dependent arrays which are set once
double *R_values, *dzpp_list, *dtdz_list, *zpp_growth, *zpp_for_evolve_list,
    *zpp_edge;
...
// a debug flag for printing results from a single cell without passing cell
// number to the functions
static int debug_printed;

int ComputeTsBox(float redshift, float prev_redshift, UserParams *user_params,
                 CosmoParams *cosmo_params, AstroParams *astro_params,
                 FlagOptions *flag_options, float perturbed_field_redshift,
                 short cleanup, PerturbedField *perturbed_field,
                 XraySourceBox *source_box, TsBox *previous_spin_temp,
  ...
  Try {  // This Try{} wraps the whole function.
    ...
    LOG_DEBUG("redshift=%f, prev_redshift=%f perturbed_field_redshift=%f",
              redshift, prev_redshift, perturbed_field_redshift);
    ...
    Broadcast_struct_global_all(user_params, cosmo_params, astro_params,
                                flag_options);
    omp_set_num_threads(user_params->N_THREADS);
    debug_printed = 0;

    ts_main(redshift, prev_redshift, user_params, cosmo_params, astro_params,
            flag_options, perturbed_field_redshift, cleanup, perturbed_field,
...
void alloc_global_arrays() {
  ...
  for (i = 0; i < x_int_NXHII; i++) {
    ...
    freq_int_heat_tbl_diff[i] =
        (double *)calloc(astro_params_global->N_STEP_TS, sizeof(double));
    freq_int_ion_tbl_diff[i] =
        (double *)calloc(astro_params_global->N_STEP_TS, sizeof(double));
    freq_int_lya_tbl_diff[i] =
        (double *)calloc(astro_params_global->N_STEP_TS, sizeof(double));
  }
  ...
  // spectral stuff
  dstarlya_dt_prefactor =
      calloc(astro_params_global->N_STEP_TS, sizeof(double));
  if (flag_options_global->USE_LYA_HEATING) {
    dstarlya_cont_dt_prefactor =
        calloc(astro_params_global->N_STEP_TS, sizeof(double));
    dstarlya_inj_dt_prefactor =
        calloc(astro_params_global->N_STEP_TS, sizeof(double));
  }

  if (flag_options_global->USE_MINI_HALOS) {
    dstarlya_dt_prefactor_MINI =
        calloc(astro_params_global->N_STEP_TS, sizeof(double));
    dstarlyLW_dt_prefactor =
        calloc(astro_params_global->N_STEP_TS, sizeof(double));
    dstarlyLW_dt_prefactor_MINI =
        calloc(astro_params_global->N_STEP_TS, sizeof(double));
    if (flag_options_global->USE_LYA_HEATING) {
      dstarlya_cont_dt_prefactor_MINI =
          calloc(astro_params_global->N_STEP_TS, sizeof(double));
      dstarlya_inj_dt_prefactor_MINI =
          calloc(astro_params_global->N_STEP_TS, sizeof(double));
    }
  }
  ...
  // Nonhalo stuff
  int num_R_boxes =
      user_params_global->MINIMIZE_MEMORY ? 1 : astro_params_global->N_STEP_TS;
...
void setup_z_edges(double zp) {
  ...
  R = L_FACTOR * user_params_global->BOX_LEN /
      (float)user_params_global->HII_DIM;
  R_factor = pow(astro_params_global->R_MAX_TS / R,
                 1 / ((float)astro_params_global->N_STEP_TS));
  ...
  for (R_ct = 0; R_ct < astro_params_global->N_STEP_TS; R_ct++) {
    ...
    zpp_edge[R_ct] = prev_zpp - (R_values[R_ct] - prev_R) * CMperMPC /
                                    drdz(prev_zpp);  // cell size
    zpp = (zpp_edge[R_ct] + prev_zpp) *
          0.5;  // average redshift value of shell: z'' + 0.5 * dz''
    ...
    M_min_R[R_ct] =
        minimum_source_mass(zpp_for_evolve_list[R_ct], true,
                            astro_params_global, flag_options_global);
    M_max_R[R_ct] = RtoM(R_values[R_ct]);

    R *= R_factor;
  }
  LOG_DEBUG("%d steps R range [%.2e,%.2e] z range [%.2f,%.2f]", R_ct,
            R_values[0], R_values[R_ct - 1], zp, zpp_edge[R_ct - 1]);
}
...
void calculate_spectral_factors(double zp) {
  ...
  // technically don't need to initialise since it is only used in R_ct > 1
  // conditional
  ...
  for (R_ct = 0; R_ct < astro_params_global->N_STEP_TS; R_ct++) {
    ...
    // We need to set up prefactors for how much of Lyman-N radiation is
    // recycled to Lyman-alpha
    sum_lyn_val = 0.;
    sum_lyn_val_MINI = 0.;
    sum_lyLW_val = 0.;
    sum_lyLW_val_MINI = 0.;
    ...
    for (n_ct = NSPEC_MAX; n_ct >= 3; n_ct--) {
      ...
      if (flag_options_global->USE_MINI_HALOS) {
        sum_lynto2_val_MINI +=
            frecycle(n_ct) * spectral_emissivity(nuprime, 0, 3);
    ...
    // At the edge of the redshift limit, part of the shell will still contain a
    // contribution
    //   This loop approximates the volume which contains the contribution
    //   and multiplies this by the previous shell's value.
    // This should probably be done separately for each line, since they all
    // face the same issue
    //   It could also be avoided by approximating the integral within each bin
    //   better than taking the value at the bin centre
    if (R_ct > 1 && sum_lyn_val == 0.0 && sum_lyn_prev > 0. && first_radii) {
      for (ii = 0; ii < n_pts_radii; ii++) {
        trial_zpp =
            prev_zpp + (zpp - prev_zpp) * (float)ii / ((float)n_pts_radii - 1.);
        counter = 0;
    ...
    LOG_ULTRA_DEBUG("z: %.2e R: %.2e int %.2e starlya: %.4e", zpp,
                    R_values[R_ct], zpp_integrand, dstarlya_dt_prefactor[R_ct]);
    ...
    if (flag_options_global->USE_MINI_HALOS) {
      ...
      LOG_ULTRA_DEBUG("starmini: %.2e LW: %.2e LWmini: %.2e",
                      dstarlya_dt_prefactor_MINI[R_ct],
...
void prepare_filter_boxes(double redshift, float *input_dens, float *input_vcb,
                          float *input_j21, fftwf_complex *output_dens,
                          fftwf_complex *output_LW) {
  int i, j, k;
  unsigned long long int ct;
  double curr_vcb, curr_j21, M_buf;

// NOTE: Meraxes just applies a pointer cast box = (fftwf_complex *) input.
// Figure out why this works,
//       They pad the input by a factor of 2 to cover the complex part, but from
//       the type I thought it would be stored [(r,c),(r,c)...] Not
//       [(r,r,r,r....),(c,c,c....)] so the alignment should be wrong, right?
#pragma omp parallel for private(i, j, k) \
    num_threads(user_params_global->N_THREADS) collapse(3)
  ...
  ////////////////// Transform unfiltered box to k-space to prepare for
  /// filtering /////////////////
  dft_r2c_cube(user_params_global->USE_FFTW_WISDOM, user_params_global->HII_DIM,
               HII_D_PARA, user_params_global->N_THREADS, output_dens);
#pragma omp parallel for num_threads(user_params_global->N_THREADS)
  ...
  if (flag_options_global->USE_MINI_HALOS) {
    curr_vcb =
        flag_options_global->FIX_VCB_AVG ? astro_params_global->FIXED_VAVG : 0;
#pragma omp parallel for firstprivate(curr_vcb) private(                     \
        i, j, k, curr_j21, M_buf) num_threads(user_params_global->N_THREADS) \
    collapse(3)
    for (i = 0; i < user_params_global->HII_DIM; i++) {
      for (j = 0; j < user_params_global->HII_DIM; j++) {
        for (k = 0; k < HII_D_PARA; k++) {
          ...
          // NOTE: we don't use reionization_feedback here, I assume it wouldn't
          // do much but it's inconsistent
    ...
    ////////////////// Transform unfiltered box to k-space to prepare for
    /// filtering /////////////////
    dft_r2c_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
...
// fill a box[R_ct][box_ct] array for use in TS by filtering on different scales
// and storing results
void fill_Rbox_table(float **result, fftwf_complex *unfiltered_box,
                     double *R_array, int n_R, double min_value,
                     double const_factor, double *min_arr, double *average_arr,
                     double *max_arr) {
  ...
  fftwf_complex *box = (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) *
                                                     HII_KSPACE_NUM_PIXELS);
  // Smooth the density field, at the same time store the minimum and maximum
  // densities for their usage in the interpolation tables
  for (R_ct = 0; R_ct < n_R; R_ct++) {
    ...
    if (R > L_FACTOR *
                (user_params_global->BOX_LEN / user_params_global->HII_DIM)) {
      filter_box(box, 1, flag_options_global->HEAT_FILTER, R, 0.);
    }

    // now fft back to real space
    dft_c2r_cube(user_params_global->USE_FFTW_WISDOM,
                 user_params_global->HII_DIM, HII_D_PARA,
                 user_params_global->N_THREADS, box);
    // copy over the values
#pragma omp parallel private(i, j, k) num_threads(user_params_global->N_THREADS)
    {
      float curr;
#pragma omp for reduction(+ : ave_buffer) reduction(max : max_out_R) \
    reduction(min : min_out_R)
      for (i = 0; i < user_params_global->HII_DIM; i++) {
        for (j = 0; j < user_params_global->HII_DIM; j++) {
          for (k = 0; k < HII_D_PARA; k++) {
...
// NOTE: I've moved this to a function to help in simplicity, it is not clear
// whether it is faster
//   to do all of one radii at once (more clustered FFT and larger thread
//   blocks) or all of one box (better memory locality)
// TODO: filter speed tests
void one_annular_filter(float *input_box, float *output_box, double R_inner,
                        double R_outer, double *u_avg, double *f_avg) {
  int i, j, k;
  unsigned long long int ct;
  double unfiltered_avg = 0;
  double filtered_avg = 0;

  fftwf_complex *unfiltered_box = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  fftwf_complex *filtered_box = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

#pragma omp parallel private(i, j, k) \
    num_threads(user_params_global->N_THREADS) reduction(+ : unfiltered_avg)
  {
    float curr_val;
#pragma omp for
    for (i = 0; i < user_params_global->HII_DIM; i++) {
  ...
  ////////////////// Transform unfiltered box to k-space to prepare for
  /// filtering /////////////////
  // this would normally only be done once but we're using a different redshift
  // for each R now
  dft_r2c_cube(user_params_global->USE_FFTW_WISDOM, user_params_global->HII_DIM,
               HII_D_PARA, user_params_global->N_THREADS, unfiltered_box);

// remember to add the factor of VOLUME/TOT_NUM_PIXELS when converting from real
// space to k-space Note: we will leave off factor of VOLUME, in anticipation of
...
#pragma omp for
  ...
  // Smooth the density field, at the same time store the minimum and maximum
  // densities for their usage in the interpolation tables copy over unfiltered
  // box
  memcpy(filtered_box, unfiltered_box,
         sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  ...
  // now fft back to real space
  dft_c2r_cube(user_params_global->USE_FFTW_WISDOM, user_params_global->HII_DIM,
               HII_D_PARA, user_params_global->N_THREADS, filtered_box);

// copy over the values
#pragma omp parallel private(i, j, k) \
    num_threads(user_params_global->N_THREADS) reduction(+ : filtered_avg)
  {
    float curr_val;
#pragma omp for
    for (i = 0; i < user_params_global->HII_DIM; i++) {
...
// fill a box[R_ct][box_ct] array for use in TS by filtering on different scales
// and storing results Similar to fill_Rbox_table but called using different
// redshifts for each scale
int UpdateXraySourceBox(UserParams *user_params, CosmoParams *cosmo_params,
                        AstroParams *astro_params, FlagOptions *flag_options,
                        HaloBox *halobox, double R_inner, double R_outer,
                        int R_ct, XraySourceBox *source_box) {
  int status;
  Try {
    ...
    one_annular_filter(halobox->halo_sfr,
                       &(source_box->filtered_sfr[R_ct * HII_TOT_NUM_PIXELS]),
                       R_inner, R_outer, &sfr_avg, &fsfr_avg);
    one_annular_filter(halobox->halo_xray,
                       &(source_box->filtered_xray[R_ct * HII_TOT_NUM_PIXELS]),
    ...
    if (R_ct == astro_params->N_STEP_TS - 1)
      LOG_DEBUG("finished XraySourceBox");

    LOG_SUPER_DEBUG(
        "R_inner = %8.3f | mean filtered sfr  = %10.3e (%10.3e MINI) "
        "unfiltered %10.3e (%10.3e MINI) mean log10McritLW %.4e",
        R_inner, fsfr_avg, fsfr_avg_mini, sfr_avg, sfr_avg_mini,
        source_box->mean_log10_Mcrit_LW[R_ct]);
    LOG_SUPER_DEBUG(
        "R_outer = %8.3f | mean filtered xray = %10.3e unfiltered %10.3e",
        R_outer, fxray_avg, xray_avg);
...
//   The x_e tables are not regular, hence the precomputation of indices/interp
//   points
void fill_freqint_tables(double zp, double x_e_ave,
                         double filling_factor_of_HI_zp,
                         double *log10_Mcrit_LW_ave, int R_mm,
...
#pragma omp parallel private(R_ct, x_e_ct, lower_int_limit) \
    num_threads(user_params_global->N_THREADS)
  {
#pragma omp for
    // In TauX we integrate Nion from zpp to zp using the LW turnover mass at zp
    // (predending its at zpp)
    //   Calculated from the average smoothed zp grid (from previous LW field)
    //   at radius R
    // NOTE: The one difference currently between the halobox and density field
    // options is the weighting of the average
    //   density -> volume weighted cell average || halo -> halo weighted
    //   average
    for (R_ct = R_start; R_ct < R_end; R_ct++) {
      if (flag_options_global->USE_MINI_HALOS) {
        lower_int_limit = fmax(nu_tau_one_MINI(zp, zpp_for_evolve_list[R_ct],
                                               x_e_ave, filling_factor_of_HI_zp,
                                               log10_Mcrit_LW_ave[R_ct], sc),
                               (astro_params_global->NU_X_THRESH) * NU_over_EV);
      } else {
        lower_int_limit = fmax(nu_tau_one(zp, zpp_for_evolve_list[R_ct],
                                          x_e_ave, filling_factor_of_HI_zp, sc),
                               (astro_params_global->NU_X_THRESH) * NU_over_EV);
      }
      // set up frequency integral table for later interpolation for the cell's
      // x_e value
      for (x_e_ct = 0; x_e_ct < x_int_NXHII; x_e_ct++) {
        ...
        if (x_e_ct > 0) {
          freq_int_heat_tbl_diff[x_e_ct - 1][R_ct] =
              freq_int_heat_tbl[x_e_ct][R_ct] -
              freq_int_heat_tbl[x_e_ct - 1][R_ct];
          freq_int_ion_tbl_diff[x_e_ct - 1][R_ct] =
              freq_int_ion_tbl[x_e_ct][R_ct] -
              freq_int_ion_tbl[x_e_ct - 1][R_ct];
          freq_int_lya_tbl_diff[x_e_ct - 1][R_ct] =
              freq_int_lya_tbl[x_e_ct][R_ct] -
              freq_int_lya_tbl[x_e_ct - 1][R_ct];
        }
      }
    }
// separating the inverse diff loop to prevent a race on different R_ct
// (shouldn't matter)
#pragma omp for
  ...
  for (R_ct = 0; R_ct < astro_params_global->N_STEP_TS; R_ct++) {
    for (x_e_ct = 0; x_e_ct < x_int_NXHII; x_e_ct++) {
      if (isfinite(freq_int_heat_tbl[x_e_ct][R_ct]) == 0 ||
        ...
        LOG_ERROR(
            "One of the frequency interpolation tables has an infinity or a "
            "NaN");
...
// construct a Ts table above Z_HEAT_MAX, this can happen if we are computing
// the first box or if we request a redshift above Z_HEAT_MAX
void init_first_Ts(TsBox *box, float *dens, float z, float zp, double *x_e_ave,
                   double *Tk_ave) {
...
int global_reion_properties(double zp, double x_e_ave,
                            double *log10_Mcrit_LW_ave, double *mean_sfr_zpp,
                            double *mean_sfr_zpp_mini, double *Q_HI) {
  int R_ct;
  double sum_nion = 0, sum_nion_mini = 0;
  double zpp;

  // For a lot of global evolution, this code uses Nion_general. We can replace
  // this with the halo field at the same snapshot, but the nu integrals go from
  // zp to zpp to find the tau = 1 barrier so it needs the QHII in a range
  // [zp,zpp]. I want to replace this whole thing with a global history struct
  // but I will need to change the Tau function chain.
  double determine_zpp_max, determine_zpp_min;

  // at z', we need a differenc constant struct
  struct ScalingConstants sc;
  set_scaling_constants(zp, astro_params_global, flag_options_global, &sc,
                        false);
  ...
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    ...
    determine_zpp_max =
        zpp_for_evolve_list[astro_params_global->N_STEP_TS - 1] * 1.001;

    // We need the tables for the frequency integrals & mean fixing
    // NOTE: These global tables confuse me, we do ~400 (x50 for mini) integrals
    // to build the table, despite only having
    //   ~100 redshifts. The benefit of interpolating here would only matter if
    //   we keep the same table over subsequent snapshots, which we don't seem
    //   to do. The Nion table is used in nu_tau_one a lot but I think there's a
    //   better way to do that
    if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
      /* initialise interpolation of the mean collapse fraction for global
       * reionization.*/
      initialise_Nion_Ts_spline(zpp_interp_points_SFR, determine_zpp_min,
                                determine_zpp_max, &sc);

      initialise_SFRD_spline(zpp_interp_points_SFR, determine_zpp_min,
                             determine_zpp_max, &sc);
    } else {
  ...
  // For consistency between halo and non-halo based, the NO_LIGHT and
  // filling_factor_zp
  //   are based on the expected global Nion. as mentioned above it would be
  //   nice to change this to a saved reionisation/sfrd history from previous
  //   snapshots
  ...
  ION_EFF_FACTOR = astro_params_global->F_STAR10 *
                   astro_params_global->F_ESC10 * astro_params_global->POP2_ION;
  ION_EFF_FACTOR_MINI = astro_params_global->F_STAR7_MINI *
                        astro_params_global->F_ESC7_MINI *
                        astro_params_global->POP3_ION;

  // NOTE: only used without MASS_DEPENDENT_ZETA
  *Q_HI =
      1 - (ION_EFF_FACTOR * sum_nion + ION_EFF_FACTOR_MINI * sum_nion_mini) /
              (1.0 - x_e_ave);
    ...
    for (R_ct = 0; R_ct < astro_params_global->N_STEP_TS; R_ct++) {
      ...
      if (flag_options_global->USE_MINI_HALOS) {
        mean_sfr_zpp_mini[R_ct] =
            EvaluateSFRD_MINI(zpp, log10_Mcrit_LW_ave[R_ct], &sc);
      }
    }
...
void calculate_sfrd_from_grid(int R_ct, float *dens_R_grid, float *Mcrit_R_grid,
                              float *sfrd_grid, float *sfrd_grid_mini,
                              double *ave_sfrd, double *ave_sfrd_mini,
                              struct ScalingConstants *sc) {
  ...
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
      initialise_SFRD_Conditional_table(
          zpp_for_evolve_list[R_ct], min_densities[R_ct] * zpp_growth[R_ct],
          max_densities[R_ct] * zpp_growth[R_ct] * 1.001, M_min_R[R_ct],
          M_max_R[R_ct], M_max_R[R_ct], sc);
    } else {
      initialise_FgtrM_delta_table(min_densities[R_ct] * zpp_growth[R_ct],
                                   max_densities[R_ct] * zpp_growth[R_ct],
                                   zpp_for_evolve_list[R_ct], zpp_growth[R_ct],
                                   sigma_min[R_ct], sigma_max[R_ct]);
    }
  }
...
#pragma omp for reduction(+ : ave_sfrd_buf, ave_sfrd_buf_mini)
    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++) {
      ...
      if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
        fcoll = EvaluateSFRD_Conditional(curr_dens, zpp_growth[R_ct],
                                         M_min_R[R_ct], M_max_R[R_ct],
                                         M_max_R[R_ct], sigma_max[R_ct], sc);
        sfrd_grid[box_ct] = (1. + curr_dens) * fcoll;

        if (flag_options_global->USE_MINI_HALOS) {
          fcoll_MINI = EvaluateSFRD_Conditional_MINI(
              curr_dens, curr_mcrit, zpp_growth[R_ct], M_min_R[R_ct],
              M_max_R[R_ct], M_max_R[R_ct], sigma_max[R_ct], sc);
          sfrd_grid_mini[box_ct] = (1. + curr_dens) * fcoll_MINI;
        }
      } else {
        fcoll = EvaluateFcoll_delta(curr_dens, zpp_growth[R_ct],
                                    sigma_min[R_ct], sigma_max[R_ct]);
        dfcoll = EvaluatedFcolldz(curr_dens, zpp_for_evolve_list[R_ct],
                                  sigma_min[R_ct], sigma_max[R_ct]);
...
struct spintemp_from_sfr_prefactors {
  ...
  double lya_star_prefactor;   // converts SFR density -> stellar baryon density
                               // + prefactors
...
void set_zp_consts(double zp, struct spintemp_from_sfr_prefactors *consts) {
  ...
  if (fabs(astro_params_global->X_RAY_SPEC_INDEX - 1.0) < 1e-6) {
    luminosity_converstion_factor = (astro_params_global->NU_X_THRESH) *
                                    NU_over_EV *
                                    log(astro_params_global->NU_X_BAND_MAX /
    ...
    luminosity_converstion_factor =
        pow((astro_params_global->NU_X_BAND_MAX) * NU_over_EV,
  ...
  // Finally, convert to the correct units. NU_over_EV*hplank as only want to
  ...
  // Note: These used to be determined in evolveInt (and other functions). But I
  // moved them all here, into a single location.
  consts->Trad = T_cmb * (1.0 + zp);
  consts->Trad_inv = 1.0 / consts->Trad;
  consts->Ts_prefactor =
      pow(1.0e-7 * (1.342881e-7 / consts->hubble_zp) * No * pow(1 + zp, 3),
          1. / 3.);

  gamma_alpha = f_alpha * pow(Ly_alpha_HZ * e_charge / (C / 10.),
                              2.);  // division of C/10. is converstion of
                                    // electric charge from esu to coulomb
  gamma_alpha /= 6. * (m_e / 1000.) * pow(C / 100., 3.) *
                 vac_perm;  // division by 1000. to convert gram to kg and
                            // division by 100. to convert cm to m

  consts->xa_tilde_prefactor = 8. * PI * pow(Ly_alpha_ANG * 1.e-8, 2.) *
                               gamma_alpha *
  ...
  consts->xc_inverse =
      pow(1.0 + zp, 3.0) * T21 / (consts->Trad * A10_HYPERFINE);

  consts->dcomp_dzp_prefactor = (-1.51e-4) / (consts->hubble_zp / Ho) /
                                (cosmo_params_global->hlittle) *
                                pow(consts->Trad, 4.0) / (1.0 + zp);

  consts->Nb_zp =
      N_b0 * (1 + zp) * (1 + zp) *
      (1 + zp);  // used for lya_X and sinks NOTE: the 2 density factors are
                 // from source & absorber since its downscattered x-ray
  consts->N_zp = No * (1 + zp) * (1 + zp) * (1 + zp);  // used for CMB
  consts->lya_star_prefactor =
      C / FOURPI * Msun / m_p *
      (1 -
       0.75 * cosmo_params_global->Y_He);  // converts SFR density -> stellar
                                           // baryon density + prefactors
  ...
  if (flag_options_global->USE_HALO_FIELD) {
    ...
    consts->volunit_inv =
        cosmo_params_global->OMb * RHOcrit * pow(CMperMPC, -3);
  }

  LOG_DEBUG("Set zp consts xr %.2e Tr %.2e Ts %.2e xa %.2e xc %.2e cm %.2e",
            consts->xray_prefactor, consts->Trad, consts->Ts_prefactor,
            consts->xa_tilde_prefactor, consts->xc_inverse,
            consts->dcomp_dzp_prefactor);
  LOG_DEBUG("Nb %.2e la %.2e vi %.2e D %.2e H %.2e dD %.2e dt %.2e",
            consts->Nb_zp, consts->lya_star_prefactor, consts->volunit_inv,
            consts->growth_zp, consts->hubble_zp, consts->dgrowth_dzp,
            consts->dt_dzp);
}
...
// Function for calculating the Ts box outputs quickly by using pre-calculated
// constants
//   as much as possible
struct Ts_cell get_Ts_fast(float zp, float dzp,
                           struct spintemp_from_sfr_prefactors *consts,
                           struct Box_rad_terms *rad) {
  // Now we can solve the evolution equations  //
  struct Ts_cell output;
  double tau21, xCMB, dxion_sink_dt, dxe_dzp, dadia_dzp, dspec_dzp, dcomp_dzp,
      dxheat_dzp;
  double dCMBheat_dzp, eps_CMB, eps_Lya_cont, eps_Lya_inj, E_continuum,
      E_injected, Ndot_alpha_cont, Ndot_alpha_inj;

  tau21 = (3 * hplank * A10_HYPERFINE * C * Lambda_21 * Lambda_21 / 32. / PI /
           k_B) *
          ((1 - rad->prev_xe) * consts->N_zp) / rad->prev_Ts /
          consts->hubble_zp;
  xCMB = (1. - exp(-tau21)) / tau21;

  // Electron density
  // NOTE: Nb_zp includes helium, TODO: make sure this is right
  dxion_sink_dt = alpha_A(rad->prev_Tk) * astro_params_global->CLUMPING_FACTOR *
                  rad->prev_xe * rad->prev_xe * f_H * consts->Nb_zp *
                  (1. + rad->delta);
  ...
  if (fabs(rad->delta) > FRACT_FLOAT_ERR)
    dadia_dzp +=
        consts->dgrowth_dzp / (consts->growth_zp * (1.0 / rad->delta + 1.0));
  ...
  // Compton heating
  dcomp_dzp = consts->dcomp_dzp_prefactor *
              (rad->prev_xe / (1.0 + rad->prev_xe + f_He)) *
              (consts->Trad - rad->prev_Tk);

  // X-ray heating
  dxheat_dzp =
      rad->dxheat_dt * consts->dt_dzp * 2.0 / 3.0 / k_B / (1.0 + rad->prev_xe);
  // CMB heating rate
  dCMBheat_dzp = 0.;
  if (flag_options_global->USE_CMB_HEATING) {
    // Meiksin et al. 2021
    eps_CMB = (3. / 4.) * (consts->Trad / T21) * A10_HYPERFINE * f_H *
              (hplank * hplank / Lambda_21 / Lambda_21 / m_p) *
              (1. + 2. * rad->prev_Tk / T21);
    dCMBheat_dzp = -eps_CMB * (2. / 3. / k_B / (1. + rad->prev_xe)) /
  ...
  if (flag_options_global->USE_LYA_HEATING) {
    ...
    Ndot_alpha_cont = (4. * PI * Ly_alpha_HZ) /
                      (consts->Nb_zp * (1. + rad->delta)) / (1. + zp) / C *
                      rad->dstarlya_cont_dt;
    Ndot_alpha_inj = (4. * PI * Ly_alpha_HZ) /
                     (consts->Nb_zp * (1. + rad->delta)) / (1. + zp) / C *
                     rad->dstarlya_inj_dt;
    eps_Lya_cont =
        -Ndot_alpha_cont * E_continuum * (2. / 3. / k_B / (1. + rad->prev_xe));
    eps_Lya_inj =
        -Ndot_alpha_inj * E_injected * (2. / 3. / k_B / (1. + rad->prev_xe));
  }
  ...
  if (x_e > 1)  // can do this late in evolution if dzp is too large
    ...
    x_e = 0;
  // NOTE: does this stop cooling if we ever go over the limit? I suppose that
  // shouldn't happen but it's strange anyway
  Tk = rad->prev_Tk;
  if (Tk < MAX_TK) {
    if (debug_printed == 0 && omp_get_thread_num() == 0)
      LOG_SUPER_DEBUG(
          "Heating Terms: T %.4e | X %.4e | c %.4e | S %.4e | A %.4e | c %.4e "
          "| lc %.4e | li %.4e | dz %.4e",
          Tk, dxheat_dzp, dcomp_dzp, dspec_dzp, dadia_dzp, dCMBheat_dzp,
          eps_Lya_cont, eps_Lya_inj, dzp);

    Tk += (dxheat_dzp + dcomp_dzp + dspec_dzp + dadia_dzp + dCMBheat_dzp +
           eps_Lya_cont + eps_Lya_inj) *
          dzp;
    if (debug_printed == 0 && omp_get_thread_num() == 0)
      LOG_SUPER_DEBUG("--> T %.4e", Tk);
  }
  // spurious bahaviour of the trapazoidalintegrator. generally overcooling in
  // underdensities
  ...
  double J_alpha_tot =
      rad->dstarlya_dt + rad->dxlya_dt;  // not really d/dz, but the lya flux

  // JD: I'm leaving these as comments in case I'm wrong, but there's NO WAY a
  // compiler doesn't know the fastest way to invert a number
  ...
  xc_fast = (1.0 + rad->delta) * consts->xc_inverse *
            ((1.0 - x_e) * No * kappa_10(Tk, 0) +
             x_e * N_b0 * kappa_10_elec(Tk, 0) + x_e * No * kappa_10_pH(Tk, 0));

  xi_power =
      consts->Ts_prefactor * cbrt((1.0 + rad->delta) * (1.0 - x_e) * T_inv_sq);

  xa_tilde_fast_arg =
      consts->xa_tilde_prefactor * J_alpha_tot *
  ...
  if (J_alpha_tot > 1.0e-20) {  // Must use WF effect
    ...
    while (fabs(TS_fast - TSold_fast) / TS_fast > 1.0e-3) {
      ...
      xa_tilde_fast = (1.0 - 0.0631789 * T_inv + 0.115995 * T_inv_sq -
                       0.401403 * T_inv * pow(TS_fast, -1.) +
                       0.336463 * T_inv_sq * pow(TS_fast, -1.)) *
                      xa_tilde_fast_arg;

      TS_fast = (xCMB + xa_tilde_fast + xc_fast) *
                pow(xCMB * consts->Trad_inv +
                        xa_tilde_fast *
                            (T_inv + 0.405535 * T_inv * pow(TS_fast, -1.) -
                             0.405535 * T_inv_sq) +
                        xc_fast * T_inv,
                    -1.);
    }
  ...
  if (debug_printed == 0 && omp_get_thread_num() == 0) {
    LOG_SUPER_DEBUG(
        "Spin terms xc %.5e xa %.5e xC %.5e Ti %.5e T2 %.5e --> T %.4e",
        xc_fast, xa_tilde_fast_arg, xCMB, T_inv, T_inv_sq, TS_fast);
    debug_printed = 1;
  }
  // It can very rarely result in a negative spin temperature. If negative, it
  // is a very small number. Take the absolute value, the optical depth can deal
  // with very large numbers, so ok to be small
...
void ts_main(float redshift, float prev_redshift, UserParams *user_params,
  ...
  init_ps();  // Must be always initialised due to the strange way
              // Ionisationbox.c expects some initialisation
  ...
  // This M_MIN just sets the sigma table range, the minimum mass for the
  // integrals is set per radius in setup_z_edges
  ...
  if (redshift >= user_params->Z_HEAT_MAX) {
    ...
    init_first_Ts(this_spin_temp, perturbed_field->density,
                  perturbed_field_redshift, redshift, &x_e_ave_p, &Tk_ave_p);
    return;
  }
  ...
  // Since we use the average Mturn for the global tables this must be done
  // first NOTE: The filtered Mturn for the previous snapshot is used for Fcoll
  // at ALL zpp
  //   regardless of distance from current reshift, this also goes for the
  //   averages
  ...
  if (!flag_options->USE_HALO_FIELD) {
    ...
    delta_unfiltered = (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) *
                                                     HII_KSPACE_NUM_PIXELS);
    if (flag_options->USE_MINI_HALOS)
      log10_Mcrit_LW_unfiltered = (fftwf_complex *)fftwf_malloc(
          sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

    prepare_filter_boxes(redshift, perturbed_field->density,
                         ini_boxes->lowres_vcb, previous_spin_temp->J_21_LW_box,
    ...
    if (!user_params->MINIMIZE_MEMORY) {
      fill_Rbox_table(delNL0, delta_unfiltered, R_values,
                      astro_params->N_STEP_TS, -1, inverse_growth_factor_z,
                      min_densities, ave_dens, max_densities);
      if (flag_options->USE_MINI_HALOS) {
        // NOTE: we are using previous_zp LW threshold for all zpp, inconsistent
        // with the halo model
        log10_Mcrit_limit = log10(lyman_werner_threshold(
            redshift, 0., 0.,
            astro_params));  // minimum turnover NOTE: should be zpp_max?
        fill_Rbox_table(log10_Mcrit_LW, log10_Mcrit_LW_unfiltered, R_values,
                        astro_params->N_STEP_TS, log10_Mcrit_limit, 1,
                        min_log10_MturnLW, ave_log10_MturnLW,
                        max_log10_MturnLW);
      }
    } else {
      ...
      if (flag_options->USE_MINI_HALOS) {
        fill_Rbox_table(log10_Mcrit_LW, log10_Mcrit_LW_unfiltered,
                        &(R_values[0]), 1, 0, 1, &min_log10_MturnLW[0],
                        &ave_log10_MturnLW[0], &max_log10_MturnLW[0]);
      }
    }
...
#pragma omp for reduction(+ : x_e_ave_p, Tk_ave_p)
  ...
  // this should initialise and use the global tables (given box average
  // turnovers)
  //   and use them to give: Filling factor at zp (only used for
  //   !MASS_DEPENDENT_ZETA to get ion_eff) global SFRD at each filter radius
  //   (numerator of ST_over_PS factor)
  double Q_HI_zp;
  NO_LIGHT = global_reion_properties(redshift, x_e_ave_p, ave_log10_MturnLW,
                                     mean_sfr_zpp, mean_sfr_zpp_mini, &Q_HI_zp);
...
#pragma omp for
    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++) {
      ...
      // Check if ionized fraction is within boundaries; if not, adjust to be
      // within
      ...
      // these are the index and interpolation term, moved outside the R loop
      // and stored to not calculate them R times
      m_xHII_low_box[box_ct] = locate_xHII_index(xHII_call);
      inverse_val_box[box_ct] =
          (xHII_call - x_int_XHII[m_xHII_low_box[box_ct]]) *
      ...
      if (flag_options->USE_MINI_HALOS) dstarlyLW_dt_box[box_ct] = 0.;
  ...
  if (!NO_LIGHT) {
    for (R_ct = astro_params->N_STEP_TS; R_ct--;) {
      ...
      if (flag_options->USE_HALO_FIELD)
        z_edge_factor =
            fabs(dzpp_for_evolve *
                 dtdz_list[R_ct]);  // dtdz'' dz'' -> dR for the radius sum (C
        ...
        z_edge_factor = dzpp_for_evolve;  // in this case we use dfcoll/dz'' so
                                          // we do not need dtdz
      ...
      if (!flag_options->USE_HALO_FIELD) {
        if (user_params->MINIMIZE_MEMORY) {
          ...
          // This unnecessarily allocates and frees a fftwf box every time but
          // surely that's not a bottleneck
          ...
          if (flag_options->USE_MINI_HALOS) {
            fill_Rbox_table(log10_Mcrit_LW, log10_Mcrit_LW_unfiltered,
                            &(R_values[R_ct]), 1, 0, 1,
                            &min_log10_MturnLW[R_ct], &ave_log10_MturnLW[R_ct],
                            &max_log10_MturnLW[R_ct]);
          }
          ...
          if (flag_options_global->USE_MINI_HALOS) {
            mean_sfr_zpp_mini[R_ct] = EvaluateSFRD_MINI(
                zpp_for_evolve_list[R_ct], ave_log10_MturnLW[R_ct], &sc);
          }
          // fill one row of the interp tables
          fill_freqint_tables(redshift, x_e_ave, Q_HI_zp, ave_log10_MturnLW,
                              R_ct, &sc);
        }
        // set input pointers (doing things this way helps with flag
        // flexibility)
        delta_box_input = delNL0[R_index];
        if (flag_options->USE_MINI_HALOS) {
          Mcrit_box_input = log10_Mcrit_LW[R_index];
        }
        calculate_sfrd_from_grid(R_ct, delta_box_input, Mcrit_box_input,
                                 del_fcoll_Rct, del_fcoll_Rct_MINI, &ave_fcoll,
                                 &ave_fcoll_MINI, &sc);
...
//   If we assume attenuation at mean density as we do in nu_tau_one(), we HAVE
//   to assume mean density absorption otherwise we do not conserve photons
#pragma omp parallel private(box_ct) num_threads(user_params->N_THREADS)
      {
        // private variables
        int xidx;
...
#pragma omp for
        for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++) {
          ...
          // MINIMIZE_MEMORY to work (replaces ~40*NUM_PIXELS with
          // ~4-16*NUM_PIXELS),
          //   as the FFT is done in the R-loop.
          // Secondly, it is *likely* faster to fill these boxes, and sum with a
          // outer R loop than an inner one.

          if (flag_options->USE_HALO_FIELD) {
            sfr_term =
                source_box
                    ->filtered_sfr[R_index * HII_TOT_NUM_PIXELS + box_ct] *
                z_edge_factor;
            // Minihalos and s->yr conversion are already included here
            xray_sfr =
                source_box
                    ->filtered_xray[R_index * HII_TOT_NUM_PIXELS + box_ct] *
                z_edge_factor * xray_R_factor * 1e38;
          } else {
            // NOTE: for !USE_MASS_DEPENDENT_ZETA, F_STAR10 is still used for
            // constant stellar fraction
            ...
            xray_sfr = sfr_term * astro_params->L_X * xray_R_factor *
                       SperYR;  // SperYR previously (3.1556226e7)
                                // (31556925.9747 in constants???)
          }
          if (flag_options->USE_MINI_HALOS) {
            if (flag_options->USE_HALO_FIELD) {
              sfr_term_mini =
              ...
              sfr_term_mini = del_fcoll_Rct_MINI[box_ct] * z_edge_factor *
                              avg_fix_term_MINI * astro_params->F_STAR7_MINI;
              xray_sfr += sfr_term_mini * astro_params->L_X_MINI *
          ...
          dstarlya_dt_box[box_ct] +=
              ...
              sfr_term_mini * starlya_factor_mini;  // the MINI factors might
                                                    // not be allocated
          if (flag_options->USE_LYA_HEATING) {
          ...
          // I cannot check the integral if we are using the halo field since
          // delNL0 (filtered density) is not calculated
#if LOG_LEVEL >= SUPER_DEBUG_LEVEL
          if (box_ct == 0 && !flag_options->USE_HALO_FIELD) {
            ...
            LOG_SUPER_DEBUG(
                ...
                integral_db, delNL0[R_index][box_ct]);
            if (flag_options->USE_MINI_HALOS)
              LOG_SUPER_DEBUG(
            ...
            LOG_SUPER_DEBUG("xh %.2e | xi %.2e | xl %.2e | sl %.2e",
                            dxheat_dt_box[box_ct] / astro_params->L_X,
                            dxion_source_dt_box[box_ct] / astro_params->L_X,
                            dxlya_dt_box[box_ct] / astro_params->L_X,
                            dstarlya_dt_box[box_ct]);
            if (flag_options->USE_LYA_HEATING)
...
#pragma omp parallel private(box_ct)
  {
    ...
    struct Box_rad_terms rad;
#pragma omp for reduction(+ : J_alpha_ave, xheat_ave, xion_ave, Ts_ave, \
                              Tk_ave, x_e_ave, eps_lya_cont_ave,        \
                              eps_lya_inj_ave)
    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++) {
      curr_delta = perturbed_field->density[box_ct] * growth_factor_zp *
                   inverse_growth_factor_z;
      // NOTE: this corrected for aliasing before, but sometimes there are still
      // some delta==-1 cells
      ...
      rad.dxion_dt = dxion_source_dt_box[box_ct] * zp_consts.xray_prefactor *
                     zp_consts.volunit_inv;
      rad.dxlya_dt =
          dxlya_dt_box[box_ct] * zp_consts.xray_prefactor *
          zp_consts.volunit_inv * zp_consts.Nb_zp *
      ...
      if (box_ct == 0) {
        LOG_SUPER_DEBUG(
            ...
            "dstarlya: %.3e",
            curr_delta, rad.dxheat_dt, rad.dxion_dt, rad.dxlya_dt,
            rad.dstarlya_dt);
        if (flag_options->USE_LYA_HEATING) {
  ...
  if (LOG_LEVEL >= DEBUG_LEVEL) {
    ...
    LOG_DEBUG("AVERAGES zp = %.2e Ts = %.2e x_e = %.2e Tk %.2e", redshift,
              Ts_ave, x_e_ave, Tk_ave);
    LOG_DEBUG("J_alpha = %.2e xheat = %.2e xion = %.2e", J_alpha_ave, xheat_ave,
              xion_ave);
    if (flag_options->USE_MINI_HALOS) {
    ...
    if (flag_options->USE_LYA_HEATING) {
      ...
      LOG_DEBUG("eps_cont %.2e eps_inj %.2e", eps_lya_cont_ave,
                eps_lya_inj_ave);
    }
  }
