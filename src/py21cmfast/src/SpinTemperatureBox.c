...
void alloc_global_arrays() {
    ...
    // Nonhalo stuff
    if (matter_options_global->SOURCE_MODEL < SOURCE_MODEL_L_INTEGRAL) {
        int num_R_boxes =
...
void fill_freqint_tables(double zp, double x_e_ave, double filling_factor_of_HI_zp,
    ...
    if (matter_options_global->MINIMIZE_MEMORY && matter_options_global->SOURCE_MODEL < SOURCE_MODEL_L_INTEGRAL) {
        R_start = R_mm;
        R_end = R_mm + 1;
    } else {
        R_start = 0;
...
int global_reion_properties(double zp, double x_e_ave, double *log10_Mcrit_LW_ave,
    ...
    if (matter_options_global->USE_INTERPOLATION_TABLES > INTERPOLATION_SIGMA) {
    ...
    *Q_HI = 1 - (ION_EFF_FACTOR * sum_nion + ION_EFF_FACTOR_MINI * sum_nion_mini) / (1.0 - x_e_ave);
    ...
    if (matter_options_global->SOURCE_MODEL >= SOURCE_MODEL_L_INTEGRAL || !matter_options_global->MINIMIZE_MEMORY) {
...
#if LOG_LEVEL >= SUPER_DEBUG_LEVEL
                    if (box_ct == 0 && matter_options_global->SOURCE_MODEL < SOURCE_MODEL_L_INTEGRAL) {
                        double integral_db;
                        if (matter_options_global->SOURCE_MODEL == SOURCE_MODEL_E_INTEGRAL) {
                            integral_db =
