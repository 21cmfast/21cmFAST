 ...
 * @param cosmo_params 
 * @param boxes 
 * @param perturbed_field 
 * @return 
 */
int ComputePerturbField(
    float redshift, UserParams *user_params, CosmoParams *cosmo_params, InitialConditions *boxes, PerturbedField *perturbed_field
){
    /*
...
#pragma omp for
        ...
        if (resampled_box == NULL) {
            LOG_DEBUG("Could not allocate memory for resampled_box.");
        }
        ...
        if (use_cuda) {
            ...
            resampled_box = MapMass_cpu(user_params, cosmo_params, boxes, resampled_box, dimension, f_pixel_factor, init_growth_factor);
        }
