...
void compute_perturbed_velocities(unsigned short axis, UserParams *user_params,
                                  ...
                                  float dDdt_over_D, int dimension,
                                  int switch_mid, float f_pixel_factor,
                                  float *velocity) {
  ...
  if (user_params->PERTURB_ON_HIGH_RES) {
    // We are going to generate the velocity field on the high-resolution
    // perturbed density grid
    memcpy(HIRES_density_perturb, HIRES_density_perturb_saved,
           sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
  } else {
    // We are going to generate the velocity field on the low-resolution
    // perturbed density grid
    memcpy(LOWRES_density_perturb, LOWRES_density_perturb_saved,
           sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    LOG_SUPER_DEBUG(
        "dDdt_over_D=%.6e, dimension=%d, switch_mid=%d, f_pixel_factor=%f",
        dDdt_over_D, dimension, switch_mid, f_pixel_factor);
  }

#pragma omp parallel shared(                                                   \
        LOWRES_density_perturb, HIRES_density_perturb, dDdt_over_D, dimension, \
...
#pragma omp for
    for (n_x = 0; n_x < dimension; n_x++) {
      ...
      for (n_y = 0; n_y < dimension; n_y++) {
        ...
        for (n_z = 0;
             n_z <=
             (unsigned long long)(user_params->NON_CUBIC_FACTOR * switch_mid);
             n_z++) {
          ...
          if ((n_x == 0) && (n_y == 0) && (n_z == 0)) {  // DC mode
            ...
            if (user_params->PERTURB_ON_HIGH_RES) {
              ...
              LOWRES_density_perturb[HII_C_INDEX(n_x, n_y, n_z)] *=
                  dDdt_over_D * kvec[axis] * I / k_sq /
                  (HII_TOT_NUM_PIXELS + 0.0);
            }
          }
        }
      }
  ...
  debugSummarizeBoxComplex((float complex *)LOWRES_density_perturb,
                           user_params->HII_DIM, user_params->HII_DIM,
                           (int)(HII_D_PARA / 2), "  ");
  ...
  if (user_params->PERTURB_ON_HIGH_RES) {
    ...
    if (user_params->DIM != user_params->HII_DIM)
      filter_box(HIRES_density_perturb, 0, 0,
                 L_FACTOR * user_params->BOX_LEN / (user_params->HII_DIM + 0.0),
                 0.);

    dft_c2r_cube(user_params->USE_FFTW_WISDOM, user_params->DIM, D_PARA,
                 user_params->N_THREADS, HIRES_density_perturb);

#pragma omp parallel shared(velocity, HIRES_density_perturb,     \
                                f_pixel_factor) private(i, j, k) \
    num_threads(user_params -> N_THREADS)
    {
#pragma omp for
    ...
    dft_c2r_cube(user_params->USE_FFTW_WISDOM, user_params->HII_DIM, HII_D_PARA,
                 user_params->N_THREADS, LOWRES_density_perturb);
...
#pragma omp for
  ...
  debugSummarizeBox(velocity, user_params->HII_DIM, user_params->HII_DIM,
                    HII_D_PARA, "  ");
}
...
int ComputePerturbField(float redshift, UserParams *user_params,
  ...
  /*
   ComputePerturbField uses the first-order Langragian displacement field to
   move the masses in the cells of the density field. The high-res density field
   is extrapolated to some high-redshift (user_params.INITIAL_REDSHIFT), then
   uses the zeldovich approximation to move the grid "particles" onto the
   lower-res grid we use for the maps. Then we recalculate the velocity fields
  ...
  */
  ...
  Try {  // This Try{} wraps the whole function, so we don't indent.
    ...
    // Variables to perform cloud in cell re-distribution of mass for the
    // perturbed field
    int xp1, yp1, zp1;
    ...
    // perform a very rudimentary check to see if we are underresolved and not
    // using the linear approx
    if ((user_params->BOX_LEN > user_params->DIM) &&
        user_params->PERTURB_ALGORITHM > 0) {
      LOG_WARNING(
    ...
    displacement_factor_2LPT =
        -(3.0 / 7.0) * growth_factor * growth_factor;  // 2LPT eq. D8
    ...
    LOWRES_density_perturb = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
    LOWRES_density_perturb_saved = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);

    if (user_params->PERTURB_ON_HIGH_RES) {
      HIRES_density_perturb = (fftwf_complex *)fftwf_malloc(
          sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
      HIRES_density_perturb_saved = (fftwf_complex *)fftwf_malloc(
          sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
    }
    ...
    LOG_SUPER_DEBUG(
        "growth_factor=%f, displacemet_factor_2LPT=%f, dDdt=%f, "
        "init_growth_factor=%f, init_displacement_factor_2LPT=%f, "
        "mass_factor=%f",
...
#pragma omp parallel shared(                                                 \
        growth_factor, boxes, LOWRES_density_perturb, HIRES_density_perturb, \
            dimension) private(i, j, k) num_threads(user_params -> N_THREADS)
      {
#pragma omp for
...
#pragma omp parallel shared(LOWRES_density_perturb, HIRES_density_perturb, \
...
#pragma omp for
      ...
      velocity_displacement_factor =
          (growth_factor - init_growth_factor) / user_params->BOX_LEN;

      // now add the missing factor of D
#pragma omp parallel shared(boxes, velocity_displacement_factor, \
                                dimension) private(i, j, k)      \
    num_threads(user_params -> N_THREADS)
      {
#pragma omp for
            ...
            for (k = 0; k < dimension_z; k++) {
              if (user_params->PERTURB_ON_HIGH_RES) {
                ...
                boxes->hires_vz[R_INDEX(i, j, k)] *=
                    (velocity_displacement_factor /
                     user_params->NON_CUBIC_FACTOR);  // this is now comoving
      ...
      // *                           BEGIN 2LPT PART * //
      // * ************************************************************************* * //
      // reference: reference: Scoccimarro R., 1998, MNRAS, 299, 1097-1118
      // Appendix D
...
#pragma omp parallel shared(boxes, velocity_displacement_factor_2LPT, \
                                dimension) private(i, j, k)           \
    num_threads(user_params -> N_THREADS)
        {
#pragma omp for
              ...
              for (k = 0; k < dimension_z; k++) {
                if (user_params->PERTURB_ON_HIGH_RES) {
                  boxes->hires_vx_2LPT[R_INDEX(i, j, k)] *=
                      velocity_displacement_factor_2LPT;  // this is now
                                                          // comoving
                                                          // displacement in
                                                          // units of box size
      ...
      // Perturbing the density field required adding over multiple cells. Store
      // intermediate result as a double to avoid rounding errors
      if (user_params->PERTURB_ON_HIGH_RES) {
...
#pragma omp parallel shared(                                                  \
        init_growth_factor, boxes, f_pixel_factor, resampled_box,             \
            ...
            for (k = 0; k < D_PARA; k++) {
              ...
              if (user_params->PERTURB_ALGORITHM == 2) {
                if (user_params->PERTURB_ON_HIGH_RES) {
                  ...
                  xf -=
                      (boxes->lowres_vx_2LPT)[HII_R_INDEX(HII_i, HII_j, HII_k)];
                  yf -=
                      (boxes->lowres_vy_2LPT)[HII_R_INDEX(HII_i, HII_j, HII_k)];
                  zf -=
                      (boxes->lowres_vz_2LPT)[HII_R_INDEX(HII_i, HII_j, HII_k)];
                }
              }
              xf *= (double)(dimension);
              yf *= (double)(dimension);
              ...
              // Determine the fraction of the perturbed cell which overlaps
              // with the 8 nearest grid cells, based on the grid cell which
              // contains the centre of the perturbed cell
              d_x = fabs(xf - (double)(xi + 0.5));
              d_y = fabs(yf - (double)(yi + 0.5));
              d_z = fabs(zf - (double)(zi + 0.5));
              if (xf < (double)(xi + 0.5)) {
                // If perturbed cell centre is less than the mid-point then
                // update fraction of mass in the cell and determine the cell
                // centre of neighbour to be the lowest grid point index
                d_x = 1. - d_x;
                xi -= 1;
                if (xi < 0) {
                  xi += (dimension);
              ...
              // Takes into account the offset based on cell centre determined
              // above
              xp1 = xi + 1;
              if (xp1 >= dimension) {
                xp1 -= (dimension);
              ...
              if (user_params->PERTURB_ON_HIGH_RES) {
                // Redistribute the mass over the 8 neighbouring cells according
                // to cloud in cell
#pragma omp atomic
...
#pragma omp parallel shared(LOWRES_density_perturb, HIRES_density_perturb, \
                                resampled_box, dimension) private(i, j, k) \
    num_threads(user_params -> N_THREADS)
      {
#pragma omp for
        for (i = 0; i < dimension; i++) {
        ...
        for (i = 0; i < dimension; i++) {
          for (j = 0; j < dimension; j++) {
            for (k = 0; k < dimension_z; k++) {
              if (user_params->PERTURB_ON_HIGH_RES) {
                boxes->hires_vx[R_INDEX(i, j, k)] /=
                    velocity_displacement_factor;  // convert back to z = 0
                                                   // quantity
                boxes->hires_vy[R_INDEX(i, j, k)] /=
                    velocity_displacement_factor;  // convert back to z = 0
                                                   // quantity
...
#pragma omp for
          for (i = 0; i < dimension; i++) {
            for (j = 0; j < dimension; j++) {
              for (k = 0; k < dimension_z; k++) {
                if (user_params->PERTURB_ON_HIGH_RES) {
                  boxes->hires_vx_2LPT[R_INDEX(i, j, k)] /=
                      velocity_displacement_factor_2LPT;  // convert back to z =
                                                          // 0 quantity
                  boxes->hires_vy_2LPT[R_INDEX(i, j, k)] /=
                      velocity_displacement_factor_2LPT;  // convert back to z =
                                                          // 0 quantity
                  boxes->hires_vz_2LPT[R_INDEX(i, j, k)] /=
                      (velocity_displacement_factor_2LPT /
                       user_params->NON_CUBIC_FACTOR);  // convert back to z = 0
                                                        // quantity
                } else {
                  boxes->lowres_vx_2LPT[HII_R_INDEX(i, j, k)] /=
                      velocity_displacement_factor_2LPT;  // convert back to z =
    ...
    // Now, if I still have the high resolution density grid
    // (HIRES_density_perturb) I need to downsample it to the low-resolution
    // grid
    if (user_params->PERTURB_ON_HIGH_RES) {
      // Transform to Fourier space to sample (filter) the box
      dft_r2c_cube(user_params->USE_FFTW_WISDOM, user_params->DIM, D_PARA,
                   user_params->N_THREADS, HIRES_density_perturb);

      // Need to save a copy of the high-resolution unfiltered density field for
      // the velocities
      ...
      if (user_params->DIM != user_params->HII_DIM) {
        filter_box(
            HIRES_density_perturb, 0, 0,
            L_FACTOR * user_params->BOX_LEN / (user_params->HII_DIM + 0.0), 0.);
      }
...
#pragma omp parallel shared(HIRES_density_perturb, LOWRES_density_perturb,    \
                                f_pixel_factor, mass_factor) private(i, j, k) \
    num_threads(user_params -> N_THREADS)
      {
#pragma omp for
        for (i = 0; i < user_params->HII_DIM; i++) {
          for (j = 0; j < user_params->HII_DIM; j++) {
            for (k = 0; k < HII_D_PARA; k++) {
              ...
              *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) -=
              ...
              if (*((float *)LOWRES_density_perturb +
                    HII_R_FFT_INDEX(i, j, k)) < -1) {
                *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) =
                    -1. + FRACT_FLOAT_ERR;
              }
            }
      ...
      if (user_params->PERTURB_ALGORITHM > 0) {
#pragma omp parallel shared(LOWRES_density_perturb, mass_factor) private( \
        i, j, k) num_threads(user_params -> N_THREADS)
        {
#pragma omp for
          for (i = 0; i < user_params->HII_DIM; i++) {
            for (j = 0; j < user_params->HII_DIM; j++) {
    ...
    debugSummarizeBox((float *)LOWRES_density_perturb, user_params->HII_DIM,
                      user_params->HII_DIM, 2 * (HII_D_PARA / 2 + 1), "  ");
    ...
    // smooth the field
    if (user_params->PERTURB_ALGORITHM > 0 &&
        user_params->SMOOTH_EVOLVED_DENSITY_FIELD) {
      filter_box(LOWRES_density_perturb, 1, 2,
                 user_params->DENSITY_SMOOTH_RADIUS * user_params->BOX_LEN /
...
#pragma omp parallel shared(LOWRES_density_perturb) private(i, j, k) \
    num_threads(user_params -> N_THREADS) reduction(+ : bad_count)
    {
#pragma omp for
      for (i = 0; i < user_params->HII_DIM; i++) {
        for (j = 0; j < user_params->HII_DIM; j++) {
          for (k = 0; k < HII_D_PARA; k++) {
            *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) /=
            ...
            if (*((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) <
              ...
              if (bad_count < 5)
                LOG_WARNING(
                    "LOWRES_density_perturb is <-1 for index %d %d %d "
                    "(value=%f)",
                    i, j, k,
                    *((float *)LOWRES_density_perturb +
    ...
    if (bad_count >= 5)
      LOG_WARNING("Total number of bad indices for LOW_density_perturb: %d",
                  bad_count);
    LOG_SUPER_DEBUG("LOWRES_density_perturb back in real space (normalized): ");
    debugSummarizeBox((float *)LOWRES_density_perturb, user_params->HII_DIM,
                      user_params->HII_DIM, 2 * (HII_D_PARA / 2 + 1), "  ");

#pragma omp parallel shared(perturbed_field, LOWRES_density_perturb) private( \
        i, j, k) num_threads(user_params -> N_THREADS)
    {
#pragma omp for
      for (i = 0; i < user_params->HII_DIM; i++) {
    ...
    if (user_params->KEEP_3D_VELOCITIES) {
      compute_perturbed_velocities(
          0, user_params, HIRES_density_perturb, HIRES_density_perturb_saved,
          LOWRES_density_perturb, LOWRES_density_perturb_saved, dDdt_over_D,
          dimension, switch_mid, f_pixel_factor, perturbed_field->velocity_x);
      compute_perturbed_velocities(
          1, user_params, HIRES_density_perturb, HIRES_density_perturb_saved,
          LOWRES_density_perturb, LOWRES_density_perturb_saved, dDdt_over_D,
          dimension, switch_mid, f_pixel_factor, perturbed_field->velocity_y);
    }

    compute_perturbed_velocities(
        2, user_params, HIRES_density_perturb, HIRES_density_perturb_saved,
        LOWRES_density_perturb, LOWRES_density_perturb_saved, dDdt_over_D,
        dimension, switch_mid, f_pixel_factor, perturbed_field->velocity_z);
