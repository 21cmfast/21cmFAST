...
/* This file contains integrals and integrands of the halo mass function for the
...
#define SHETH_b_DEXM \
  (0.15)  // Sheth-Tormen b parameter found to match the ST mass function using
          // DexM
#define SHETH_c_DEXM \
  (0.05)  // Sheth-Tormen c parameter found to match the ST mass function using
          // DexM

// For the HMF
#define SHETH_a \
  (0.73)  // Sheth and Tormen a parameter (from Jenkins et al. 2001)
#define SHETH_p \
  (0.175)  // Sheth and Tormen p parameter (from Jenkins et al. 2001)
#define SHETH_A \
  (0.353)  // Sheth and Tormen A parameter (from Jenkins et al. 2001)

// Gauss-Legendre integration constants
#define NGL_INT 100  // 100
// These arrays hold the points and weights for the Gauss-Legendre integration
// routine (JD) Since these were always malloc'd one at a time with fixed length
//~100, I've changed them to fixed-length arrays
...
double sheth_delc_dexm(double del, double sig) {
  return sqrt(SHETH_a) * del *
         (1. +
          SHETH_b_DEXM * pow(sig * sig / (SHETH_a * del * del), SHETH_c_DEXM));
}

/*DexM uses a fit to this barrier to acheive MF similar to ST, Here I use the
 * fixed version for the sampler*/
// NOTE: if I made this a table it would save a pow call per condition in the
// sampler
...
double euler_to_lagrangian_delta(double delta) {
  double dp1 = delta + 1;
  return -1.35 * pow(dp1, -2. / 3.) + 0.78785 * pow(dp1, -0.58661) -
         1.12431 * pow(dp1, -0.5) + 1.68647;
}

////MASS FUNCTIONS BELOW//////
/*
Unconditional Mass function from Delos 2023
(https://arxiv.org/pdf/2311.17986.pdf) Matches well with N-bodies (M200), has a
corresponding Conditional Mass Function (below) and an excursion set method.
Hence can be consistently used throughout the Halo Finder, Halo Sampler And
radiation. The mass functions are based off a constant barrier delta = 1.5 and a
...
double dNdlnM_Delos(double growthf, double lnM) {
  ...
  sigma_inv = 1 / sigma;
  dsigmadm =
      EvaluatedSigmasqdm(lnM) * (0.5 * sigma_inv);  // d(s^2)/dm z0 to dsdm
...
double dNdlnM_conditional_Delos(double growthf, double lnM, double delta_cond,
                                double sigma_cond) {
  ...
  sigdiff_inv =
      sigma == sigma_cond ? 1e6 : 1 / (sigma * sigma - sigma_cond * sigma_cond);
...
// Sheth Tormen 2002 fit for the CMF, while the moving barrier does not allow
// for a simple rescaling, it has been found That a taylor expansion of the
// barrier shape around the point of interest well approximates the simulations
double st_taylor_factor(double sig, double sig_cond, double growthf,
                        double *zeroth_order) {
  int i;
  double a = JENKINS_a;
  double alpha =
      JENKINS_c;  // fixed instead of SHETH_c_DEXM bc of DexM corrections
  ...
  *zeroth_order =
      prefactor_1 *
      (1 + prefactor_2);  // 0th order term gives the barrier for efficiency
  return result;
}

// CMF Corresponding to the Sheth Mo Tormen HMF (Sheth+ 2002)
double dNdM_conditional_ST(double growthf, double lnM, double delta_cond,
                           double sigma_cond) {
  ...
  sigdiff_inv = sigma1 == sigma_cond
                    ? 1e6
                    : 1 / (sigma1 * sigma1 - sigma_cond * sigma_cond);

  result =
      -dsigmasqdm * factor * pow(sigdiff_inv, 1.5) *
...
/*
 ...
 Computes the Press_schechter mass function with Sheth-Torman correction for
 ellipsoidal collapse at redshift z, and dark matter halo mass M (in solar
 masses). Moving barrier B(z,sigma) and sharp-k window functino

 Uses interpolated sigma and dsigmadm to be computed faster. Necessary for
 mass-dependent ionising efficiencies.

 The return value is the number density per unit mass of halos in the mass range
 M to M+dM in units of: comoving Mpc^-3 Msun^-1
...
double dNdlnM_st(double growthf, double lnM) {
  ...
  return -(dsigmadm / sigma) * sqrt(2. / PI) * SHETH_A *
         (1 + pow(nuhat, -2 * SHETH_p)) * nuhat * pow(E, -nuhat * nuhat / 2.0);
}

// Conditional Extended Press-Schechter Mass function, with constant barrier
// delta=1.682 and sharp-k window function
double dNdM_conditional_EPS(double growthf, double lnM, double delta_cond,
                            double sigma_cond) {
  ...
  return -del * dsigmasqdm * pow(sigdiff_inv, 1.5) *
         exp(-del * del * 0.5 * sigdiff_inv) / sqrt(2. * PI);
}

/*
 FUNCTION dNdM(growthf, M)
 Computes the Press_schechter mass function at
 redshift z (using the growth factor), and dark matter halo mass M (in solar
 masses).
...
/*
 ...
 Computes the Press_schechter mass function with Warren et al. 2011 correction
 for ellipsoidal collapse at redshift z, and dark matter halo mass M (in solar
 masses).
...
/*
 ...
 The Universial FOF function, with redshift evolution (Eq. 12 - 15) of Watson et
 al. 2013.
...
double dNdlnM_WatsonFOF_z(double z, double growthf, double lnM) {
  ...
  alpha_z = Omega_m_z * (Watson_alpha_z_1 * pow(1. + z, Watson_alpha_z_2) +
                         Watson_alpha_z_3);
  beta_z = Omega_m_z *
           (Watson_beta_z_1 * pow(1. + z, Watson_beta_z_2) + Watson_beta_z_3);

  f_sigma = A_z * (pow(beta_z / sigma, alpha_z) + 1.) *
            exp(-Watson_gamma_z / (sigma * sigma));
...
double nion_fraction(double lnM, void *param_struct) {
  struct parameters_gsl_MF_integrals p =
      *(struct parameters_gsl_MF_integrals *)param_struct;
  double Fstar = log_scaling_PL_limit(lnM, p.f_star_norm, p.alpha_star,
                                      10 * LN10, p.Mlim_star);
  double Fesc = log_scaling_PL_limit(lnM, p.f_esc_norm, p.alpha_esc, 10 * LN10,
                                     p.Mlim_esc);
...
double nion_fraction_mini(double lnM, void *param_struct) {
  ...
  double Fstar = log_scaling_PL_limit(lnM, p.f_star_norm, p.alpha_star,
                                      7 * LN10, p.Mlim_star);
  double Fesc = log_scaling_PL_limit(lnM, p.f_esc_norm, p.alpha_esc, 7 * LN10,
                                     p.Mlim_esc);
  double M = exp(lnM);
...
// Due to the log(1+Mstar) in the metallicity, this is hard to simplify into
// log-space
//   As a result this will be slower, and is currently used only for integrals
//   in HaloBox below the sampler mass.
double xray_fraction_doublePL(double lnM, void *param_struct) {
  struct parameters_gsl_MF_integrals p =
      *(struct parameters_gsl_MF_integrals *)param_struct;
  double M = exp(lnM);
  double Fstar = exp(log_scaling_PL_limit(lnM, p.f_star_norm, p.alpha_star,
                                          10 * LN10, p.Mlim_star) -
  ...
  if (flag_options_global->USE_MINI_HALOS)
    Fstar_mini = exp(log_scaling_PL_limit(lnM, p.f_esc_norm, p.alpha_esc,
                                          7 * LN10, p.Mlim_esc) -
                     p.Mturn_mcg / M - M / p.Mturn_upper + p.f_esc_norm);

  double stars =
      M * Fstar * cosmo_params_global->OMb / cosmo_params_global->OMm;
  double stars_mini =
      M * Fstar_mini * cosmo_params_global->OMb / cosmo_params_global->OMm;
  double sfr = stars / (p.t_star * p.t_h);
  double sfr_mini = stars_mini / (p.t_star * p.t_h);

  double metallicity;
  get_halo_metallicity(sfr + sfr_mini, stars + stars_mini, p.redshift,
                       &metallicity);
...
double conditional_mf(double growthf, double lnM, double delta, double sigma,
                      int HMF) {
  ...
  // NOTE: Normalisation scaling is currently applied outside the integral, per
  // condition This will be the rescaled EPS CMF,
  return dNdM_conditional_EPS(growthf, lnM, delta, sigma);
}
...
double c_nion_integrand_mini(double lnM, void *param_struct) {
  return nion_fraction_mini(lnM, param_struct) *
         c_mf_integrand(lnM, param_struct);
}

double c_xray_integrand(double lnM, void *param_struct) {
  return xray_fraction_doublePL(lnM, param_struct) *
         c_mf_integrand(lnM, param_struct);
}
...
double unconditional_mf(double growthf, double lnM, double z, int HMF) {
  ...
  // NOTE: HMF > 4 or < 0 gets caught earlier, so unless some strange change is
  // made this is fine
  if (HMF == 0) {
...
double u_nion_integrand_mini(double lnM, void *param_struct) {
  return nion_fraction_mini(lnM, param_struct) *
         u_mf_integrand(lnM, param_struct);
}
...
// In future all MF integrals will go through here, simply selecting the
// integrand function from a switch
double IntegratedNdM_QAG(double lnM_lo, double lnM_hi,
                         struct parameters_gsl_MF_integrals params,
  ...
  if (status != 0) {
    ...
    LOG_ERROR(
        "(function argument): lower_limit=%.3e (%.3e) upper_limit=%.3e (%.3e) "
        "rel_tol=%.3e result=%.3e error=%.3e",
        lower_limit, exp(lower_limit), upper_limit, exp(upper_limit), rel_tol,
        result, error);
    LOG_ERROR("data: z=%.3e growthf=%.3e  HMF=%d ", params.redshift,
              params.growthf, params.HMF);
    LOG_ERROR("sigma=%.3e delta=%.3e", params.sigma_cond, params.delta);
    LOG_ERROR("Mturn_acg=%.3e f*=%.3e a*=%.3e Mlim*=%.3e", params.Mturn_acg,
              params.f_star_norm, params.alpha_star, params.Mlim_star);
    LOG_ERROR("f_escn=%.3e a_esc=%.3e Mlim_esc=%.3e", params.f_esc_norm,
              params.alpha_esc, params.Mlim_esc);
    LOG_ERROR("t_*=%.3e t_h=%.3e lx=%.3e lxmini %.3e", params.t_star,
              params.t_h, params.l_x_norm, params.l_x_norm_mini);
...
// calculates the weightings and the positions for any Gauss-Legendre
// quadrature.
void gauleg(double x1, double x2, double x[], double w[], int n)
// Given the lower and upper limits of integration x1 and x2, and given n, this
// routine returns arrays x[1..n] and w[1..n] of length n, containing the
// abscissas and weights of the Gauss- Legendre n-point quadrature formula.
{
  ...
  for (i = 1; i <= m; i++) {
    ...
    // them. Loop over the desired roots.
...
double IntegratedNdM_GL(double lnM_lo, double lnM_hi,
                        struct parameters_gsl_MF_integrals params,
                        double (*integrand)(double, void *)) {
  int i;
  double integral = 0;
  // compare to lower precision?
  if ((float)lnM_lo != (float)GL_limit[0] ||
      (float)lnM_hi != (float)GL_limit[1]) {
    LOG_ERROR(
        "Integral limits [%.8e %.8e] do not match Gauss Legendre limits [%.8e "
        "%.8e]!",
...
// JBM: Integral of a power-law times exponential for EPS: \int dnu nu^beta *
// exp(-nu/2)/sqrt(nu) from numin to infty.
...
// This takes into account the last approximation in Munoz+22, where erfc
// (beta=0) is used NOTE: even though nu_condition is defined in the
// unconditional (no sigma_cond), here it
...
// This routine assumes sharp cutoffs for each turnover rather than exponential,
// assumes a triple power-law form for sigma(M)
//   and takes advantage of the fact that Gamma_inc(x,min) = integral_min^inf
//   (t^(x-1)exp(-t)) dt which is satisfied for the HMF when the above
//   approximations are made
// Originally written by JBM within the GL integration before it was separated
// here and generalised to the other integrals
double MFIntegral_Approx(double lnM_lo, double lnM_hi,
                         struct parameters_gsl_MF_integrals params) {
  ...
  if (params.HMF != 0) {
    ...
    LOG_ERROR(
        "Ensure parameter input specifically to this function has HMF==0");
    Throw(TableGenerationError);
  }
  if (abs(params.gamma_type) > 4) {
    LOG_ERROR(
        "Approximate Fcoll only works for single power-law scaling relations");
    LOG_ERROR(
        "These include the following General/Conditional integration "
        "functions");
    LOG_ERROR("Nhalo, Fcoll, Nion, Nion_MINI");
    LOG_ERROR(
        "Something has gone wrong in the backend such that the 'Gamma-Approx'");
  ...
  double lnMturn_l = abs(params.gamma_type < 4) ? log(params.Mturn_acg)
                                                : log(params.Mturn_mcg);
  ...
  // The below limit setting is done simply so that variables which do not
  // conern particular integrals
  //       can be left undefined, rather than explicitly set to some value (0 or
  //       1e20)
  ...
  double beta1 = index_base * AINDEX1 *
                 0.5;  // exponent for Fcollapprox for nu>nupivot1 (large M)
  double beta2 =
      index_base * AINDEX2 *
      0.5;  // exponent for Fcollapprox for nupivot2<nu<nupivot1 (small M)
  double beta3 = index_base * AINDEX3 *
                 0.5;  // exponent for Fcollapprox for nu<nupivot2 (smallest M)

  // There are 5 nu(M) points of interest: the two power-law pivot points, the
  // lower and upper integral limits and the condition.
  // NOTE: Since sigma(M) is approximated as a power law, not (sigma(M)^2 -
  // sigma_cond^2), this is not a simple gamma function.
  //  note especially which nu subtracts the condition sigma and not, see
  //  Appendix B of Munoz+22 (2110.13919)
  ...
  // These nu use the CMF delta (subtracted the condition delta), but not the
  // condition sigma
  ...
  double nu_pivot1 =
      delta_arg / (sigma_pivot1 * sigma_pivot1 - sigma_c * sigma_c);
  double nu_pivot2 =
      delta_arg / (sigma_pivot2 * sigma_pivot2 - sigma_c * sigma_c);

  // These nu subtract the condition sigma as in the CMF
  double nu_lo_limit =
      delta_arg / (sigma_lo_limit * sigma_lo_limit - sigma_c * sigma_c);
  double nu_hi_limit =
      delta_arg / (sigma_hi_limit * sigma_hi_limit - sigma_c * sigma_c);

  double fcoll = 0.;

  // NOTES: For speed the minihalos ignore the condition mass limit (assuming
  // nu_hi_limit(tilde) < nu_condition (no tilde))
  //     and never get into the high mass power law (nu_hi_limit < nu_pivot1
  //     (both tilde))
  // ACGs ignore the upper mass limit (no upper turnover), both assume the
  // condition is above the highest pivot
  if (abs(params.gamma_type) == 4) {
    // re-written for further speedups
    if (nu_hi_limit <=
        nu_pivot2) {  // if both are below pivot2 don't bother adding and
      ...
      if (nu_lo_limit > nu_pivot2) {
        ...
        fcoll +=
            (Fcollapprox(nu_lo_limit, beta3) - Fcollapprox(nu_pivot2, beta3)) *
            pow(nu_pivot2_umf, -beta3);
      }
    }
  } else {
    if (nu_lo_limit >= nu_condition) {  // fully in the flat part of sigma(nu),
                                        // M^alpha is nu-independent.
      // This is just an erfc, remembering that the conditional nu can be higher
      // than the unconditional nu of the condition
      return Fcollapprox(nu_lo_limit, 0.);
    }

    if (nu_lo_limit >= nu_pivot1) {
      // We use the condition version wherever the nu range may intersect
      // nu_condition (i.e beta1)
      fcoll += Fcollapprox_condition(nu_lo_limit, nu_condition, beta1) *
               pow(nu_pivot1_umf, -beta1);
    } else {
      fcoll += Fcollapprox_condition(nu_pivot1, nu_condition, beta1) *
               pow(nu_pivot1_umf, -beta1);
      if (nu_lo_limit > nu_pivot2) {
        fcoll +=
            (Fcollapprox(nu_lo_limit, beta2) - Fcollapprox(nu_pivot1, beta2)) *
            pow(nu_pivot1_umf, -beta2);
      } else {
        fcoll +=
            (Fcollapprox(nu_pivot2, beta2) - Fcollapprox(nu_pivot1, beta2)) *
            pow(nu_pivot1_umf, -beta2);
        fcoll +=
            (Fcollapprox(nu_lo_limit, beta3) - Fcollapprox(nu_pivot2, beta3)) *
            pow(nu_pivot2_umf, -beta3);
...
double IntegratedNdM(double lnM_lo, double lnM_hi,
                     struct parameters_gsl_MF_integrals params,
...
double Fcoll_General(double z, double lnM_min, double lnM_max) {
  ...
  return IntegratedNdM(lnM_min, lnM_max, integral_params, &u_fcoll_integrand,
                       0);
}

double Nion_General(double z, double lnM_Min, double lnM_Max,
                    double MassTurnover, struct ScalingConstants *sc) {
  struct parameters_gsl_MF_integrals params = {
      .redshift = z,
      .growthf = dicke(z),
...
double Nion_General_MINI(double z, double lnM_Min, double lnM_Max,
                         double MassTurnover, struct ScalingConstants *sc) {
  struct parameters_gsl_MF_integrals params = {
      .redshift = z,
      .growthf = dicke(z),
...
double Xray_General(double z, double lnM_Min, double lnM_Max, double mturn_acg,
                    double mturn_mcg, struct ScalingConstants *sc) {
  // NOTE:in the _General functions, we don't use the scaling relation constants
  //  that are z-dependent so we can evaluate them at multiple redshifts without
  //  redoing the constants
  struct parameters_gsl_MF_integrals params = {
      .redshift = z,
      .growthf = dicke(z),
      .Mturn_acg = mturn_acg,
...
double Nhalo_Conditional(double growthf, double lnM1, double lnM2,
                         double lnM_cond, double sigma, double delta,
                         int method) {
  struct parameters_gsl_MF_integrals params = {
      .growthf = growthf,
      .HMF = user_params_global->HMF,
  ...
  // NOTE: The delta <=-1 limit has been removed in all hmf integrals as it
  // created a discontinuity
  ...
  if (delta > MAX_DELTAC_FRAC * get_delta_crit(params.HMF, sigma, growthf)) {
    if (lnM_cond * (1 - FRACT_FLOAT_ERR) <=
        lnM2)  // this limit is not ideal, but covers floating point errors when
               // we set lnM2 == log(M_cond)
...
double Mcoll_Conditional(double growthf, double lnM1, double lnM2,
                         double lnM_cond, double sigma, double delta,
                         int method) {
  struct parameters_gsl_MF_integrals params = {
      .growthf = growthf,
...
double Nion_ConditionalM_MINI(double growthf, double lnM1, double lnM2,
  ...
  if (delta2 > MAX_DELTAC_FRAC * get_delta_crit(params.HMF, sigma2, growthf)) {
    if (lnM_cond * (1 - FRACT_FLOAT_ERR) <=
      ...
      return nion_fraction_mini(lnM_cond, &params) /
             exp(lnM_cond);  // NOTE: condition mass is used as if it were
                             // Lagrangian (no 1+delta)
    else
      return 0.;
  }
...
double Nion_ConditionalM(double growthf, double lnM1, double lnM2,
                         double lnM_cond, double sigma2, double delta2,
                         double MassTurnover, struct ScalingConstants *sc,
                         int method) {
  struct parameters_gsl_MF_integrals params = {
  ...
  if (lnM1 >= lnM_cond) return 0.;
  // return 1 halo at the condition mass if delta is exceeded and the condition
  // is within the integral limits
  if (delta2 > MAX_DELTAC_FRAC * get_delta_crit(params.HMF, sigma2, growthf)) {
...
double Xray_ConditionalM(double redshift, double growthf, double lnM1,
                         double lnM2, double lnM_cond, double sigma2,
                         double delta2, double mturn_acg, double mturn_mcg,
...
double splined_erfc(double x) {
  ...
  // The Actual ERFC spline was removed a while ago, if we remake it it should
  // be
...
/*
 calculates the fraction of mass contained in haloes with mass > M at redshift
 z, in regions with a linear overdensity of del_bias, and standard deviation
 sig_bias
*/

// I wrote a version of FgtrM which takes the growth func instead of z for a bit
// of speed
double FgtrM_bias_fast(float growthf, float del_bias, float sig_small,
                       float sig_large) {
  double del, sig;
  ...
  // sometimes they are the same to float precision, where the M_condition ~
  // M_Min
  ...
  // NOTE: should we allow del < 0??? We would need to change dfcolldz to
  // prevent zero dfcoll
...
/* compute a mass limit where the stellar baryon fraction and the escape
 * fraction exceed unity */
// NOTE (JD): Why aren't we using 1e10 * pow(FRAC,-1/PL)? what am I missing here
// that makes the rootfind necessary
...
float Mass_limit_bisection(float Mmin, float Mmax, float PL, float FRAC) {
  ...
  do {
    if ((Mass_limit(logMlow, PL, FRAC) - 1.) * (Mass_limit(x, PL, FRAC) - 1.) <
        0.)
      logMupper = x;
    else
      logMlow = x;
  ...
  LOG_ERROR(
      "Failed to find a mass limit to regulate stellar fraction/escape "
...
// set the minimum source mass for the integrals, If we have an exponential
// cutoff we go below the chosen mass by a factor of 50 NOTE: previously, with
// USE_MINI_HALOS, the sigma table was initialised with M_MIN_INTEGRAL/50, but
// then all integrals perofmed
//       from M_MIN_INTEGRAL
double minimum_source_mass(double redshift, bool xray,
  ...
  // automatically true if USE_MASS_DEPENDENT_ZETA
    ...
    // NOTE: previously this divided Mturn by 50 in spin temperature, but not in
    // the ionised box
    //      which I think is a bug with M_MIN_in_Mass, since there is a sharp
    //      cutoff
    Mmin = astro_params->M_TURN;
  } else {
    // if the virial temp minimum is set below ionisation we need to set mu
    // accordingly
    t_vir_min =
        xray ? astro_params->X_RAY_Tvir_MIN : astro_params->ION_Tvir_MIN;
