...
void adj_complex_conj(fftwf_complex *HIRES_box, UserParams *user_params,
  ...
  HIRES_box[C_INDEX(0, 0, MIDDLE_PARA)] =
      crealf(HIRES_box[C_INDEX(0, 0, MIDDLE_PARA)]);
  HIRES_box[C_INDEX(0, MIDDLE, 0)] = crealf(HIRES_box[C_INDEX(0, MIDDLE, 0)]);
  HIRES_box[C_INDEX(0, MIDDLE, MIDDLE_PARA)] =
      crealf(HIRES_box[C_INDEX(0, MIDDLE, MIDDLE_PARA)]);
  HIRES_box[C_INDEX(MIDDLE, 0, 0)] = crealf(HIRES_box[C_INDEX(MIDDLE, 0, 0)]);
  HIRES_box[C_INDEX(MIDDLE, 0, MIDDLE_PARA)] =
      crealf(HIRES_box[C_INDEX(MIDDLE, 0, MIDDLE_PARA)]);
  HIRES_box[C_INDEX(MIDDLE, MIDDLE, 0)] =
      crealf(HIRES_box[C_INDEX(MIDDLE, MIDDLE, 0)]);
      ...
      for (j = 0; j <= MIDDLE; j += MIDDLE) {
        for (k = 0; k <= MIDDLE_PARA; k += MIDDLE_PARA) {
          HIRES_box[C_INDEX(i, j, k)] =
              conjf(HIRES_box[C_INDEX((user_params->DIM) - i, j, k)]);
        }
      }
      ...
      for (j = 1; j < MIDDLE; j++) {
        for (k = 0; k <= MIDDLE_PARA; k += MIDDLE_PARA) {
          HIRES_box[C_INDEX(i, j, k)] = conjf(HIRES_box[C_INDEX(
              (user_params->DIM) - i, (user_params->DIM) - j, k)]);
          HIRES_box[C_INDEX(i, (user_params->DIM) - j, k)] =
              conjf(HIRES_box[C_INDEX((user_params->DIM) - i, j, k)]);
        }
      }
    }  // end loop over i
...
#pragma omp for
    for (i = 0; i <= MIDDLE; i += MIDDLE) {
      for (j = 1; j < MIDDLE; j++) {
        for (k = 0; k <= MIDDLE_PARA; k += MIDDLE_PARA) {
          HIRES_box[C_INDEX(i, j, k)] =
              conjf(HIRES_box[C_INDEX(i, (user_params->DIM) - j, k)]);
        }
      }
...
int ComputeInitialConditions(unsigned long long random_seed,
  ...
  //     (user_params->DIM^3) as well as the equal or lower resolution velocity
  //     fields, and smoothed density field (user_params->HII_DIM^3).
  //
  ...
  Try {  // This Try wraps the entire function so we don't indent.
    ...
    fftwf_complex *HIRES_box = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
    fftwf_complex *HIRES_box_saved = (fftwf_complex *)fftwf_malloc(
        sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
...
#pragma omp parallel shared(HIRES_box, r) private(           \
        n_x, n_y, n_z, k_x, k_y, k_z, k_mag, p, a, b, p_vcb) \
    num_threads(user_params -> N_THREADS)
    {
...
#pragma omp for
      for (n_x = 0; n_x < user_params->DIM; n_x++) {
        ...
        // = (2*pi/L) * n
        if (n_x > MIDDLE)
          k_x = (n_x - user_params->DIM) *
                DELTA_K;  // wrap around for FFT convention
        else
          k_x = n_x * DELTA_K;

        for (n_y = 0; n_y < user_params->DIM; n_y++) {
          // convert index to numerical value for this component of the k-mode:
          // k = (2*pi/L) * n
          if (n_y > MIDDLE)
          ...
          for (n_z = 0; n_z <= MIDDLE_PARA; n_z++) {
            // convert index to numerical value for this component of the
            // k-mode: k = (2*pi/L) * n
            k_z = n_z * DELTA_K_PARA;

            // now get the power spectrum; remember, only the magnitude of k
            // counts (due to issotropy) this could be used to speed-up later
            // maybe
    ...
    memcpy(HIRES_box_saved, HIRES_box,
           sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);

    // FFT back to real space
    int stat = dft_c2r_cube(user_params->USE_FFTW_WISDOM, user_params->DIM,
                            D_PARA, user_params->N_THREADS, HIRES_box);
...
#pragma omp for
    ...
    // *** If required, let's also create a lower-resolution version of the
    // density field  *** //
    memcpy(HIRES_box, HIRES_box_saved,
           sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
    ...
    if (!user_params->PERTURB_ON_HIGH_RES) {
      ...
      dft_c2r_cube(user_params->USE_FFTW_WISDOM, user_params->DIM, D_PARA,
                   user_params->N_THREADS, HIRES_box);

      // Renormalise the FFT'd box (sample the high-res box if we are perturbing
      // on the low-res grid)
...
#pragma omp for
    ...
    if (user_params->USE_RELATIVE_VELOCITIES) {
      ...
      for (ii = 0; ii < 3; ii++) {
        memcpy(HIRES_box, HIRES_box_saved,
               sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
#pragma omp parallel shared(HIRES_box, ii) private(n_x, n_y, n_z, k_x, k_y,  \
                                                       k_z, k_mag, p, p_vcb) \
    num_threads(user_params -> N_THREADS)
        {
#pragma omp for
...
#pragma omp parallel shared(boxes, HIRES_box, f_pixel_factor, ii) private( \
        i, j, k, vcb_i) num_threads(user_params -> N_THREADS)
        {
#pragma omp for
          for (i = 0; i < user_params->HII_DIM; i++) {
            for (j = 0; j < user_params->HII_DIM; j++) {
              for (k = 0; k < HII_D_PARA; k++) {
                vcb_i = *((float *)HIRES_box +
                          R_FFT_INDEX(
                              (unsigned long long)(i * f_pixel_factor + 0.5),
                              (unsigned long long)(j * f_pixel_factor + 0.5),
                              (unsigned long long)(k * f_pixel_factor + 0.5)));
...
#pragma omp parallel shared(boxes, HIRES_box, f_pixel_factor, ii, \
...
#pragma omp for
        for (i = 0; i < dimension; i++) {
          for (j = 0; j < dimension; j++) {
            for (k = 0; k < (unsigned long long)(user_params->NON_CUBIC_FACTOR *
                                                 dimension);
                 k++) {
              if (user_params->PERTURB_ON_HIGH_RES) {
                if (ii == 0) {
    ...
    // Generation of the second order Lagrangian perturbation theory (2LPT)
    // corrections to the ZA reference: Scoccimarro R., 1998, MNRAS, 299,
    // 1097-1118 Appendix D
...
#define PHI_INDEX(i, j) ((int)((i) - (j)) + 3 * ((j)) - ((int)(j)) / 2)
      ...
      fftwf_complex *phi_1 = (fftwf_complex *)fftwf_malloc(
          sizeof(fftwf_complex) * KSPACE_NUM_PIXELS);
...
#pragma omp for
      ...
      // We'll also save these temporarily to the hires_vi_2LPT boxes which will
      // get overwritten later with the correct 2LPT velocities
      for (phi_component = 0; phi_component < 3; phi_component++) {
        i = j = phi_component;

        // generate the phi_1 boxes in Fourier transform
#pragma omp parallel shared(HIRES_box, phi_1, i, j) private( \
        n_x, n_y, n_z, k_x, k_y, k_z, k_sq, k)               \
    num_threads(user_params -> N_THREADS)
        {
#pragma omp for
          for (n_x = 0; n_x < user_params->DIM; n_x++) {
              ...
              for (n_z = 0; n_z <= MIDDLE_PARA; n_z++) {
                ...
                if ((n_x == 0) && (n_y == 0) && (n_z == 0)) {  // DC mode
                  ...
                  phi_1[C_INDEX(n_x, n_y, n_z)] =
                      ...
                      k_sq / VOLUME;
                  // note the last factor of 1/VOLUME accounts for the scaling
                  // in real-space, following the FFT
                }
              }
            }
          }
...
#pragma omp for
      ...
      for (phi_component = 0; phi_component < 3; phi_component++) {
        // Now calculate the cross components and start evaluating the 2LPT
        // field
        i = phi_directions[phi_component][0];
        j = phi_directions[phi_component][1];

        // generate the phi_1 boxes in Fourier transform
#pragma omp parallel shared(HIRES_box, phi_1) private(n_x, n_y, n_z, k_x, k_y, \
                                                          k_z, k_sq, k)        \
    num_threads(user_params -> N_THREADS)
        {
#pragma omp for
        ...
        // After that we have to return in Fourier space and generate the
        // Fourier transform of phi_2
...
#pragma omp for
          for (i = 0; i < user_params->DIM; i++) {
            for (j = 0; j < user_params->DIM; j++) {
              for (k = 0; k < D_PARA; k++) {
                // Note, I have temporarily stored the components into other
                // arrays to minimise memory usage phi - {0, 1, 2} ->
                // {hires_vx_2LPT, hires_vy_2LPT, hires_vz_2LPT} This may be
                // opaque to the user, but this shouldn't need modification
                if (phi_component == 0) {
                ...
                // Kept in this form to maintain similar (possible) rounding
                // errors
                *((float *)HIRES_box + R_FFT_INDEX((unsigned long long)(i),
                                                   (unsigned long long)(j),
                                                   (unsigned long long)(k))) +=
...
#pragma omp for
      ...
      // For each component, we generate the velocity field (same as the ZA
      // part)
...
#pragma omp for
          for (i = 0; i < dimension; i++) {
            for (j = 0; j < dimension; j++) {
              for (k = 0;
                   k < (unsigned long long)(user_params->NON_CUBIC_FACTOR *
                                            dimension);
                   k++) {
                if (user_params->PERTURB_ON_HIGH_RES) {
                  if (ii == 0) {
