...
int ComputeInitialConditions(unsigned long long random_seed, InitialConditions *boxes) {
    ...
    if (use_cuda) {
        LOG_DEBUG("GPU InitialConditions: falling back to CPU (USE_RELATIVE_VELOCITIES=%d)",
...
#pragma omp for
        ...
        // Diagnostic output - compute stats of output arrays
        {
            ...
            fprintf(stderr, "[DIAG] IC_CPU hires_density mean=%.6e std=%.6e min=%.6e max=%.6e\n", mean, std, vmin, vmax);
            ...
            mean = sum / n; std = sqrt(sum2 / n - mean * mean);
            fprintf(stderr, "[DIAG] IC_CPU lowres_density mean=%.6e std=%.6e min=%.6e max=%.6e\n", mean, std, vmin, vmax);
            ...
            mean = sum / n; std = sqrt(sum2 / n - mean * mean);
            fprintf(stderr, "[DIAG] IC_CPU lowres_vx mean=%.6e std=%.6e min=%.6e max=%.6e\n", mean, std, vmin, vmax);
            ...
            mean = sum / n; std = sqrt(sum2 / n - mean * mean);
            fprintf(stderr, "[DIAG] IC_CPU lowres_vy mean=%.6e std=%.6e min=%.6e max=%.6e\n", mean, std, vmin, vmax);
            ...
            mean = sum / n; std = sqrt(sum2 / n - mean * mean);
            fprintf(stderr, "[DIAG] IC_CPU lowres_vz mean=%.6e std=%.6e min=%.6e max=%.6e\n", mean, std, vmin, vmax);
        }
    }  // End of Try{}
