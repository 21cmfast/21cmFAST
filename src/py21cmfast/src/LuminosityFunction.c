...
int initialise_ComputeLF(int nbins, UserParams *user_params,
...
int ComputeLF(int nbins, UserParams *user_params, CosmoParams *cosmo_params,
  ...
  */
  ...
  Try {  // This try block covers the whole function.
    ...
    // user_params, cosmo_params etc. can change on each call, freeing up the
    // memory.
    initialise_ComputeLF(nbins, user_params, cosmo_params, astro_params,
                         flag_options);
    ...
    if (astro_params->ALPHA_STAR < -0.5)
      LOG_WARNING(
          "ALPHA_STAR is %f, which is unphysical value given the observational "
          "LFs.\n"
          "Also, when ALPHA_STAR < -.5, LFs may show a kink. It is recommended "
          "to set ALPHA_STAR > -0.5.",
    ...
    for (i_z = 0; i_z < NUM_OF_REDSHIFT_FOR_LF; i_z++) {
      ...
      for (i = 0; i < nbins; i++) {
        ...
        if (component == 1)
          Fstar = astro_params->F_STAR10 *
                  pow(Mhalo_i / 1e10, astro_params->ALPHA_STAR);
        else
          Fstar = astro_params->F_STAR7_MINI *
                  pow(Mhalo_i / 1e7, astro_params->ALPHA_STAR_MINI);
        if (Fstar > 1.) Fstar = 1;

        if (i_unity <
            0) {  // Find the array number at which Fstar crosses unity.
        ...
        SFRparam = Mhalo_i * cosmo_params->OMb / cosmo_params->OMm *
                   (double)Fstar *
                   (double)(hubble(z_LF[i_z]) * SperYR /
                            astro_params->t_STAR);  // units of M_solar/year

        Muv_param[i] =
            51.63 - 2.5 * log10(SFRparam * Luv_over_SFR);  // UV magnitude
        // except if Muv value is nan or inf, but avoid error put the value
        // as 10.
      ...
      // There is a kink on LFs at which Fstar crosses unity. This kink is a
      // numerical artefact caused by the derivate of dMuvdMhalo. Most of the
      // cases the kink doesn't appear in magnitude ranges we are interested
      // (e.g. -22 < Muv < -10). However, for some extreme parameters, it
      // appears. To avoid this kink, we use the interpolation of the derivate
      // in the range where the kink appears. 'i_unity' is the array number at
      // which the kink appears. 'i_unity-3' and 'i_unity+12' are related to the
      // range of interpolation, which is an arbitrary choice. NOTE: This method
      // does NOT work in cases with ALPHA_STAR < -0.5. But, this parameter
      // range is unphysical given that the
      //       observational LFs favour positive ALPHA_STAR in this model.
      // i_smth = 0: calculates LFs without interpolation.
      // i_smth = 1: calculates LFs using interpolation where Fstar crosses
      // unity.
      if (i_unity - 3 < 0)
        i_smth = 0;
      ...
      if (i_smth == 0) {
        for (i = 0; i < nbins; i++) {
          ...
          Muv_1 = gsl_spline_eval(LF_spline, lnMhalo_i - delta_lnMhalo,
                                  LF_spline_acc);
          Muv_2 = gsl_spline_eval(LF_spline, lnMhalo_i + delta_lnMhalo,
                                  LF_spline_acc);
          ...
          log10phi[i + i_z * nbins] = log10(
              unconditional_mf(growthf, lnMhalo_i, z_LF[i_z], mf) /
              Mhalo_param[i] * exp(-(M_TURNs[i_z] / Mhalo_param[i])) *
              (cosmo_params->OMm * RHOcrit) * f_duty_upper / fabs(dMuvdMhalo));

          if (isinf(log10phi[i + i_z * nbins]) ||
              isnan(log10phi[i + i_z * nbins]) ||
        ...
        // generate interpolation arrays to smooth discontinuity of the
        // derivative causing a kink Note that the number of array elements and
        // the range of interpolation are made by arbitrary choices.
        ...
        for (i = 0; i < nbins; i++) {
          ...
          dndm = unconditional_mf(growthf, log(Mhalo_param[i]), z_LF[i_z], mf) *
                 (cosmo_params->OMm * RHOcrit) / Mhalo_param[i];
          log10phi[i + i_z * nbins] =
              log10(dndm * exp(-(M_TURNs[i_z] / Mhalo_param[i])) *
