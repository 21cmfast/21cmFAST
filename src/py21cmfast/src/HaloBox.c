/* This file contains fucntions for calculating the HaloBox output for 21cmfast,
 * containing the gridded source properties, either from integrating the
 * conditional mass functions in a cell or from the halo sampler */
#include "HaloBox.h"
...
// The outputs include all sampled halo properties PLUS all properties which
// cannot be recovered when mixing all the halos together
//   i.e escape fraction weighting, minihalo stuff that has separate parameters
// Since there are so many spectral terms in the spin temperature calculation,
// it will be most efficient to split SFR into regular and minihalos
//   BUT not split the ionisedbox fields.
// in order to remain consistent with the minihalo treatment in default (Nion_a
// * exp(-M/M_a) + Nion_m * exp(-M/M_m - M_a/M))
//   we treat the minihalos as a shift in the mean, where each halo will have
//   both components, representing a smooth transition in halo mass from one set
//   of SFR/emmissivity parameters to the other.
void set_halo_properties(double halo_mass, double M_turn_a, double M_turn_m,
  ...
  get_halo_stellarmass(halo_mass, M_turn_a, M_turn_m, input_rng[0], consts,
                       &stellar_mass, &stellar_mass_mini);

  double sfr, sfr_mini;
  get_halo_sfr(stellar_mass, stellar_mass_mini, input_rng[1], consts, &sfr,
               &sfr_mini);

  double metallicity = 0;
  double xray_lum = 0;
  if (flag_options_global->USE_TS_FLUCT) {
    get_halo_metallicity(sfr + sfr_mini, stellar_mass + stellar_mass_mini,
                         consts->redshift, &metallicity);
  ...
  if (flag_options_global->USE_MINI_HALOS)
    fesc_mini =
        fmin(consts->fesc_7 * pow(halo_mass / 1e7, consts->alpha_esc), 1);

  n_ion_sample = stellar_mass * consts->pop2_ion * fesc +
                 stellar_mass_mini * consts->pop3_ion * fesc_mini;
  wsfr_sample =
      sfr * consts->pop2_ion * fesc + sfr_mini * consts->pop3_ion * fesc_mini;
...
int get_box_averages(double M_min, double M_max, double M_turn_a,
                     double M_turn_m, struct ScalingConstants *consts,
                     struct HaloProperties *averages_out) {
  LOG_SUPER_DEBUG("Getting Box averages z=%.2f M [%.2e %.2e] Mt [%.2e %.2e]",
                  consts->redshift, M_min, M_max, M_turn_a, M_turn_m);
  double t_h = consts->t_h;
  double lnMmax = log(M_max);
  double lnMmin = log(M_min);

  double prefactor_mass = RHOcrit * cosmo_params_global->OMm;
  double prefactor_stars =
      RHOcrit * cosmo_params_global->OMb * consts->fstar_10;
  double prefactor_stars_mini =
      RHOcrit * cosmo_params_global->OMb * consts->fstar_7;
  ...
  double prefactor_nion_mini =
      prefactor_stars_mini * consts->fesc_7 * consts->pop3_ion;
  double prefactor_wsfr = prefactor_sfr * consts->fesc_10 * consts->pop2_ion;
  double prefactor_wsfr_mini =
      prefactor_sfr_mini * consts->fesc_7 * consts->pop3_ion;
  ...
  intgrl_fesc_weighted =
      Nion_General(consts->redshift, lnMmin, lnMmax, M_turn_a, consts);
  intgrl_stars_only =
      Nion_General(consts->redshift, lnMmin, lnMmax, M_turn_a, &consts_sfrd);
  if (flag_options_global->USE_MINI_HALOS) {
    intgrl_fesc_weighted_mini =
        Nion_General_MINI(consts->redshift, lnMmin, lnMmax, M_turn_m, consts);

    intgrl_stars_only_mini = Nion_General_MINI(consts->redshift, lnMmin, lnMmax,
                                               M_turn_m, &consts_sfrd);
  }
  if (flag_options_global->USE_TS_FLUCT) {
    integral_xray = Xray_General(consts->redshift, lnMmin, lnMmax, M_turn_a,
                                 M_turn_m, consts);
  }
  ...
  averages_out->stellar_mass_mini =
      intgrl_stars_only_mini * prefactor_stars_mini;
  averages_out->sfr_mini = intgrl_stars_only_mini * prefactor_sfr_mini;
  averages_out->n_ion = intgrl_fesc_weighted * prefactor_nion +
                        intgrl_fesc_weighted_mini * prefactor_nion_mini;
  averages_out->fescweighted_sfr =
      intgrl_fesc_weighted * prefactor_wsfr +
...
// This takes a HaloBox struct and fixes it's mean to exactly what we expect
// from the UMF integrals.
...
void mean_fix_grids(double M_min, double M_max, HaloBox *grids,
                    struct HaloProperties *averages_box,
  ...
  get_box_averages(M_min, M_max, M_turn_a_global, M_turn_m_global, consts,
                   &averages_global);
...
// Fixed halo grids, where each property is set as the integral of the CMF on
// the EULERIAN cell scale As per default 21cmfast (strange pretending that the
// lagrangian density is eulerian and then *(1+delta)) This outputs the
// UN-NORMALISED grids (before mean-adjustment)
int set_fixed_grids(double M_min, double M_max, InitialConditions *ini_boxes,
                    ...
                    struct HaloProperties *averages, const bool eulerian) {
...
#pragma omp for reduction(min : min_density, min_log10_mturn_a,        \
                              min_log10_mturn_m)                       \
    ...
    for (i = 0; i < HII_TOT_NUM_PIXELS; i++) {
      ...
      if (min_log10_mturn_a > mturn_a_grid[i])
        min_log10_mturn_a = mturn_a_grid[i];
      if (min_log10_mturn_m > mturn_m_grid[i])
        min_log10_mturn_m = mturn_m_grid[i];
      if (max_log10_mturn_a < mturn_a_grid[i])
        max_log10_mturn_a = mturn_a_grid[i];
      if (max_log10_mturn_m < mturn_m_grid[i])
        max_log10_mturn_m = mturn_m_grid[i];
  ...
  LOG_DEBUG(
      "Mean halo boxes || M = [%.2e %.2e] | Mcell = %.2e (s=%.2e) | z = %.2e | "
      "D = %.2e",
      M_min, M_max, M_cell, sigma_cell, consts->redshift, growth_z);

  // These tables are coarser than needed, an initial loop for Mturn to find
  // limits may help
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    ...
    initialise_SFRD_Conditional_table(consts->redshift, min_density,
                                      max_density, M_min, M_max, M_cell,
                                      consts);

    // This table includes reionisation feedback, but takes the atomic turnover
    // anyway for the upper turnover
    initialise_Nion_Conditional_spline(
        consts->redshift, min_density, max_density, M_min, M_max, M_cell,
        min_log10_mturn_a, max_log10_mturn_a, min_log10_mturn_m,
        max_log10_mturn_m, consts, false);

    initialise_dNdM_tables(min_density, max_density, lnMmin, lnMmax, growth_z,
                           lnMcell, false);
    if (flag_options_global->USE_TS_FLUCT) {
      initialise_Xray_Conditional_table(consts->redshift, min_density,
                                        max_density, M_min, M_max, M_cell,
                                        consts);
    }
  }
...
#pragma omp for reduction(+ : hm_sum, sm_sum, sm_sum_mini, sfr_sum, \
                              sfr_sum_mini, xray_sum, nion_sum, wsfr_sum)
    for (i = 0; i < HII_TOT_NUM_PIXELS; i++) {
      if (eulerian) {
        dens = perturbed_field->density[i];
        dens_fac = (1. + dens);
      ...
      h_count = EvaluateNhalo(dens, growth_z, lnMmin, lnMmax, M_cell,
                              sigma_cell, dens);
      mass_intgrl = EvaluateMcoll(dens, growth_z, lnMmin, lnMmax, M_cell,
                                  sigma_cell, dens);
      intgrl_fesc_weighted =
          EvaluateNion_Conditional(dens, l10_mturn_a, growth_z, M_min, M_max,
                                   M_cell, sigma_cell, consts, false);
      intgrl_stars_only = EvaluateSFRD_Conditional(dens, growth_z, M_min, M_max,
                                                   M_cell, sigma_cell, consts);
      // TODO: SFRD tables still assume no reion feedback, this should be fixed
      //   although it doesn't affect the histories (only used in Ts) it makes
      //   outputs wrong for post-processing
      if (flag_options_global->USE_MINI_HALOS) {
        intgrl_stars_only_mini =
            EvaluateSFRD_Conditional_MINI(dens, l10_mturn_m, growth_z, M_min,
                                          M_max, M_cell, sigma_cell, consts);
        intgrl_fesc_weighted_mini = EvaluateNion_Conditional_MINI(
            dens, l10_mturn_m, growth_z, M_min, M_max, M_cell, sigma_cell,
            consts, false);
      }

      if (flag_options_global->USE_TS_FLUCT) {
        integral_xray = EvaluateXray_Conditional(
            dens, l10_mturn_m, consts->redshift, growth_z, M_min, M_max, M_cell,
            sigma_cell, consts);
      }
      ...
      grids->halo_sfr_mini[i] =
          intgrl_stars_only_mini * prefactor_sfr_mini * dens_fac;
      ...
      if (i == 0) {
        // LOG_ULTRA_DEBUG("(%d %d %d) i_cell %llu i_halo %llu",x,y,z,i_cell,
        // i_halo);
        LOG_ULTRA_DEBUG(
            "Cell 0 fixed: HM: %.2e SM: %.2e (%.2e) SF: %.2e (%.2e) X: %.2e "
            "NI: %.2e WS: %.2e ct : %llu",
            grids->halo_mass[i], grids->halo_stars[i],
            grids->halo_stars_mini[i], grids->halo_sfr[i],
            grids->halo_sfr_mini[i], grids->halo_xray[i], grids->n_ion[i],
            grids->whalo_sfr[i], grids->count[i]);
...
void halobox_debug_print_avg(struct HaloProperties *averages_box,
                             ...
                             struct ScalingConstants *consts, double M_min,
                             double M_max) {
  if (LOG_LEVEL < DEBUG_LEVEL) return;
  struct HaloProperties averages_sub_expected, averages_global;
  LOG_DEBUG("HALO BOXES REDSHIFT %.2f [%.2e %.2e]", consts->redshift, M_min,
            M_max);
  if (flag_options_global->FIXED_HALO_GRIDS) {
    get_box_averages(M_min, M_max, averages_box->m_turn_acg,
                     averages_box->m_turn_mcg, consts, &averages_global);
  } else {
    get_box_averages(user_params_global->SAMPLER_MIN_MASS, M_max,
                     averages_box->m_turn_acg, averages_box->m_turn_mcg, consts,
                     &averages_global);
    if (user_params_global->AVG_BELOW_SAMPLER &&
        M_min < user_params_global->SAMPLER_MIN_MASS) {
      get_box_averages(M_min, user_params_global->SAMPLER_MIN_MASS,
                       averages_box->m_turn_acg, averages_box->m_turn_mcg,
                       consts, &averages_sub_expected);
    }
  }

  LOG_DEBUG(
      "Exp. averages: (HM %11.3e, SM %11.3e SM_MINI %11.3e SFR %11.3e, "
      "SFR_MINI %11.3e, XRAY %11.3e, NION %11.3e)",
      averages_global.halo_mass, averages_global.stellar_mass,
      averages_global.stellar_mass_mini, averages_global.halo_sfr,
      averages_global.sfr_mini, averages_global.halo_xray,
      averages_global.n_ion);
  LOG_DEBUG(
      "Box. averages: (HM %11.3e, SM %11.3e SM_MINI %11.3e SFR %11.3e, "
      "SFR_MINI %11.3e, XRAY %11.3e, NION %11.3e)",
      averages_box->halo_mass, averages_box->stellar_mass,
      averages_box->stellar_mass_mini, averages_box->halo_sfr,
...
// We need the mean log10 turnover masses for comparison with expected global
// Nion and SFRD. Sometimes we don't calculate these on the grid (if we use
// halos and no sub-sampler) So this function simply returns the volume-weighted
// average log10 turnover mass
void get_mean_log10_turnovers(InitialConditions *ini_boxes,
...
void sum_halos_onto_grid(InitialConditions *ini_boxes,
                         TsBox *previous_spin_temp,
                         IonizedBox *previous_ionize_box,
                         PerturbHaloField *halos,
                         struct ScalingConstants *consts, HaloBox *grids,
...
#pragma omp for reduction(+ : hm_avg, sm_avg, sm_avg_mini, sfr_avg,        \
                              sfr_avg_mini, n_ion_avg, xray_avg, wsfr_avg, \
                              M_turn_a_avg, M_turn_m_avg, M_turn_r_avg,    \
                              n_halos_cut)
    for (i_halo = 0; i_halo < halos->n_halos; i_halo++) {
      hmass = halos->halo_masses[i_halo];
      // It is sometimes useful to make cuts to the halo catalogues before
      // gridding.
      //   We implement this in a simple way, if the user sets a halo's mass to
      //   zero we skip it
      if (hmass == 0.) {
        n_halos_cut++;
        continue;
      }
      // NOTE:Unlike HaloField, PerturbHaloField is on HII_DIM so we don't need
      // to correct here
      ...
      // NOTE: I could easily apply reionization feedback without minihalos but
      ...
      set_halo_properties(hmass, M_turn_a, M_turn_m, consts, in_props,
                          &out_props);
      ...
      if (x + y + z == 0) {
        ...
        // i_halo);
        LOG_ULTRA_DEBUG(
            "Cell 0 Halo: HM: %.2e SM: %.2e (%.2e) SF: %.2e (%.2e) X: %.2e NI: "
            "%.2e WS: %.2e Z : %.2e ct : %llu",
            hmass, stars, stars_mini, sfr, sfr_mini, xray, nion, wsfr,
            out_props.metallicity, i_halo);

        // LOG_ULTRA_DEBUG("Cell 0 Sums: HM: %.2e SM: %.2e (%.2e) SF: %.2e
        // (%.2e) X: %.2e NI: %.2e WS: %.2e ct : %d",
        ...
        LOG_ULTRA_DEBUG("Mturn_a %.2e Mturn_m %.2e RNG %.3f %.3f %.3f",
                        M_turn_a, M_turn_m, in_props[0], in_props[1],
                        in_props[2]);
      }
#endif
...
#pragma omp for
  ...
  LOG_SUPER_DEBUG(
      "Cell 0 Totals: HM: %.2e SM: %.2e (%.2e) SF: %.2e (%.2e) X: %.2e NI: "
      "%.2e WS: %.2e ct : %d",
      grids->halo_mass[HII_R_INDEX(0, 0, 0)],
      grids->halo_stars[HII_R_INDEX(0, 0, 0)],
      grids->halo_stars_mini[HII_R_INDEX(0, 0, 0)],
      grids->halo_sfr[HII_R_INDEX(0, 0, 0)],
      grids->halo_sfr_mini[HII_R_INDEX(0, 0, 0)],
      grids->halo_xray[HII_R_INDEX(0, 0, 0)],
      grids->n_ion[HII_R_INDEX(0, 0, 0)],
      grids->whalo_sfr[HII_R_INDEX(0, 0, 0)],
      grids->count[HII_R_INDEX(0, 0, 0)]);

  // NOTE: There is an inconsistency here, the sampled grids use a halo-averaged
  // turnover mass
  //   whereas the fixed grids / default 21cmfast uses the volume averaged
  //   LOG10(turnover mass). Neither of these are a perfect representation due
  //   to the nonlinear way turnover mass affects N_ion
  if (total_n_halos > 0) {
...
// We grid a PERTURBED halofield into the necessary quantities for calculating
...
#pragma omp parallel for num_threads(user_params->N_THREADS) private(idx)
    ...
    if (user_params->USE_INTERPOLATION_TABLES > 0) {
      initialiseSigmaMInterpTable(
          M_min / 2, M_MAX_INTEGRAL);  // this needs to be initialised above
                                       // MMax because of Nion_General
    }
    // do the mean HMF box
    // The default 21cmFAST has a strange behaviour where the nonlinear density
    // is used as linear, the condition mass is at mean density, but the total
    // cell mass is multiplied by delta This part mimics that behaviour Since we
    // need the average turnover masses before we can calculate the global
    // means, we do the CMF integrals first Then we calculate the expected UMF
    // integrals before doing the adjustment
    if (flag_options->FIXED_HALO_GRIDS) {
      set_fixed_grids(M_min, M_max, ini_boxes, perturbed_field,
                      previous_spin_temp, previous_ionize_box, &hbox_consts,
                      grids, &averages_box, true);
    } else {
      // set below-resolution properties
      if (user_params->AVG_BELOW_SAMPLER &&
          M_min < user_params->SAMPLER_MIN_MASS) {
        set_fixed_grids(M_min, user_params->SAMPLER_MIN_MASS, ini_boxes,
                        perturbed_field, previous_spin_temp,
                        previous_ionize_box, &hbox_consts, grids,
                        &averages_subsampler, false);
// This is pretty redundant, but since the fixed grids have density units (X
// Mpc-3) I have to re-multiply before adding the halos.
//       I should instead have a flag to output the summed values in cell.
//       (2*N_pixel > N_halo so generally i don't want to do it in the halo
//       loop)
#pragma omp parallel for num_threads(user_params->N_THREADS) private(idx)
        for (idx = 0; idx < HII_TOT_NUM_PIXELS; idx++) {
        ...
        LOG_DEBUG("finished subsampler M[%.2e %.2e]", M_min,
                  user_params->SAMPLER_MIN_MASS);
      } else {
        // we still need the average turnovers for global values in spintemp, so
        // get them here
        get_mean_log10_turnovers(ini_boxes, previous_spin_temp,
                                 previous_ionize_box, perturbed_field,
                                 &hbox_consts, turnovers);
    ...
    halobox_debug_print_avg(&averages_box, &averages_subsampler, &hbox_consts,
                            M_min, M_max);

    // NOTE: the density-grid based calculations (!USE_HALO_FIELD)
    //  use the cell-weighted average of the log10(Mturn) (see issue #369)
    LOG_SUPER_DEBUG(
        "log10 Mutrn ACG: log10 cell-weighted %.6e Halo-weighted %.6e",
        pow(10, grids->log10_Mcrit_ACG_ave), averages_box.m_turn_acg);
    LOG_SUPER_DEBUG(
        "log10 Mutrn MCG: log10 cell-weighted %.6e Halo-weighted %.6e",
...
// test function for getting halo properties from the wrapper, can use a lot of
// memory for large catalogs
int test_halo_props(double redshift, UserParams *user_params,
                    CosmoParams *cosmo_params, AstroParams *astro_params,
                    FlagOptions *flag_options, float *vcb_grid,
                    float *J21_LW_grid, float *z_re_grid,
                    float *Gamma12_ion_grid, int n_halos, float *halo_masses,
                    int *halo_coords, float *star_rng, float *sfr_rng,
...
#pragma omp for
      for (i_halo = 0; i_halo < n_halos; i_halo++) {
        ...
        // It is sometimes useful to make cuts to the halo catalogues before
        // gridding.
        //   We implement this in a simple way, if the user sets a halo's mass
        //   to zero we skip it
        if (m == 0.) {
          continue;
        }
        ...
        // set values before reionisation feedback
        // NOTE: I could easily apply reionization feedback without minihalos
        // but this was not done previously
        if (flag_options_global->USE_MINI_HALOS) {
          if (!flag_options_global->FIX_VCB_AVG &&
              user_params_global->USE_RELATIVE_VELOCITIES)
          ...
          M_turn_r = reionization_feedback(redshift, Gamma12_val, zre_val);
          M_turn_a =
              fmax(M_turn_a, fmax(M_turn_r, astro_params_global->M_TURN));
          M_turn_m =
              fmax(M_turn_m, fmax(M_turn_r, astro_params_global->M_TURN));
        }
        ...
        set_halo_properties(m, M_turn_a, M_turn_m, &hbox_consts, in_props,
                            &out_props);
        ...
        if (i_halo < 10) {
          LOG_ULTRA_DEBUG("HM %.2e SM %.2e SF %.2e NI %.2e LX %.2e",
                          out_props.halo_mass, out_props.stellar_mass,
                          out_props.halo_sfr, out_props.n_ion,
                          out_props.halo_xray);
          LOG_ULTRA_DEBUG("MINI: SM %.2e SF %.2e WSF %.2e",
                          out_props.stellar_mass_mini, out_props.sfr_mini,
                          out_props.fescweighted_sfr);
          LOG_ULTRA_DEBUG("Mturns ACG %.2e MCG %.2e Reion %.2e", M_turn_a,
                          M_turn_m, M_turn_r);
          LOG_ULTRA_DEBUG("RNG: STAR %.2e SFR %.2e XRAY %.2e", in_props[0],
                          in_props[1], in_props[2]);
        }
      }
    }
  }
