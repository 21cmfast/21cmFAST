...
*/
float distance_coord(float x1, float y1, float z1, float x2, float y2, float z2,
                     float *x, float *y, float *z) {
  float minimumsq, xsq, ysq, zsq, xplussq, yplussq, zplussq, xminsq, yminsq,
      zminsq;
...
void check_region(float *box, int dimensions, int dimensions_ncf,
                  float Rsq_curr_index, int x, int y, int z, int x_min,
      ...
      for (z_curr = z_min; z_curr <= z_max; z_curr++) {
        ...
        if (box[HII_R_INDEX(
          ...
          if ((Rsq_curr_index > (xsq + ysq + zsq)) ||
            ...
            // we are within the sphere defined by R, so change flag in box
            // array
...
*/
void update_in_sphere(float *box, int dimensions, int dimensions_ncf, float R,
                      float xf, float yf, float zf) {
  int x_curr, y_curr, z_curr, xb_min, xb_max, yb_min, yb_max, zb_min, zb_max,
      R_index;
  ...
  /*****  first, just automatically fill in the inner cube whose diagonal is R,
   * side is 2R/sqrt(3) *****/
  ...
  /****** now check the pixels between the smaller and the larger cube which
   * encloses the sphere ******/
  ...
  //    check_region(box, dimensions, Rsq_curr_index, x,y,z, xb_min, xb_max,
  //    yb_min, yb_max, zb_min, zb_max);

  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xb_min,
               xl_min, yb_min, yb_max, zb_min, zb_max);
  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xl_max,
               xb_max, yb_min, yb_max, zb_min, zb_max);

  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xb_min,
               xb_max, yb_min, yl_min, zb_min, zb_max);
  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xb_min,
               xb_max, yl_max, yb_max, zb_min, zb_max);

  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xb_min,
               xb_max, yb_min, yb_max, zb_min, zl_min);
  check_region(box, dimensions, dimensions_ncf, Rsq_curr_index, x, y, z, xb_min,
               xb_max, yb_min, yb_max, zl_max, zb_max);
}
