...
void get_velocity_gradient(UserParams *user_params, float *v,
                           float *vel_gradient) {
...
int ComputeBrightnessTemp(float redshift, UserParams *user_params,
  ...
  Try {  // Try block around whole function.
    LOG_DEBUG("Starting Brightness Temperature calculation for redshift %f",
              redshift);
    ...
    float *vel_gradient =
        (float *)calloc(HII_TOT_FFT_NUM_PIXELS, sizeof(float));
...
#pragma omp for
    ...
    H = hubble(redshift);
    const_factor = 27 *
                   (cosmo_params->OMb * cosmo_params->hlittle *
                    cosmo_params->hlittle / 0.023) *
                   sqrt((0.15 / (cosmo_params->OMm) / (cosmo_params->hlittle) /
    ...
    // ok, lets fill the delta_T box; which will be the same size as the bubble
    // box
#pragma omp parallel shared(                                                   \
        const_factor, perturb_field, ionized_box, box, redshift, spin_temp,    \
...
#pragma omp for reduction(+ : ave)
          ...
          for (k = 0; k < HII_D_PARA; k++) {
            ...
            if (flag_options->USE_TS_FLUCT) {
              if (flag_options->SUBCELL_RSD) {
                // Converting the prefactors into the optical depth, tau. Factor
                // of 1000 is the conversion of spin temperature from K to mK
                box->brightness_temp[HII_R_INDEX(i, j, k)] *=
                    (1. + redshift) /
    ...
    if (flag_options->APPLY_RSDS) {
      ...
      // now add the velocity correction to the delta_T maps (only used for T_S
      // >> T_CMB case).
...
#pragma omp parallel shared(                                        \
        vel_gradient, T_rad, redshift, spin_temp, box,              \
            max_v_deriv) private(i, j, k, gradient_component, dvdx) \
    num_threads(user_params -> N_THREADS)
        {
#pragma omp for
          for (i = 0; i < user_params->HII_DIM; i++) {
            for (j = 0; j < user_params->HII_DIM; j++) {
              for (k = 0; k < HII_D_PARA; k++) {
                dvdx = clip(vel_gradient[HII_R_FFT_INDEX(i, j, k)],
                            -max_v_deriv, max_v_deriv);
...
#pragma omp for
            ...
            for (j = 0; j < user_params->HII_DIM; j++) {
              for (k = 0; k < HII_D_PARA; k++) {
                ...
                if (gradient_component < FRACT_FLOAT_ERR) {
                  ...
                  box->brightness_temp[HII_R_INDEX(i, j, k)] =
                      1000. *
                      (spin_temp->Ts_box[HII_R_INDEX(i, j, k)] - T_rad) /
                      (1. + redshift);
                } else {
                  box->brightness_temp[HII_R_INDEX(i, j, k)] =
      ...
      if (flag_options->SUBCELL_RSD) {
        ave = apply_subcell_rsds(user_params, cosmo_params, flag_options,
                                 astro_params, ionized_box, box, redshift,
                                 spin_temp, T_rad, v, H);
      }
    }
    ...
    if (isfinite(ave) == 0) {
      LOG_ERROR(
          "Average brightness temperature (after including velocities) is "
          "infinite or NaN!");
