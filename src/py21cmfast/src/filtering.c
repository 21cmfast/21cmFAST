...
double exp_mfp_filter(double k, double R, double mfp, double exp_term) {
  ...
  if (kR < 1e-4) {
    double ts_0 =
        6 * pow(ratio, 3) -
        exp_term * (6 * pow(ratio, 3) + 6 * pow(ratio, 2) + 3 * ratio);
    return ts_0 + (exp_term * (2 * pow(ratio, 2) + 0.5 * ratio) -
                   2 * ts_0 * pow(ratio, 2)) *
                      kR * kR;
  }
...
double spherical_shell_filter(double k, double R_outer, double R_inner) {
  ...
  return 3.0 / (pow(kR_outer, 3) - pow(kR_inner, 3)) *
         (sin(kR_outer) - cos(kR_outer) * kR_outer - sin(kR_inner) +
          cos(kR_inner) * kR_inner);
}
...
#pragma omp for
    for (n_x = 0; n_x < dimension; n_x++) {
      ...
      for (n_y = 0; n_y < dimension; n_y++) {
        ...
        for (n_z = 0;
             n_z <= (int)(user_params_global->NON_CUBIC_FACTOR * midpoint);
             n_z++) {
          k_z = n_z * DELTA_K_PARA;
          k_mag_sq = k_x * k_x + k_y * k_y + k_z * k_z;

          grid_index =
              RES == 1 ? HII_C_INDEX(n_x, n_y, n_z) : C_INDEX(n_x, n_y, n_z);
          ...
          if (filter_type == 0) {  // real space top-hat
            ...
            //  This is actually (kR^2) but since we zero the value and find kR
            //  > 1 this is more computationally efficient kR = 0.17103765852*(
            //  k_x*k_x + k_y*k_y + k_z*k_z )*R*R;
            kR = sqrt(k_mag_sq) * R;
            box[grid_index] *= sharp_k_filter(kR);
          } else if (filter_type == 2) {  // gaussian
            // This is actually (kR^2) but since we zero the value and find kR >
            // 1 this is more computationally efficient
            kR = k_mag_sq * R * R;
            box[grid_index] *= gaussian_filter(kR);
          }
          // The next two filters are not given by the HII_FILTER global, but
          // used for specific grids
          else if (filter_type == 3) {  // exponentially decaying tophat, param
                                        // == scale of decay (MFP)
            // NOTE: This should be optimized, I havne't looked at it in a while
            box[grid_index] *=
                exp_mfp_filter(sqrt(k_mag_sq), R, R_param, R_const);
          } else if (filter_type ==
                     4) {  // spherical shell, R_param == inner radius
            box[grid_index] *=
                spherical_shell_filter(sqrt(k_mag_sq), R, R_param);
          } else {
...
int test_filter(UserParams *user_params, CosmoParams *cosmo_params,
  ...
  // setup the box
  fftwf_complex *box_unfiltered = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
  fftwf_complex *box_filtered = (fftwf_complex *)fftwf_malloc(
      sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS);
