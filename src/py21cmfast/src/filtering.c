...
void initialize_alphas_and_betas_for_multiple_scattering(
    ...
    if (R_star == 0.) {
        ...
        double x_em_inner = R_inner / R_star;
        double x_em_outer = R_outer / R_star;

        double mu_inner = compute_mu_for_multiple_scattering(x_em_inner);
        double eta_inner = compute_eta_for_multiple_scattering(x_em_inner);
        double mu_outer = compute_mu_for_multiple_scattering(x_em_outer);
...
double asymptotic_2F3(double kR, double alpha, double beta) {
    ...
    if (alpha < 10.) {
        ...
        decay_term1 = M_PI * gamma_a1 * gamma_b1_minus_a1_inv / tgamma(b2 - a1) / tgamma(b3 - a1) /
                      pow(kR / 2., alpha + 2.);  // gamma(a2-a1) = sqrt(pi)
        decay_term2 = -2. * M_PI * gamma_a2 * gamma_b1_minus_a2_inv * gamma_b2_minus_a2_inv /
    ...
    F_asymp = (cos(kR - M_PI * (2. + beta) / 2.) -
               (1. + (alpha - 1.) * beta) / kR * sin(kR - M_PI * (2. + beta) / 2.)) /
              pow(kR / 2, beta + 2);
        ...
        for (n_x = 0; n_x < box_dim[0]; n_x++) {
            ...
            for (n_y = 0; n_y < box_dim[1]; n_y++) {
                ...
                for (n_z = 0; n_z <= box_dim[2] / 2; n_z++) {
                    ...
                    } else if (filter_type == 4) {  // spherical shell, R_param == outer radius
...
void filter_box(fftwf_complex *box, int box_dim[3], int filter_type, float R, float R_param,
    ...
    if (use_cuda) {
#if USE_CUDA
        filter_box_gpu(box, box_dim, filter_type, R, R_param, R_star);
#else
...
int test_filter_cpu(float *input_box, double R, double R_param, double R_star, int filter_flag,
    ...
    filter_box_cpu(box_filtered, box_dim, filter_flag, R, R_param, R_star);
...
int test_filter(float *input_box, double R, double R_param, double R_star, int filter_flag,
    ...
    if (use_cuda) {
#if USE_CUDA
        return test_filter_gpu(input_box, R, R_param, R_star, filter_flag, result);
#else
        LOG_ERROR("CUDA version of test_filter() called but code was not compiled for CUDA.");
        return 1;
#endif
    } else {
        return test_filter_cpu(input_box, R, R_param, R_star, filter_flag, result);
    }
}
