...
float ComputeFullyIoinizedTemperature(float z_re, float z, float delta,
  ...
  if (fabs(z - z_re) < 1e-4)
    ...
    // evolving ionized box eq. 6 of McQuinn 2015, ignored the dependency of
    // density at ionization
...
/* returns the case A hydrogen recombination coefficient (Abel et al. 1997) in
 * cm^3 s^-1*/
...
/* returns the case B hydrogen recombination coefficient (Spitzer 1978) in cm^3
 * s^-1*/
...
double neutral_fraction(double density, double T4, double gamma, int usecaseB) {
  ...
  //  this code, while mathematically accurate, is numerically buggy for very
  //  small x_HI, so i will use valid approximation x_HI <<1 above when x_HI <
  //  1e-5, and this otherwise... the two converge seemlessly
...
/* function HeI_ion_crosssec returns the HI ionization cross section at
 parameter frequency (taken from Verner et al (1996) */
...
/* function HeII_ion_crosssec returns the HeII ionization cross section at
 parameter frequency (taken from Osterbrock, pg. 14) */
double HeII_ion_crosssec(double nu) {
  ...
  return (6.3e-18) / Z / Z * pow(HeII_NUIONIZATION / nu, 4) *
         pow(E, 4 - (4 * atan(epsilon) / epsilon)) /
         (1 - pow(E, -2 * PI / epsilon));
}

/* function HI_ion_crosssec returns the HI ionization cross section at parameter
 frequency (taken from Osterbrock, pg. 14) */
...
/* Return the thomspon scattering optical depth from zstart to zend through
 fully ionized IGM. The hydrogen reionization history is given by the zarry and
 xHarry parameters, in increasing redshift order of length len.*/
typedef struct {
  float *z, *xH;
  int len;
} tau_e_params;
double dtau_e_dz(double z, void *params) {
  ...
  if ((p.len == 0) || !(p.z)) {
    ...
    xH = p.xH[i - 1] +
         (p.xH[i] - p.xH[i - 1]) / (p.z[i] - p.z[i - 1]) * (z - p.z[i - 1]);
    xi = 1.0 - xH;
    if (xi < 0) {
...
double tau_e(float zstart, float zend, float *zarry, float *xHarry, int len,
  ...
  if (zend > z_re_HeII) {  // && (zstart < Zreion_HeII)){
    if (zstart < z_re_HeII) {
      status = gsl_integration_qag(&F, z_re_HeII, zstart, 0, rel_tol, 1000,
                                   GSL_INTEG_GAUSS61, w, &prehelium, &error);
      ...
      status = gsl_integration_qag(&F, zend, z_re_HeII, 0, rel_tol, 1000,
                                   GSL_INTEG_GAUSS61, w, &posthelium, &error);
      ...
      prehelium = 0;
      status = gsl_integration_qag(&F, zend, zstart, 0, rel_tol, 1000,
                                   GSL_INTEG_GAUSS61, w, &posthelium, &error);
...
double lyman_werner_threshold(float z, float J_21_LW, float vcb,
  ...
  // correction follows Schauer+20, fit jointly to LW feedback and relative
  // velocities. They find weaker effect of LW feedback than before (Stacy+11,
  // Greif+11, etc.) due to HII self shielding.
  double mcrit_noLW =
      3.314e7 *
      pow(1. + z, -1.5);  // this follows Visbal+15, which is taken as the
                          // optimal fit from Fialkov+12 which was calibrated
                          // with the simulations of Stacy+11 and Greif+11;

  double f_LW = 1.0 + astro_params->A_LW * pow(J_21_LW, astro_params->BETA_LW);

  double f_vcb =
      pow(1.0 + astro_params->A_VCB * vcb / SIGMAVCB, astro_params->BETA_VCB);

  // double mcrit_LW = mcrit_noLW * (1.0 + 10. * sqrt(J_21_LW)); //Eq. (12) in
  // Schauer+20 return pow(10.0, log10(mcrit_LW) + 0.416 * vcb/SIGMAVCB ); //vcb
  // and sigmacb in km/s, from Eq. (9)
