/*
    ...
    NOTE: Not all 21cmFAST variables will be found below. Only those useful for
   21CMMC
...
#define L_FACTOR \
  (float)(0.620350491)  // factor relating cube length to filter radius =
                        // (4PI/3)^(-1/3)

/*
 Filenames of the appropriate output from RECFAST to be used as boundary
 conditions in Ts.c as well as other tables used to compute the spin temperature
 */
...
/* Maximum allowed value for the kinetic temperature. Useful to set to avoid
 some spurious behaviour when the code is run with redshift poor resolution and
 very high X-ray heating efficiency */
...
#define Watson_alpha \
  (2.163)  // Watson FOF HMF, alpha parameter (Watson et al. 2013)
#define Watson_beta \
  (1.406)  // Watson FOF HMF, beta parameter (Watson et al. 2013)
#define Watson_gamma \
  (1.210)  // Watson FOF HMF, gamma parameter (Watson et al. 2013)

// Universal FOF HMF with redshift evolution (Watson et al. 2013)
#define Watson_A_z_1 \
  (0.990)  // Watson FOF HMF, normalisation of A_z parameter (Watson et al.
           // 2013)
#define Watson_A_z_2 \
  (-3.216)  // Watson FOF HMF, power law of A_z parameter (Watson et al. 2013)
#define Watson_A_z_3 \
  (0.074)  // Watson FOF HMF, offset of A_z parameter (Watson et al. 2013)
#define Watson_alpha_z_1 \
  (5.907)  // Watson FOF HMF, normalisation of alpha_z parameter (Watson et al.
           // 2013)
#define Watson_alpha_z_2 \
  (-3.058)  // Watson FOF HMF, power law of alpha_z parameter (Watson et al.
            // 2013)
#define Watson_alpha_z_3 \
  (2.349)  // Watson FOF HMF, offset of beta_z parameter (Watson et al. 2013)
#define Watson_beta_z_1 \
  (3.136)  // Watson FOF HMF, normalisation of beta_z parameter (Watson et al.
           // 2013)
#define Watson_beta_z_2 \
  (-3.599)  // Watson FOF HMF, power law of beta_z parameter (Watson et al.
            // 2013)
#define Watson_beta_z_3 \
  (2.344)  // Watson FOF HMF, offset of beta_z parameter (Watson et al. 2013)
#define Watson_gamma_z \
  (1.318)  // Watson FOF HMF, gamma parameter (Watson et al. 2013)

// CONSTANTS //
#define LN10 (double)(2.30258509299)
#define SIGMAT \
  (double)(6.6524e-25)  // Thomson scattering cross section in cm^-2
#define SIGMA_HI \
  (double)(6.3e-18)  // HI ionization  cross section at 13.6 eV in cm^-2
...
#define e_charge \
  (double)4.80320467e-10  // elemetary charge (esu=g^1/2 cm^3/2 s^-1
#define SQDEG_ALLSKY \
  (double)((360.0 * 360.0) / PI)  // Square degrees in all sky
...
#define HeII_NUIONIZATION \
  (double)(NUIONIZATION * 4)  // ionization frequency of HeII
#define HeI_NUIONIZATION \
  (double)(24.59 * NU_over_EV)  // ionization frequency of HeI
#define T21 (double)0.0682      // temperature corresponding to the 21cm photon
#define A10_HYPERFINE \
  (double)2.85e-15  // spontaneous emission coefficient in s^-1

#define Lambda_21 (double)21.106114054160  // Wavelength of 21cm Radiation in cm
#define A21_Lya \
  (double)6.24e8  // Spontaneous emission coefficient for Lyman-Alpha line in
                  // s^-1
...
#define zpp_interp_points_SFR                                                  \
  (int)(400) /* Number of interpolation points for the interpolation table for \
                z'' */
#define dens_Ninterp                                                           \
  (int)(400) /* Number of interpolation points for the interpolation table for \
...
#define KBOT_CLASS \
  (float)(1e-5)  // max and min k in  CLASS transfer function, temporary until
                 // interfaced properly
...
#define AINDEX1 \
  (double)(9.0)  // power-law index of nu(M) between MPIVOT1 and infinite
#define AINDEX2 \
  (double)(13.6)  // power-law index of nu(M) between MPIVOT2 and MPIVOT1
#define AINDEX3 \
  (double)(21.0)  // power-law index of nu(M) between 0 and
                  // MPIVOT2
#define MMIN_FAST \
  (double)(1e5)  // min mass at which the sigma table is computed if
                 // FAST_FCOLL_TABLES is turned on. Has to be below MPIVOT2
...
#define RHOcrit                               \
  (double)((3.0 * Ho * Ho / (8.0 * PI * G)) * \
           (CMperMPC * CMperMPC * CMperMPC) / \
           Msun)  // Msun Mpc^-3 ---- at z=0
#define RHOcrit_cgs \
  (double)(3.0 * Ho * Ho / (8.0 * PI * G))  // g pcm^-3 ---- at z=0
#define No                                          \
  (double)(RHOcrit_cgs * cosmo_params_global->OMb * \
           (1 - cosmo_params_global->Y_He) /        \
           m_p)  //  current hydrogen number density estimate  (#/cm^3)
                 //  ~1.92e-7
#define He_No                                       \
  (double)(RHOcrit_cgs * cosmo_params_global->OMb * \
           cosmo_params_global->Y_He /              \
