...
SpectralIntegDeviceData *init_spectral_integration_gpu(
    unsigned long long num_pixels, int n_step_ts, double **freq_int_heat_tbl,
    double **freq_int_ion_tbl, double **freq_int_lya_tbl, double **freq_int_heat_tbl_diff,
    double **freq_int_ion_tbl_diff, double **freq_int_lya_tbl_diff, int *m_xHII_low_box,
    float *inverse_val_box, bool use_mini_halos, bool use_x_ray_heating, bool use_lya_heating);

void launch_spectral_integration_kernel(
    SpectralIntegDeviceData *dev, int R_ct, float *del_fcoll_Rct, float *del_fcoll_Rct_MINI,
    double z_edge_factor, double xray_R_factor, double avg_fix_term, double avg_fix_term_MINI,
    double F_STAR10, double L_X, double s_per_yr, double F_STAR7_MINI, double L_X_MINI,
    double dstarlya_dt_prefactor_R, double dstarlyLW_dt_prefactor_R,
    double dstarlyLW_dt_prefactor_MINI_R, double dstarlya_dt_prefactor_MINI_R,
    double dstarlya_cont_dt_prefactor_R, double dstarlya_inj_dt_prefactor_R,
    double dstarlya_cont_dt_prefactor_MINI_R, double dstarlya_inj_dt_prefactor_MINI_R);

void download_spectral_integration_results(SpectralIntegDeviceData *dev, double *dxheat_dt_box,
                                           double *dxion_source_dt_box, double *dxlya_dt_box,
                                           double *dstarlya_dt_box, double *dstarlyLW_dt_box,
...
void launch_spin_temperature_kernel(
    unsigned long long num_pixels, float zp, float dzp, double xray_prefactor, double volunit_inv,
    double lya_star_prefactor, double Nb_zp, double Trad, double Trad_inv, double Ts_prefactor,
    double xa_tilde_prefactor, double xc_inverse, double dcomp_dzp_prefactor, double hubble_zp,
    double N_zp, double growth_zp, double dgrowth_dzp, double dt_dzp, double growth_factor_zp,
    double inverse_growth_factor_z, double No_val, double N_b0_val, double H_FRAC_val,
    double HE_FRAC_val, double CLUMPING_FACTOR, double A10, double c_cms, double lambda_21,
    double k_B, double h_p, double T_21, double m_p, bool use_x_ray_heating, bool use_mini_halos,
    double *dxheat_dt_box, double *dxion_source_dt_box, double *dxlya_dt_box,
    double *dstarlya_dt_box, double *dstarlyLW_dt_box, float *density, float *prev_spin_temperature,
    float *prev_kinetic_temp, float *prev_xray_ionised_fraction, float *spin_temperature,
    float *kinetic_temp, float *xray_ionised_fraction, float *J_21_LW);
