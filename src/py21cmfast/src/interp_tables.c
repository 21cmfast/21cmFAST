/* This file defines specific interpolation table initialisation functions, kept
   separate from the general interpolation table routines In order to allow them
   to use calculations based on other interpolation tables. Most importantly
   these fucntions require those from ps.c which requires the sigma(M)
...
// we need to define a density minimum for the tables, since we are in
// lagrangian density / linear growth it's possible to go below -1 so we
// explicitly set a minimum here which sets table limits and puts no halos in
// cells below that (Lagrangian) density
...
// TODO: SFRD tables assume no reionisation feedback, this is self-inconsistent,
// but probably okay given
...
// Since we want to easily construct it from the sigma table, it won't be
// uniform so use GSL
// TODO: Consider a rootfind on the integrals for accuracy and speed if we want
// an RGTable
...
// NOTE: this table is initialised for up to N_redshift x N_Mturn, but only
// called N_filter times to assign ST_over_PS in Spintemp.
//   It may be better to just do the integrals at each R
void initialise_SFRD_spline(int Nbin, float zmin, float zmax,
  ...
  if (flag_options_global->USE_MINI_HALOS) {
    ...
    SFRD_z_table_MINI.y_width =
        (LOG10_MTURN_MAX - LOG10_MTURN_MIN) / ((double)NMTURN - 1.);
  }
...
#pragma omp for
    for (i = 0; i < Nbin; i++) {
      z_val = SFRD_z_table.x_min +
              i * SFRD_z_table
                      .x_width;  // both tables will have the same values here
      sc_sfrd = evolve_scaling_constants_to_redshift(
          z_val, astro_params_global, flag_options_global, &sc_sfrd, false);
      lnMmin = log(minimum_source_mass(z_val, true, astro_params_global,
                                       flag_options_global));

      if (flag_options_global->USE_MINI_HALOS) {
        for (j = 0; j < NMTURN; j++) {
          mturn_mcg =
              pow(10, SFRD_z_table_MINI.y_min + j * SFRD_z_table_MINI.y_width);
    ...
    if (flag_options_global->USE_MINI_HALOS) {
      for (j = 0; j < NMTURN; j++) {
        if (isfinite(SFRD_z_table_MINI.z_arr[i][j]) == 0) {
          LOG_ERROR(
              "Detected either an infinite or NaN value in SFRD_MINI table");
          Throw(TableGenerationError);
        }
      }
    }
...
// Unlike the SFRD spline, this one is used more due to the nu_tau_one()
// rootfind although still ignores reionisation feedback
void initialise_Nion_Ts_spline(int Nbin, float zmin, float zmax,
                               struct ScalingConstants *sc) {
  int i, j;
  double Mmax = M_MAX_INTEGRAL;
  double lnMmax = log(Mmax);
...
#pragma omp for
    for (i = 0; i < Nbin; i++) {
      z_val = Nion_z_table.x_min +
              i * Nion_z_table
                      .x_width;  // both tables will have the same values here
      sc_z = evolve_scaling_constants_to_redshift(
          z_val, astro_params_global, flag_options_global, sc, false);
      // Minor note: while this is called in xray, we use it to estimate ionised
      // fraction, do we use ION_Tvir_MIN if applicable?
      lnMmin = log(minimum_source_mass(z_val, true, astro_params_global,
                                       flag_options_global));
      if (flag_options_global->USE_MINI_HALOS) {
        for (j = 0; j < NMTURN; j++) {
          mturn_mcg =
              pow(10, Nion_z_table_MINI.y_min + j * Nion_z_table_MINI.y_width);
          Nion_z_table_MINI.z_arr[i][j] =
              Nion_General_MINI(z_val, lnMmin, lnMmax, mturn_mcg, &sc_z);
        }
      }
      Nion_z_table.y_arr[i] =
          Nion_General(z_val, lnMmin, lnMmax, sc_z.mturn_a_nofb, &sc_z);
    }
  }
    ...
    if (flag_options_global->USE_MINI_HALOS) {
      for (j = 0; j < NMTURN; j++) {
        if (isfinite(Nion_z_table_MINI.z_arr[i][j]) == 0) {
          LOG_ERROR(
              "Detected either an infinite or NaN value in Nion_z_val_MINI");
          Throw(TableGenerationError);
        }
      }
    }
...
void initialise_FgtrM_delta_table(double min_dens, double max_dens, double zpp,
                                  double growth_zpp, double smin_zpp,
                                  double smax_zpp) {
  int i;
  double dens;

  LOG_SUPER_DEBUG(
      "Initialising FgtrM table between delta %.3e and %.3e, sigma %.3e and "
      "%.3e",
  ...
  for (i = 0; i < dens_Ninterp; i++) {
    ...
    fcoll_conditional_table.y_arr[i] =
        FgtrM_bias_fast(growth_zpp, dens, smin_zpp, smax_zpp);
    dfcoll_conditional_table.y_arr[i] =
        dfcoll_dz(zpp, smin_zpp, dens, smax_zpp);
  }
}
...
void init_FcollTable(double zmin, double zmax, bool x_ray) {
  ...
  for (i = 0; i < n_z; i++) {
    ...
    M_min = minimum_source_mass(z_val, x_ray, astro_params_global,
                                flag_options_global);
    lnMmin = log(M_min);
...
// NOTE: since reionisation feedback is not included in the Ts calculation, the
// SFRD spline
//   is Rx1D unlike the Mini table, which is Rx2D
// NOTE: SFRD tables have fixed Mturn range, Nion tables vary
// NOTE: it would be slightly less accurate but maybe faster to tabulate in
// linear delta, linear Fcoll
//   rather than linear-log, check the profiles
void initialise_Nion_Conditional_spline(
    double z, double min_density, double max_density, double Mmin, double Mmax,
    double Mcond, double log10Mturn_min, double log10Mturn_max,
  ...
  LOG_SUPER_DEBUG(
      "Initialising Nion conditional table at mass %.2e from delta %.2e to "
      "%.2e",
      Mcond, min_density, max_density);
  LOG_SUPER_DEBUG("l10Mturns ACG %.2e %.2e MCG %.2e %.2e", log10Mturn_min,
  ...
  // If we use minihalos, both tables are 2D (delta,mturn) due to reionisaiton
  // feedback otherwise, the Nion table is 1D, since reionsaiton feedback is
  // only active with minihalos
  if (flag_options_global->USE_MINI_HALOS) {
    if (prev) {
    ...
    table_mini->y_width =
        (log10Mturn_max_MINI - log10Mturn_min_MINI) / (NMTURN - 1.);
  } else {
    ...
    Nion_conditional_table1D.x_width =
        (max_density - min_density) / (NDELTA - 1.);
  }

  for (i = 0; i < NDELTA; i++) {
    overdense_table[i] = min_density + (float)i / ((float)NDELTA - 1.) *
                                           (max_density - min_density);
  }
  if (flag_options_global->USE_MINI_HALOS) {
    for (i = 0; i < NMTURN; i++) {
      mturns[i] =
          pow(10., log10Mturn_min + (float)i / ((float)NMTURN - 1.) *
                                        (log10Mturn_max - log10Mturn_min));
      mturns_MINI[i] =
          pow(10., log10Mturn_min_MINI +
                       (float)i / ((float)NMTURN - 1.) *
...
#pragma omp for
    for (i = 0; i < NDELTA; i++) {
      if (!flag_options_global->USE_MINI_HALOS) {
        ...
        Nion_conditional_table1D.y_arr[i] = log(
            Nion_ConditionalM(growthf, lnMmin, lnMmax, log(Mcond), sigma2,
                              overdense_table[i], sc->mturn_a_nofb, sc,
      ...
      for (j = 0; j < NMTURN; j++) {
        table_2d->z_arr[i][j] = log(Nion_ConditionalM(
            ...
            mturns[j], sc, user_params_global->INTEGRATION_METHOD_ATOMIC));

        if (table_2d->z_arr[i][j] < -40.) table_2d->z_arr[i][j] = -40.;

        table_mini->z_arr[i][j] = log(Nion_ConditionalM_MINI(
            growthf, lnMmin, lnMmax, log(Mcond), sigma2, overdense_table[i],
            mturns_MINI[j], sc, user_params_global->INTEGRATION_METHOD_MINI));
  ...
  for (i = 0; i < NDELTA; i++) {
    ...
    for (j = 0; j < NMTURN; j++) {
      ...
      if (isfinite(table_2d->z_arr[i][j]) == 0) {
        LOG_ERROR(
            "Detected either an infinite or NaN value in Nion_spline_MINI");
        Throw(TableGenerationError);
      }
    }
  }
...
// since SFRD is not used in Ionisationbox, and reionisation feedback is not
// included in the Ts calculation,
//     The non-minihalo table is always Rx1D and the minihalo table is always
//     Rx2D

// This function initialises one table, for table Rx arrays I will call this
// function in a loop
void initialise_SFRD_Conditional_table(double z, double min_density,
                                       double max_density, double Mmin,
                                       double Mmax, double Mcond,
                                       struct ScalingConstants *sc) {
  float sigma2;
  int i, k;

  LOG_SUPER_DEBUG(
      "Initialising SFRD conditional table at mass %.2e from delta %.2e to "
      "%.2e",
      Mcond, min_density, max_density);
  ...
  double lnMmax = log(Mmax);
  sigma2 =
      EvaluateSigma(lnM_condition);  // sigma is always the condition, whereas
                                     // lnMmax is just the integral limit
  double growthf = dicke(z);

  float MassTurnover[NMTURN];
  for (i = 0; i < NMTURN; i++) {
    MassTurnover[i] =
        pow(10., LOG10_MTURN_MIN + (float)i / ((float)NMTURN - 1.) *
                                       (LOG10_MTURN_MAX - LOG10_MTURN_MIN));
  }

  // NOTE: Here we use the constant Mturn limits instead of variables like in
  // the Nion tables
...
#pragma omp for
    for (i = 0; i < NDELTA; i++) {
      curr_dens = min_density +
                  (float)i / ((float)NDELTA - 1.) * (max_density - min_density);
      SFRD_conditional_table.y_arr[i] =
          log(Nion_ConditionalM(growthf, lnMmin, lnMmax, lnM_condition, sigma2,
                                curr_dens, sc_sfrd.mturn_a_nofb, &sc_sfrd,
                                user_params_global->INTEGRATION_METHOD_ATOMIC));
      ...
      for (k = 0; k < NMTURN; k++) {
        SFRD_conditional_table_MINI.z_arr[i][k] = log(Nion_ConditionalM_MINI(
            growthf, lnMmin, lnMmax, lnM_condition, sigma2, curr_dens,
            MassTurnover[k], &sc_sfrd,
  ...
  for (i = 0; i < NDELTA; i++) {
    if (isfinite(SFRD_conditional_table.y_arr[i]) == 0) {
      LOG_ERROR(
          "Detected either an infinite or NaN value in ACG SFRD conditional "
          "table");
      Throw(TableGenerationError);
    }
    if (!flag_options_global->USE_MINI_HALOS) continue;
    ...
    for (k = 0; k < NMTURN; k++) {
      if (isfinite(SFRD_conditional_table_MINI.z_arr[i][k]) == 0) {
        LOG_ERROR(
            "Detected either an infinite or NaN value in MCG SFRD conditional "
            "table");
        Throw(TableGenerationError);
      }
    }
...
// This function initialises one table, for table Rx arrays I will call this
// function in a loop
void initialise_Xray_Conditional_table(double redshift, double min_density,
                                       double max_density, double Mmin,
                                       double Mmax, double Mcond,
                                       struct ScalingConstants *sc) {
  int i, k;
  ...
  LOG_SUPER_DEBUG(
      "Initialising Xray conditional table at mass %.2e from delta %.2e to "
      "%.2e",
      Mcond, min_density, max_density);
  ...
  if (flag_options_global->USE_MINI_HALOS) {
    ...
    Xray_conditional_table_2D.x_width =
        (max_density - min_density) / (NDELTA - 1.);
    ...
    Xray_conditional_table_1D.x_width =
        (max_density - min_density) / (NDELTA - 1.);
  }
    ...
    for (i = 0; i < NDELTA; i++) {
      ...
      if (!flag_options_global->USE_MINI_HALOS) {
        Xray_conditional_table_1D.y_arr[i] = log(
            Xray_ConditionalM(redshift, growthf, lnMmin, lnMmax, lnM_condition,
                              sigma2, curr_dens, sc->mturn_a_nofb, 0., sc,
                              user_params_global->INTEGRATION_METHOD_ATOMIC));
      ...
      for (k = 0; k < NMTURN; k++) {
        Xray_conditional_table_2D.z_arr[i][k] = log(Xray_ConditionalM(
            ...
            user_params_global
                ->INTEGRATION_METHOD_MINI));  // Using mini integration method
                                              // for both
  ...
  for (i = 0; i < NDELTA; i++) {
    if (!flag_options_global->USE_MINI_HALOS) {
      if (isfinite(Xray_conditional_table_1D.y_arr[i]) == 0) {
        LOG_ERROR(
            "Detected either an infinite or NaN value in 1D Xray conditional "
            "table");
        Throw(TableGenerationError);
      }
      continue;
    ...
    for (k = 0; k < NMTURN; k++) {
      if (isfinite(Xray_conditional_table_2D.z_arr[i][k]) == 0) {
        LOG_ERROR(
            "Detected either an infinite or NaN value in 2D Xray conditional "
            "table");
        Throw(TableGenerationError);
      }
    }
...
void initialise_dNdM_tables(double xmin, double xmax, double ymin, double ymax,
                            double growth_out, double param,
                            bool from_catalog) {
  int nx;
  double lnM_cond = 0.;
  double sigma_cond = 0.;
  LOG_SUPER_DEBUG(
      "Initialising dNdM Tables from [%.2e,%.2e] (Intg. Limits %.2e %.2e)",
      xmin, xmax, ymin, ymax);
  LOG_SUPER_DEBUG("D_out %.2e P %.2e from_cat %d", growth_out, param,
                  from_catalog);
  ...
  for (i = 0; i < nx; i++)
    xa[i] = xmin + (xmax - xmin) * ((double)i) / ((double)nx - 1);
...
#pragma omp parallel num_threads(user_params_global->N_THREADS) private(i) \
...
#pragma omp for
    for (i = 0; i < nx; i++) {
      ...
      if (from_catalog) {
        ...
        delta = get_delta_crit(user_params_global->HMF, sigma_cond, param) /
                param * growth_out;
      } else {
        delta = x;
      }

      Nhalo_table.y_arr[i] = Nhalo_Conditional(growth_out, ymin, ymax, lnM_cond,
                                               sigma_cond, delta, 0);
      Mcoll_table.y_arr[i] = Mcoll_Conditional(growth_out, ymin, ymax, lnM_cond,
                                               sigma_cond, delta, 0);
    }
  }
...
double dndm_inv_f(double lnM_min, void *params) {
  ...
  double integral = Nhalo_Conditional(p->growthf, lnM_min, p->lnM_cond,
                                      p->lnM_cond, p->sigma, p->delta, 0);
  // This ensures that we never find the root if the ratio is zero, since that
  // will set to M_cond
  double result = integral == 0 ? 2 * user_params_global->MIN_LOGPROB
                                : log(integral / p->rf_norm);

  return result - p->rf_target;
}

// This table is N(>M | M_in), the CDF of dNdM_conditional
// NOTE: Assumes you give it ymin as the minimum lower-integral limit, and ymax
// as the maximum
//  `param` is either the constant log condition mass for the grid case
//  (!from_catalog) OR the descendant growth factor with from_catalog
void initialise_dNdM_inverse_table(double xmin, double xmax, double lnM_min,
                                   double growth_out, double param,
                                   bool from_catalog) {
  LOG_SUPER_DEBUG("Initialising dNdM Tables from [%.2e,%.2e] (Intg. Min. %.2e)",
                  xmin, xmax, lnM_min);
  ...
  for (i = 0; i < nx; i++)
    xa[i] = xmin + (xmax - xmin) * ((double)i) / ((double)nx - 1);
  xa[nx - 1] = xmax;  // avoiding floating point errors in final bin due to the
                      // hard boundary at Deltac
  for (k = 0; k < np; k++) {
...
#pragma omp parallel num_threads(user_params_global->N_THREADS) private(i, k) \
    firstprivate(sigma_cond, lnM_cond)
  {
    double x;
    double norm;
...
#pragma omp for
    for (i = 0; i < nx; i++) {
      ...
      if (from_catalog) {
        ...
        delta = x;
        if (delta > MAX_DELTAC_FRAC * get_delta_crit(user_params_global->HMF,
                                                     sigma_cond, growth_out)) {
      ...
      norm = Nhalo_Conditional(growth_out, lnM_min, lnM_cond, lnM_cond,
                               sigma_cond, delta, 0);
      ...
      // if the condition has no halos set the dndm table directly to avoid
      // integration and divide by zero
      if (norm == 0) {
        for (k = 1; k < np - 1; k++)
      ...
      for (k = np - 2; k >= 0; k--) {
        ...
        do {
          ...
          status =
              gsl_root_test_interval(lnM_lo, lnM_hi, rf_tol_abs, rf_tol_rel);
...
void initialise_J_split_table(int Nbin, double umin, double umax,
                              double gamma1) {
  int i;
...
// JD: moving the interp table evaluations here since some of them are needed in
// nu_tau_one NOTE: with !USE_MASS_DEPENDENT_ZETA both EvaluateNionTs and
// EvaluateSFRD return Fcoll
double EvaluateNionTs(double redshift, struct ScalingConstants *sc) {
  ...
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    if (flag_options_global->USE_MASS_DEPENDENT_ZETA)
      return EvaluateRGTable1D(
          redshift, &Nion_z_table);  // the correct table should be passed
    return EvaluateRGTable1D(redshift, &fcoll_z_table);
  }

  // Currently assuming this is only called in the X-ray/spintemp calculation,
  // this will only affect !USE_MASS_DEPENDENT_ZETA and !M_MIN_in_mass
  //       and only if the minimum virial temperatures ION_Tvir_min and
  //       X_RAY_Tvir_min are different
  double lnMmin = log(minimum_source_mass(redshift, true, astro_params_global,
                                          flag_options_global));
  double lnMmax = log(M_MAX_INTEGRAL);

  struct ScalingConstants sc_z = evolve_scaling_constants_to_redshift(
      redshift, astro_params_global, flag_options_global, sc, false);
...
double EvaluateNionTs_MINI(double redshift, double log10_Mturn_LW_ave,
  ...
  return Nion_General_MINI(redshift, lnMmin, lnMmax,
                           pow(10., log10_Mturn_LW_ave), &sc_z);
}
...
double EvaluateSFRD(double redshift, struct ScalingConstants *sc) {
  ...
  // The SFRD calls the same function as N_ion but sets escape fractions to
  // unity NOTE: since this only occurs on integration, the struct copy
  // shouldn't be a bottleneck
  struct ScalingConstants sc_sfrd = evolve_scaling_constants_sfr(sc);
  sc_sfrd = evolve_scaling_constants_to_redshift(
      redshift, astro_params_global, flag_options_global, &sc_sfrd, false);

  if (flag_options_global->USE_MASS_DEPENDENT_ZETA)
    return Nion_General(redshift, lnMmin, lnMmax, sc_sfrd.mturn_a_nofb,
                        &sc_sfrd);
  return Fcoll_General(redshift, lnMmin, lnMmax);
}

double EvaluateSFRD_MINI(double redshift, double log10_Mturn_LW_ave,
                         struct ScalingConstants *sc) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
  ...
  return Nion_General_MINI(redshift, lnMmin, lnMmax,
                           pow(10., log10_Mturn_LW_ave), &sc_sfrd);
}

double EvaluateSFRD_Conditional(double delta, double growthf, double M_min,
  ...
  return Nion_ConditionalM(growthf, log(M_min), log(M_max), log(M_cond),
                           sigma_max, delta, sc_sfrd.mturn_a_nofb, &sc_sfrd,
                           user_params_global->INTEGRATION_METHOD_ATOMIC);
}

double EvaluateSFRD_Conditional_MINI(double delta, double log10Mturn_m,
                                     double growthf, double M_min, double M_max,
                                     double M_cond, double sigma_max,
                                     struct ScalingConstants *sc) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    return exp(
        EvaluateRGTable2D_f(delta, log10Mturn_m, &SFRD_conditional_table_MINI));
  }

  struct ScalingConstants sc_sfrd = evolve_scaling_constants_sfr(sc);
  return Nion_ConditionalM_MINI(growthf, log(M_min), log(M_max), log(M_cond),
                                sigma_max, delta, pow(10, log10Mturn_m),
                                &sc_sfrd,
                                user_params_global->INTEGRATION_METHOD_MINI);
}

double EvaluateNion_Conditional(double delta, double log10Mturn, double growthf,
                                double M_min, double M_max, double M_cond,
                                double sigma_max, struct ScalingConstants *sc,
                                bool prev) {
  RGTable2D_f *table =
      prev ? &Nion_conditional_table_prev : &Nion_conditional_table2D;
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    if (flag_options_global->USE_MINI_HALOS)
  ...
  // NOTE: turning minihalos off turns off feedback in the model. This may be
  // slightly misleading
  //   to ignore a passed parameter but until we make the change in the model we
  //   force it here
  double mturn = flag_options_global->USE_MINI_HALOS ? pow(10, log10Mturn)
                                                     : sc->mturn_a_nofb;
  return Nion_ConditionalM(growthf, log(M_min), log(M_max), log(M_cond),
                           sigma_max, delta, mturn, sc,
                           user_params_global->INTEGRATION_METHOD_ATOMIC);
}

double EvaluateNion_Conditional_MINI(double delta, double log10Mturn_m,
                                     double growthf, double M_min, double M_max,
                                     double M_cond, double sigma_max,
                                     struct ScalingConstants *sc, bool prev) {
  RGTable2D_f *table =
      prev ? &Nion_conditional_table_MINI_prev : &Nion_conditional_table_MINI;
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    return exp(EvaluateRGTable2D_f(delta, log10Mturn_m, table));
  }

  return Nion_ConditionalM_MINI(growthf, log(M_min), log(M_max), log(M_cond),
                                sigma_max, delta, pow(10, log10Mturn_m), sc,
                                user_params_global->INTEGRATION_METHOD_MINI);
}

double EvaluateXray_Conditional(double delta, double log10Mturn_m,
  ...
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
    if (flag_options_global->USE_MINI_HALOS)
      return exp(
          EvaluateRGTable2D_f(delta, log10Mturn_m, &Xray_conditional_table_2D));
  ...
  return Xray_ConditionalM(redshift, growthf, log(M_min), log(M_max),
                           log(M_cond), sigma_max, delta, sc->mturn_a_nofb,
                           pow(10, log10Mturn_m), sc,
                           user_params_global->INTEGRATION_METHOD_MINI);
}

double EvaluateFcoll_delta(double delta, double growthf, double sigma_min,
                           double sigma_max) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
...
double EvaluatedFcolldz(double delta, double redshift, double sigma_min,
                        double sigma_max) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1) {
...
double EvaluateNhalo(double condition, double growthf, double lnMmin,
                     double lnMmax, double M_cond, double sigma, double delta) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1)
    return EvaluateRGTable1D(condition, &Nhalo_table);
  return Nhalo_Conditional(growthf, lnMmin, lnMmax, log(M_cond), sigma, delta,
                           0);
}

double EvaluateMcoll(double condition, double growthf, double lnMmin,
                     double lnMmax, double M_cond, double sigma, double delta) {
  if (user_params_global->USE_INTERPOLATION_TABLES > 1)
    return EvaluateRGTable1D(condition, &Mcoll_table);
  return Mcoll_Conditional(growthf, lnMmin, lnMmax, log(M_cond), sigma, delta,
                           0);
}

// extrapolation function for log-probability based tables
// NOTE: this is very similar to the EvaluateRGTableX function,
//   it may be worth allowing extrapolation there by simply setting the indices
//   to the min/max
double extrapolate_dNdM_inverse(double condition, double lnp) {
  ...
  double extrap_point_y =
      (lnp - user_params_global->MIN_LOGPROB) / Nhalo_inv_table.y_width;
...
double EvaluateNhaloInv(double condition, double prob) {
  if (prob == 0.) return 1.;  // q == 1 -> condition mass
  double lnp = log(prob);
...
double EvaluateJ(double u_res, double gamma1) {
  ...
  double u_max =
      J_split_table.x_min + (J_split_table.n_bin - 1) * J_split_table.x_width;
...
void InitialiseSigmaInverseTable() {
  ...
  for (i = 0; i < n_bin; i++) {
    ...
    ya[i] =
        Sigma_InterpTable.x_min + (n_bin - i - 1) * Sigma_InterpTable.x_width;
  }
...
double EvaluateSigmaInverse(double sigma) {
  if (!(user_params_global->USE_INTERPOLATION_TABLES > 0)) {
    LOG_ERROR(
        "Cannot currently do sigma inverse without USE_INTERPOLATION_TABLES");
    Throw(ValueError);
  }
...
void initialiseSigmaMInterpTable(float M_min, float M_max) {
  ...
  dSigmasqdm_InterpTable.x_width =
      (log(M_max) - log(M_min)) / (N_MASS_INTERP - 1.);
