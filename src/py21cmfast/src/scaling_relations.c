...
void print_sc_consts(struct ScalingConstants *c) {
  ...
  LOG_DEBUG("SHMR: f10 %.2e a %.2e f7 %.2e a_mini %.2e sigma %.2e", c->fstar_10,
            c->alpha_star, c->fstar_7, c->alpha_star_mini, c->sigma_star);
  LOG_DEBUG("Upper: a_upper %.2e pivot %.2e", c->alpha_upper, c->pivot_upper);
  LOG_DEBUG("FESC: f10 %.2e a %.2e f7 %.2e", c->fesc_10, c->alpha_esc,
            c->fesc_7);
  LOG_DEBUG("SSFR: t* %.2e th %.8e sigma %.2e idx %.2e", c->t_star, c->t_h,
            c->sigma_sfr_lim, c->sigma_sfr_idx);
  LOG_DEBUG("Turnovers (nofb) ACG %.2e MCG %.2e Upper %.2e", c->mturn_a_nofb,
            c->mturn_m_nofb, c->acg_thresh);
  LOG_DEBUG("Limits (ACG,MCG) F* (%.2e %.2e) Fesc (%.2e %.2e)", c->Mlim_Fstar,
            c->Mlim_Fstar_mini, c->Mlim_Fesc, c->Mlim_Fesc_mini);
  return;
}

void set_scaling_constants(double redshift, AstroParams *astro_params,
                           FlagOptions *flag_options,
                           struct ScalingConstants *consts,
                           bool use_photoncons) {
  consts->redshift = redshift;

  // Set on for the fixed grid case since we are missing halos above the cell
  // mass
  ...
  consts->upper_pivot_ratio =
      pow(consts->pivot_upper / 1e10, consts->alpha_star) +
  ...
  consts->l_x =
      astro_params->L_X *
      1e-38;  // setting units to 1e38 erg s -1 so we can store in float
  ...
  if (flag_options->USE_MINI_HALOS) {
    consts->vcb_norel =
        flag_options->FIX_VCB_AVG ? astro_params->FIXED_VAVG : 0;
    consts->mturn_m_nofb =
        lyman_werner_threshold(redshift, 0., consts->vcb_norel, astro_params);
  }

  if (flag_options->FIXED_HALO_GRIDS || user_params_global->AVG_BELOW_SAMPLER) {
    consts->Mlim_Fstar = Mass_limit_bisection(
        M_MIN_INTEGRAL, M_MAX_INTEGRAL, consts->alpha_star, consts->fstar_10);
    consts->Mlim_Fesc = Mass_limit_bisection(
        M_MIN_INTEGRAL, M_MAX_INTEGRAL, consts->alpha_esc, consts->fesc_10);

    if (flag_options->USE_MINI_HALOS) {
      consts->Mlim_Fstar_mini = Mass_limit_bisection(
          M_MIN_INTEGRAL, M_MAX_INTEGRAL, consts->alpha_star_mini,
          consts->fstar_7 * pow(1e3, consts->alpha_star_mini));
      consts->Mlim_Fesc_mini = Mass_limit_bisection(
          M_MIN_INTEGRAL, M_MAX_INTEGRAL, consts->alpha_esc,
...
struct ScalingConstants evolve_scaling_constants_sfr(
...
struct ScalingConstants evolve_scaling_constants_to_redshift(
    double redshift, AstroParams *astro_params, FlagOptions *flag_options,
    struct ScalingConstants *sc, bool use_photoncons) {
  ...
  if (flag_options->USE_MINI_HALOS) {
    ...
    sc_z.mturn_m_nofb =
        lyman_werner_threshold(redshift, 0., sc_z.vcb_norel, astro_params);
  }
...
by keeping them in log. Since they are called within integrals they don't use
the ScalingConstants. Instead pulling from the GSL integration parameter structs
*/
...
// single power-law with provided limit for scaling == 1, returns f(M)/f(pivot)
// == norm limit is provided ahead of time for efficiency
double scaling_PL_limit(double M, double norm, double alpha, double pivot,
                        double limit) {
  if ((alpha > 0. && M > limit) || (alpha < 0. && M < limit)) return 1 / norm;

  // if alpha is zero, this returns 1 as expected (note strict inequalities
  // above)
  return scaling_single_PL(M, alpha, pivot);
}

// log version for possible acceleration
double log_scaling_PL_limit(double lnM, double ln_norm, double alpha,
                            double ln_pivot, double ln_limit) {
  if ((alpha > 0. && lnM > ln_limit) || (alpha < 0. && lnM < ln_limit))
    return -ln_norm;

  // if alpha is zero, this returns log(1) as expected (note strict inequalities
  // above)
  return log_scaling_single_PL(lnM, alpha, ln_pivot);
}

// concave-down double power-law, we pass pivot_ratio == denominator(M=pivot_lo)
//   to save pow() calls. Due to the double power-law we gain little from a log
//   verison returns f(M)/f(M==pivot_lo)
double scaling_double_PL(double M, double alpha_lo, double pivot_ratio,
                         double alpha_hi, double pivot_hi) {
  // if alpha is zero, this returns 1 as expected (note strict inequalities
  // above)
  return pivot_ratio /
         (pow(M / pivot_hi, -alpha_lo) + pow(M / pivot_hi, -alpha_hi));
}

/*
Scaling relations used in the halo sampler. Quantities calculated for a single
halo mass and are summed onto grids.
*/

// The mean Lx_over_SFR given by Lehmer+2021, by integrating analyitically over
// their double power-law + exponential Luminosity function NOTE: this relation
// is fit to low-z, and there is a PEAK of Lx/SFR around 10% solar due to the
// critical L term NOTE: this relation currently also has no normalisation, and
// is fixed to the Lehmer values for now
double lx_on_sfr_Lehmer(double metallicity) {
  ...
  double l10break_L =
      38.54 - 38.;  // the -38 written explicitly for clarity on our units
  double l10crit_L = 39.98 - 38. + 0.6 * l10z;
  double L_ratio = pow(10, l10break_L - l10crit_L);

  // The double power-law + exponential integrates to an upper and lower
  // incomplete Gamma function since the slope is < 2 we don't need to set a
  // lower limit, but this can be done by replacing gamma(2-y_low) with
  // gamma_inc(2-y_low,L_lower/L_crit)
  double prefactor_low = pow(10, l10crit_L * (2 - slope_low));
  double prefactor_high = pow(
      10, l10crit_L * (2 - slope_high) + l10break_L * (slope_high - slope_low));
  double gamma_low =
      gsl_sf_gamma(2 - slope_low) - gsl_sf_gamma_inc(2 - slope_low, L_ratio);
  double gamma_high = gsl_sf_gamma_inc(2 - slope_high, L_ratio);

  double lx_over_sfr =
      xray_norm * (prefactor_low * gamma_low + prefactor_high * gamma_high);
...
double lx_on_sfr_doublePL(double metallicity, double lx_constant) {
  ...
  return lx_constant *
         scaling_double_PL(metallicity, lo_z_index, 1., hi_z_index, z_pivot);
}
...
double lx_on_sfr_PL_Kaur(double sfr, double metallicity, double lx_constant) {
  // Hardcoded for now (except the lx normalisation and the scatter): 3 extra
  // fit parameters in the equation taking values from Kaur+22, constant factors
  // controlled by astro_params->L_X
  ...
  double lx_over_sfr =
      (cross_index * l10z + sfr_index) * log10(sfr * SperYR) + z_index * l10z;
...
void get_halo_stellarmass(double halo_mass, double mturn_acg, double mturn_mcg,
                          double star_rng, struct ScalingConstants *consts,
                          double *star_acg, double *star_mcg) {
  ...
  double stoc_adjustment_term =
      consts->scaling_median ? 0 : sigma_star * sigma_star / 2.;

  // We don't want an upturn even with a negative ALPHA_STAR
  if (flag_options_global->USE_UPPER_STELLAR_TURNOVER && (f_a > fu_a)) {
    fstar_mean = scaling_double_PL(halo_mass, f_a, consts->upper_pivot_ratio,
                                   fu_a, fu_p);
  } else {
    fstar_mean =
        scaling_single_PL(halo_mass, consts->alpha_star, 1e10);  // PL term
  }
...
void get_halo_sfr(double stellar_mass, double stellar_mass_mini, double sfr_rng,
                  struct ScalingConstants *consts, double *sfr,
                  double *sfr_mini) {
  ...
  // We use the total stellar mass (MCG + ACG) NOTE: it might be better to
  // separate later
  double sigma_sfr = 0.;

  if (sigma_sfr_lim > 0.) {
    sigma_sfr =
        sigma_sfr_idx * log10((stellar_mass + stellar_mass_mini) / 1e10) +
        sigma_sfr_lim;
  ...
  // adjustment to the mean for lognormal scatter
  double stoc_adjustment_term =
      consts->scaling_median ? 0 : sigma_sfr * sigma_sfr / 2.;
  ...
  *sfr = sfr_sample;
  ...
  sfr_sample_mini =
      sfr_mean_mini * exp(sfr_rng * sigma_sfr - stoc_adjustment_term);
  *sfr_mini = sfr_sample_mini;
}

void get_halo_metallicity(double sfr, double stellar, double redshift,
                          double *z_out) {
...
void get_halo_xray(double sfr, double sfr_mini, double metallicity,
                   double xray_rng, struct ScalingConstants *consts,
                   double *xray_out) {
  double sigma_xray = consts->sigma_xray;

  // adjustment to the mean for lognormal scatter
  double stoc_adjustment_term =
      consts->scaling_median ? 0 : sigma_xray * sigma_xray / 2.;
  ...
  if (flag_options_global->USE_MINI_HALOS) {
    lx_over_sfr = get_lx_on_sfr(
        sfr_mini, metallicity,
        consts->l_x_mini);  // Since there *are* some SFR-dependent models, this
                            // is done separately
