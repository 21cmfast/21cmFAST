...
static double BinWidth_pH, inv_BinWidth_pH, BinWidth_elec, inv_BinWidth_elec,
    BinWidth_10, inv_BinWidth_10, PS_ION_EFF;
...
// approximation for the adiabatic index at z=6-50 from 2302.08506 (also
// 1506.04152). Linear only, used to initialize the Tk box at high z so it's not
// homogeneous. Otherwise half of the adiabatic fluctuations are missing.
// Definition is \delta Tk = Tk * cT * \delta (at each z).
float cT_approx(float z) { return 0.58 - 0.006 * (z - 10.0); }

//* Returns recycling fraction (=fraction of photons converted into Lyalpha for
// Ly-n resonance * //
...
double spectral_emissivity(double nu_norm, int flag, int Population) {
  ...
  switch (flag) {
      ...
      for (i = 1; i < (NSPEC_MAX - 1); i++) {
        if ((nu_norm >= nu_n[i]) && (nu_norm < nu_n[i + 1])) {
          ...
          if (Population == 2) {
            ...
            result = N0_2[i] / (alpha_S_2[i] + 1) *
                     (pow(nu_n[i + 1], alpha_S_2[i] + 1) -
                      pow(nu_norm, alpha_S_2[i] + 1));
            return result > 0 ? result : 1e-40;
          } else {
            result = N0_3[i] / (alpha_S_3[i] + 1) *
                     (pow(nu_n[i + 1], alpha_S_3[i] + 1) -
                      pow(nu_norm, alpha_S_3[i] + 1));
            return result > 0 ? result : 1e-40;
          }
        }
      }
    ...
    case 1:
      ...
      for (i = 1; i < (NSPEC_MAX - 1); i++) {
        n0_fac = (pow(nu_n[i + 1], alpha_S_2[i] + 1) -
                  pow(nu_n[i], alpha_S_2[i] + 1));
        N0_2[i] *= (alpha_S_2[i] + 1) / n0_fac * astro_params_global->POP2_ION;
        n0_fac = (pow(nu_n[i + 1], alpha_S_3[i] + 1) -
                  pow(nu_n[i], alpha_S_3[i] + 1));
...
double kappa_10(double TK, int flag) {
  static double tkin_spline[KAPPA_10_NPTS_Spline],
      kap_spline[KAPPA_10_NPTS_Spline];
  double ans;
  int tkin_spline_int;
  ...
  if (TK < tkin_spline[0]) {  // Below 1 K, just use that value
    ...
    ans = kap_spline[tkin_spline_int] +
          (TK - (tkin_spline[0] + BinWidth_10 * (float)tkin_spline_int)) *
              (kap_spline[tkin_spline_int + 1] - kap_spline[tkin_spline_int]) *
              inv_BinWidth_10;
  }
...
double kappa_10_pH(double T, int flag) {
  static double TK_spline[KAPPA_10_pH_NPTS_Spline],
      kappa_spline[KAPPA_10_pH_NPTS_Spline];
  double ans;
  int TK_spline_int;
  ...
  if (T < TK_spline[0]) {  // Below 1 K, just use that value
    ...
    ans = kappa_spline[KAPPA_10_pH_NPTS_Spline - 1] +
          ((kappa_spline[KAPPA_10_pH_NPTS_Spline - 1] -
           ...
           (TK_spline[KAPPA_10_pH_NPTS_Spline - 1] -
            TK_spline[KAPPA_10_pH_NPTS_Spline - 2]) *
...
double kappa_10_elec(double T, int flag) {
  static double TK_spline[KAPPA_10_elec_NPTS_Spline],
      kappa_spline[KAPPA_10_elec_NPTS_Spline];
  double ans;
  int TK_spline_int;
  ...
  if (T < TK_spline[0]) {  // Below 1 K, just use that value
    ...
    // Power law extrapolation
    ans = kappa_spline[KAPPA_10_elec_NPTS_Spline - 1] +
          ((kappa_spline[KAPPA_10_elec_NPTS_Spline - 1] -
            kappa_spline[KAPPA_10_elec_NPTS_Spline - 2]) /
           (TK_spline[KAPPA_10_elec_NPTS_Spline - 1] -
            TK_spline[KAPPA_10_elec_NPTS_Spline - 2]) *
...
double taugp(double z, double delta, double xe) {
  return 1.342881e-7 / hubble(z) * No * pow(1 + z, 3) * (1.0 + delta) *
         (1.0 - xe);
}
...
double xalpha_tilde(double z, double Jalpha, double TK, double TS, double delta,
                    double xe) {
  double tgp, Stilde, x;
...
double xcoll(double z, double TK, double delta, double xe) {
  return xcoll_HI(z, TK, delta, xe) + xcoll_elec(z, TK, delta, xe) +
         xcoll_prot(z, TK, delta, xe);
}
...
float get_Ts(float z, float delta, float TK, float xe, float Jalpha,
             float *curr_xalpha) {
...
double integrand_in_nu_heat_integral(double nu, void *params) {
  ...
  species_sum += interp_fheat((nu - HeI_NUIONIZATION) / NU_over_EV, x_e) *
                 hplank * (nu - HeI_NUIONIZATION) * f_He * (1 - x_e) *
                 HeI_ion_crosssec(nu);

  // HeII
  species_sum += interp_fheat((nu - HeII_NUIONIZATION) / NU_over_EV, x_e) *
                 hplank * (nu - HeII_NUIONIZATION) * f_He * x_e *
                 HeII_ion_crosssec(nu);

  return species_sum *
         pow(nu / ((astro_params_global->NU_X_THRESH) * NU_over_EV),
...
double integrand_in_nu_lya_integral(double nu, void *params) {
  ...
  // HI
  species_sum = interp_n_Lya((nu - NUIONIZATION) / NU_over_EV, x_e) * f_H *
...
double integrate_over_nu(double zp, double local_x_e, double lower_int_limit,
                         int FLAG) {
  double result, error;
  ...
  status = gsl_integration_qag(
      &F, lower_int_limit, astro_params_global->NU_X_MAX * NU_over_EV, 0,
  ...
  if (status != 0) {
    LOG_ERROR(
        ...
        "result=%e error=%e",
        lower_int_limit, astro_params_global->NU_X_MAX * NU_over_EV, rel_tol,
        result, error);
  ...
  //       if (isnan(result))
  //     fprintf(stderr, "We have a NaN in the intergrator with calling params:
...
// Calculates the optical depth for a photon arriving at z = zp with frequency
// nu, emitted at z = zpp. The filling factor of neutral IGM at zp is
// HI_filling_factor_zp.
//
// *** Brad Greig (22/11/2016) ***
// An approximation to evaluate this using the global averaged filling factor at
// that zp. Same approximation that uses the global averaged x_e
//
// Used to speed up Ts.c and remove parameter dependence reducing the
// dimensionality of the required interpolation table in the new version of
// 21CMMC (including spin-temperature fluctuations).
  ...
  if ((fcoll < 1e-20) && (fcoll_MINI < 1e-20)) {
    ...
    HI_filling_factor_zhat =
        1 - (p->ion_eff * fcoll + p->ion_eff_MINI * fcoll_MINI) /
                (1.0 - p->x_e_ave);  // simplification to use the <x_e> value at
                                     // zp and not zhat.  should'nt matter much
                                     // since the evolution in x_e_ave is slower
                                     // than fcoll.  in principle should make an
                                     // array to store past values of x_e_ave..
  }
  if (HI_filling_factor_zhat < 1e-4)
    HI_filling_factor_zhat =
        1e-4;  // set a floor for post-reionization stability
...
double tauX_MINI(double nu, double x_e, double x_e_ave, double zp, double zpp,
  ...
  p.ion_eff_MINI = astro_params_global->POP3_ION *
  ...
  status = gsl_integration_qag(&F, zpp, zp, 0, rel_tol, 1000, GSL_INTEG_GAUSS15,
                               w, &result, &error);

  if (status != 0) {
    LOG_ERROR("(function argument): zp=%e zpp=%e rel_tol=%e result=%e error=%e",
              zp, zpp, rel_tol, result, error);
    LOG_ERROR("data: nu=%e nu_0=%e x_e=%e x_e_ave=%e", nu, p.nu_0, p.x_e,
              p.x_e_ave);
    LOG_ERROR("data: ion_eff=%e ion_eff_MINI=%e log10_Mturn_MINI=%e", p.ion_eff,
              p.ion_eff_MINI, p.log10_Mturn_MINI);
    CATCH_GSL_ERROR(status);
  }
...
double tauX(double nu, double x_e, double x_e_ave, double zp, double zpp,
  ...
  if (flag_options_global->USE_MASS_DEPENDENT_ZETA) {
    ...
    // if we don't have an explicit ionising efficiency, we estimate one by
    // using the values at zp
    if (HI_filling_factor_zp > FRACT_FLOAT_ERR) {
      fcoll = EvaluateNionTs(
          zp, sc);  // since !USE_MASS_DEPENDENT_ZETA, Mlim doesn't matter
      p.ion_eff = (1.0 - HI_filling_factor_zp) / fcoll * (1.0 - x_e_ave);
      PS_ION_EFF = p.ion_eff;
    } else {
      p.ion_eff =
          PS_ION_EFF;  // uses the previous one in post reionization regime
    }
  }
  F.params = &p;
  ...
  if (status != 0) {
    LOG_ERROR("(function argument): zp=%e zpp=%e rel_tol=%e result=%e error=%e",
              zp, zpp, rel_tol, result, error);
    LOG_ERROR("data: nu=%e nu_0=%e x_e=%e x_e_ave=%e ion_eff=%e", nu,
              nu / (1 + zp), x_e, x_e_ave, p.ion_eff);
    CATCH_GSL_ERROR(status);
  }
  gsl_integration_workspace_free(w);
...
double nu_tau_one_helper(double nu, void *params) {
  nu_tau_one_params *p = (nu_tau_one_params *)params;
  return tauX(nu, p->x_e, p->x_e, p->zp, p->zpp, p->HI_filling_factor_zp,
              p->scale_consts) -
         1;
}
double nu_tau_one_MINI(double zp, double zpp, double x_e,
                       double HI_filling_factor_zp, double log10_Mturn_MINI,
                       struct ScalingConstants *sc) {
  ...
  if (tauX_MINI(HeI_NUIONIZATION, x_e, x_e, zp, zpp, HI_filling_factor_zp,
                log10_Mturn_MINI, sc) < 1)
...
double nu_tau_one(double zp, double zpp, double x_e,
                  double HI_filling_factor_zp, struct ScalingConstants *sc) {
  int status, iter, max_iter;
  const gsl_root_fsolver_type *T;
  gsl_root_fsolver *s;
...
// Useful functions for effeciently navigating through the heating interpolation
// tables
...
int find_nearest_point(double min, double max, int n, double value) {
  ...
  if (value <= (min + dn))
    ...
    pos = (int)floor((value - min) /
                     dn);  // round it down to ensure we are always either side
...
double interpolate_heating_efficiencies(double tk, double ts, double taugp,
                                        double *arrE) {
  tk = log10(tk);
  // Check that the value doesn't exceed the bound of the table. If it does, set
  // it to the edge value (do not interpolate)
  if (tk < Tk_min) {
    tk = Tk_min;
  }
  if (tk > Tk_max) {
  ...
  x0 = Tk_min +
       itk * (Tk_max - Tk_min) /
           (nT -
            1);  // Making these (nT-1) ensures we reach the correct edge value
...
double Energy_Lya_heating(double Tk, double Ts, double tau_gp, int flag) {
  ...
  if (flag == 1) {
    ...
    if (!(F = fopen(filename, "r"))) {
      LOG_ERROR("Energy_Lya_heating: Unable to open file: %s for reading.",
                filename);
      Throw(IOError);
    }
  ...
  if (flag == 2) {
    ans = interpolate_heating_efficiencies(Tk, Ts, tau_gp,
                                           dEC);  // For Continuum Flux
  } else if (flag == 3) {
    ans = interpolate_heating_efficiencies(Tk, Ts, tau_gp,
                                           dEI);  // For Injected Flux
  } else {
